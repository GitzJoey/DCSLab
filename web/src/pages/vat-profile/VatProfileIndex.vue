...
const titleView = ref<string>('views.vat_profile.page_title');
...
const vatProfileNotification = ref<NotificationElement>();

const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[vatProfileNotification]', (el: NotificationElement) => {
  vatProfileNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-product-vat-profile-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.vat_profile.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.vat_profile.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.vat_profile.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (vatProfileNotification.value) vatProfileNotification.value.showToast();
};

const clearCache = (mode: ViewMode) => {
  switch (mode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('VAT_PROFILE_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('VAT_PROFILE_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="vatProfileNotification" :options="{ duration: 3000 }" class="flex">
