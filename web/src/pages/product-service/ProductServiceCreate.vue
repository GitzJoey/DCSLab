<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    loadFromCache();
    await Promise.all([getCategoryDDL(), getUnitDDL(), getVatProfileDDL(), getStatusDDL()]);
    setCompanyIdData();
  });
  // #endregion
  ...
  const loadFromCache = () => {
    let data = cacheServices.getLastEntity('PRODUCT_SERVICE_CREATE') as Record<string, unknown>;
    if (!data) return;
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12">
              ...
              <FormImagesField entity="product" v-model="productServiceForm.image_hashes" />
            </div>
          </div>
        </div>
      </template>
