<script setup lang="ts">
  ...
  import StockAdjustmentOutItemSerialService from '@/services/StockAdjustmentOutItemSerialService';
  ...
  import { StockAdjustmentOutItemSerialReadAnyPaginateRequest } from '@/types/services/stock-adjustment-out-item-serial/StockAdjustmentOutItemSerialRequest';
  ...
  const stockAdjustmentOutItemSerialService = new StockAdjustmentOutItemSerialService();
  ...
  const stockAdjustmentOutItemSerialLists = ref<Collection<Array<StockAdjustmentOutItemSerial>> | null>({
    data: [],
    meta: {
      current_page: 0,
      from: null,
  ...
  onMounted(async () => {
    ...
    await getStockAdjustmentOutItemSerials('', true, 1, 10);
  });

  const getStockAdjustmentOutItemSerials = async (search: string, refresh: boolean, page: number, per_page: number) => {
    emits('loading-state', true);
    searchText.value = search;

    const request: StockAdjustmentOutItemSerialReadAnyPaginateRequest = {
      with_trashed: false,
    ...
    const result: ServiceResponse<Collection<Array<StockAdjustmentOutItemSerial>> | null> =
      await stockAdjustmentOutItemSerialService.readAnyPaginate(request);

    if (result.success && result.data) {
      stockAdjustmentOutItemSerialLists.value = result.data;
  ...
  const handleDataListChange = async (data: DataListEmittedData) => {
    await getStockAdjustmentOutItemSerials(
      data.search.text, 
      true, 
  ...
  const handleDateFilterChange = async () => {
    const perPage = stockAdjustmentOutItemSerialLists.value?.meta.per_page || 10;
    await getStockAdjustmentOutItemSerials(searchText.value, true, 1, perPage);
  };
  ...
  const deleteSelected = (idx: number) => {
    if (!stockAdjustmentOutItemSerialLists.value) return;

    const ulid = stockAdjustmentOutItemSerialLists.value.data[idx].ulid;
  ...
  const confirmDelete = async () => {
    ...
    const result = await stockAdjustmentOutItemSerialService.delete(deleteUlid.value);
  ...
  const getProductMainImageUrl = (item: StockAdjustmentOutItemSerial): string | null => {
    const images = item.stock_adjustment_out_item?.product_unit?.product?.product_images ?? [];
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12 intro-y lg:col-span-12">
      ...
      <DataListFlex
        :data="stockAdjustmentOutItemSerialLists"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="stockAdjustmentOutItemSerialLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="stockAdjustmentOutItemSerialLists ? stockAdjustmentOutItemSerialLists.meta : null"
      ...
      >
        <template #row="{ item, index }">
            <div class="col-span-12 lg:col-span-1 md:col-span-12 flex items-center justify-center md:justify-start">
              <ProductImagePreview
                :image-url="getProductMainImageUrl(item as StockAdjustmentOutItemSerial)"
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.date') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.date ? formatDate(String((item as StockAdjustmentOutItemSerial).stock_adjustment?.date), 'DD-MMM-YYYY HH:mm:ss') : '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.category_id') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.category?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.in_warehouse_id') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.in_warehouse?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.out_warehouse_id') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.out_warehouse?.name ?? '-' }}
              </div>
              <div v-if="(item as StockAdjustmentOutItemSerial).stock_adjustment?.remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.remarks') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment?.remarks }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment_out_item?.product_unit?.product?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.name') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment_out_item?.product_unit?.product?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.unit_code') }}:
                {{ (item as StockAdjustmentOutItemSerial).stock_adjustment_out_item?.product_unit?.code ?? '-' }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-2 md:col-span-2 self-start">
              ...
              <div class="text-slate-500 text-xs break-all">
                {{ (item as StockAdjustmentOutItemSerial).serial || '-' }}
              </div>
            </div>
