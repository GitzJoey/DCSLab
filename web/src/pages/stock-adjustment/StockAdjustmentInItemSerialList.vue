<script setup lang="ts">
  ...
  import StockAdjustmentInItemSerialService from '@/services/StockAdjustmentInItemSerialService';
  import { StockAdjustmentInItemSerial } from '@/types/models/StockAdjustmentInItemSerial';
  ...
  import { StockAdjustmentInItemSerialReadAnyPaginateRequest } from '@/types/services/stock-adjustment-in-item-serial/StockAdjustmentInItemSerialRequest';
  ...
  const stockAdjustmentInItemSerialService = new StockAdjustmentInItemSerialService();
  ...
  const stockAdjustmentInItemSerialLists = ref<Collection<Array<StockAdjustmentInItemSerial>> | null>({
    data: [],
    meta: {
      current_page: 0,
      from: null,
  ...
  onMounted(async () => {
    ...
    await getStockAdjustmentInItemSerials('', true, 1, 10);
  });

  const getStockAdjustmentInItemSerials = async (search: string, refresh: boolean, page: number, per_page: number) => {
    emits('loading-state', true);
    searchText.value = search;

    const request: StockAdjustmentInItemSerialReadAnyPaginateRequest = {
      with_trashed: false,
    ...
    const result: ServiceResponse<Collection<Array<StockAdjustmentInItemSerial>> | null> =
      await stockAdjustmentInItemSerialService.readAnyPaginate(request);

    if (result.success && result.data) {
      stockAdjustmentInItemSerialLists.value = result.data;
  ...
  const handleDataListChange = async (data: DataListEmittedData) => {
    await getStockAdjustmentInItemSerials(
      data.search.text, 
      true, 
  ...
  const handleDateFilterChange = async () => {
    const perPage = stockAdjustmentInItemSerialLists.value?.meta.per_page || 10;
    await getStockAdjustmentInItemSerials(searchText.value, true, 1, perPage);
  };
  ...
  const deleteSelected = (idx: number) => {
    if (!stockAdjustmentInItemSerialLists.value) return;

    const ulid = stockAdjustmentInItemSerialLists.value.data[idx].ulid;
  ...
  const confirmDelete = async () => {
    ...
    const result = await stockAdjustmentInItemSerialService.delete(deleteUlid.value);
  ...
  const getProductMainImageUrl = (item: StockAdjustmentInItemSerial): string | null => {
    const images = item.stock_adjustment_in_item?.product_unit?.product?.product_images ?? [];
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12 intro-y lg:col-span-12">
      ...
      <DataListFlex
        :data="stockAdjustmentInItemSerialLists"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="stockAdjustmentInItemSerialLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="stockAdjustmentInItemSerialLists ? stockAdjustmentInItemSerialLists.meta : null"
      ...
      >
        <template #row="{ item, index }">
            <div class="col-span-12 lg:col-span-1 md:col-span-12 flex items-center justify-center md:justify-start">
              <ProductImagePreview
                :image-url="getProductMainImageUrl(item as StockAdjustmentInItemSerial)"
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.date') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.date ? formatDate(String((item as StockAdjustmentInItemSerial).stock_adjustment?.date), 'DD-MMM-YYYY HH:mm:ss') : '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.category_id') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.category?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.in_warehouse_id') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.in_warehouse?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.out_warehouse_id') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.out_warehouse?.name ?? '-' }}
              </div>
              <div v-if="(item as StockAdjustmentInItemSerial).stock_adjustment?.remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.remarks') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment?.remarks }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment_in_item?.product_unit?.product?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.name') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment_in_item?.product_unit?.product?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.unit_code') }}:
                {{ (item as StockAdjustmentInItemSerial).stock_adjustment_in_item?.product_unit?.code ?? '-' }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-2 md:col-span-2 self-start">
              ...
              <div class="text-slate-500 text-xs break-all">
                {{ (item as StockAdjustmentInItemSerial).serial || '-' }}
              </div>
            </div>
