...
class PurchaseOrderPaymentController extends BaseController
{
    public function __construct(
        private readonly PurchaseOrderPaymentActions $purchaseOrderPaymentActions,
    ) {
        parent::__construct();
    }
    ...
    public function readAny(Request $request)
    {
        ...
        $this->authorize('viewAny', PurchaseOrderPayment::class);
        ...
        try {
            $result = $this->purchaseOrderPaymentActions->readAny(
        ...
        return PurchaseOrderPaymentResource::collection($result);
    }

    public function read(PurchaseOrderPayment $purchaseOrderPayment)
    {
        if (! Auth::check()) return response()->error(trans('auth.unauthenticated'), 401);
        $this->authorize('view', $purchaseOrderPayment);

        $result = null;
        $errorMsg = '';

        try {
            $result = $this->purchaseOrderPaymentActions->read($purchaseOrderPayment);
        } catch (Exception $e) {
            $errorMsg = app()->environment('production') ? '' : $e->getMessage();
        }
        ...
        return new PurchaseOrderPaymentResource($result);
    }
    ...
    public function getAllocationStatuses()
    {
        ...
        return $this->purchaseOrderPaymentActions->getAllocationStatuses();
    }
}
