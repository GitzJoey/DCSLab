<script setup lang="ts">
  ...
  const handleDateFilterChange = async () => {
    const perPage = stockAdjustmentInProductLists.value?.meta.per_page || 10;
    await getStockAdjustmentInProducts(searchText.value, true, 1, perPage);
  };
  ...
  const getProductMainImageUrl = (item: StockAdjustmentInProduct): string | null => {
  ...
  const getBaseUnitName = (item: StockAdjustmentInProduct): string => {
  ...
  const getQtyDisplay = (item: StockAdjustmentInProduct): string => {
  ...
  const getConversionValueDisplay = (item: StockAdjustmentInProduct): string => {
  ...
  const getBaseUnitCogsDisplay = (item: StockAdjustmentInProduct): string => {
    const baseUnitName = getBaseUnitName(item);
  ...
  const getSerialsDisplay = (item: StockAdjustmentInProduct): string => {
  ...
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12 intro-y lg:col-span-12">
      ...
      <DataListFlex
        ...
        :can-print="true"
        :can-export="true"
        :rows="stockAdjustmentInProductLists?.data ?? []"
      ...
      >
        <template #row="{ item, index }">
            ...
            <div class="col-span-12 lg:col-span-1 md:col-span-12 flex items-center justify-center md:justify-start">
              <ProductImagePreview
                :image-url="getProductMainImageUrl(item as StockAdjustmentInProduct)"
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.date') }}:
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.date ? formatDate((item as StockAdjustmentInProduct).stock_adjustment.date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.category_id') }}:
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.category?.name ?? '-' }}
              </div>              
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.in_warehouse_id') }}:
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.in_warehouse?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.out_warehouse_id') }}:
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.out_warehouse?.name ?? '-' }}
              </div>
              <div v-if="(item as StockAdjustmentInProduct).stock_adjustment?.remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.remarks') }}:
                {{ (item as StockAdjustmentInProduct).stock_adjustment?.remarks }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-3 md:col-span-4 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentInProduct).product_unit?.product?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.name') }}:
                {{ (item as StockAdjustmentInProduct).product_unit?.product?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.category_id') }}:
                {{ (item as StockAdjustmentInProduct).product_unit?.product?.category?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.brand_id') }}:
                {{ (item as StockAdjustmentInProduct).product_unit?.product?.brand?.name ?? '-' }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-3 md:col-span-3 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentInProduct).product_unit?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_in_product.table.cols.qty') }}:
                {{ getQtyDisplay(item as StockAdjustmentInProduct) }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_in_product.table.cols.product_unit_cogs') }}:
                {{ formatCurrency((item as StockAdjustmentInProduct).product_unit_cogs) }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_in_product.table.cols.product_unit_total_cogs') }}:
                {{ formatCurrency((item as StockAdjustmentInProduct).product_unit_total_cogs) }}
              </div>
              <div v-if="
                Number((item as StockAdjustmentInProduct).product_unit_conversion_value ?? 1) > 1
                && !((item as StockAdjustmentInProduct).product_unit?.is_base)
              " class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_in_product.table.cols.product_unit_base_unit_cogs') }}:
                {{ getBaseUnitCogsDisplay(item as StockAdjustmentInProduct) }}
              </div>
              <div v-if="(item as StockAdjustmentInProduct).remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_in_product.table.cols.remarks') }}:
                {{ (item as StockAdjustmentInProduct).remarks }}
              </div>
              <div v-if="((item as StockAdjustmentInProduct).serials?.length ?? 0) > 0" class="text-slate-500 text-xs mt-1">
                <span class="font-medium">{{ t('views.product.fields.serial_number') }}:</span>
                <span class="break-all"> {{ getSerialsDisplay(item as StockAdjustmentInProduct) }}</span>
              </div>
            </div>
