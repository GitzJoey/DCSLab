...
import SalesOrderItemList from './SalesOrderItemList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.sales_order_item.page_title');
...
const salesOrderItemNotification = ref<NotificationElement>();

provide('bind[salesOrderItemNotification]', (el: NotificationElement) => {
  salesOrderItemNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.sales_order_item.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (salesOrderItemNotification.value) {
    salesOrderItemNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesOrderItemNotification" :options="{ duration: 3000 }" class="flex">
