    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            $this->mergeWhen($this->relationLoaded('payments'), [
                'payments' => IncomePaymentResource::collection($this->whenLoaded('payments')),
            ]),
            $this->mergeWhen($this->relationLoaded('images'), [
                'income_images' => IncomeImageResource::collection($this->whenLoaded('images')),
            ]),
            $this->mergeWhen($this->relationLoaded('mainImage'), [
                'main_income_image' => new IncomeImageResource($this->whenLoaded('mainImage')),
            ]),
        ];
    }
}
