...
const titleView = ref<string>("views.investor.page_title");
...
const investorNotification = ref<NotificationElement>();
...
provide("bind[investorNotification]", (el: NotificationElement) => {
  investorNotification.value = el;
});
// #endregion
...
const createNew = () => {
  ...
  router.push({ name: "side-menu-company-investor-create" });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = "views.investor.actions.create";
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = "views.investor.actions.edit";
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = "views.investor.page_title";
      break;
  }
};
...
const clearCache = (mode: ViewMode) => {
  switch (mode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity("INVESTOR_CREATE");
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity("INVESTOR_EDIT");
      break;
    default:
      break;
  }
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (investorNotification.value) investorNotification.value.showToast();
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="investorNotification" :options="{ duration: 3000 }" class="flex">
