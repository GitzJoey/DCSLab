...
const titleView = ref<string>('views.expense_payment.page_title');
...
const expensePaymentNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[expensePaymentNotification]', (el: NotificationElement) => {
  expensePaymentNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-expense-payment-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.expense_payment.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.expense_payment.actions.edit';
      break;
    default:
      titleView.value = 'views.expense_payment.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  expensePaymentNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('EXPENSE_PAYMENT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('EXPENSE_PAYMENT_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="expensePaymentNotification" :options="{ duration: 3000 }" class="flex">
