...
import SalesOrderPaymentList from './SalesOrderPaymentList.vue';
import { AllocationStatus } from '@/types/enums/AllocationStatus';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.sales_order_payment.page_title_not_fully_allocated');
...
const salesOrderPaymentNotification = ref<NotificationElement>();

provide('bind[salesOrderPaymentNotification]', (el: NotificationElement) => {
  salesOrderPaymentNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.sales_order_payment.page_title_not_fully_allocated';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (salesOrderPaymentNotification.value) {
    salesOrderPaymentNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesOrderPaymentNotification" :options="{ duration: 3000 }" class="flex">
