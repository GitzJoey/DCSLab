...
import PurchaseOrderPaymentRefundService from '@/services/PurchaseOrderPaymentRefundService';
...
import type { PurchaseOrderPaymentRefundReadAnyPaginateRequest } from '@/types/services/purchase-order-payment-refund/PurchaseOrderPaymentRefundRequest';
import type { ServiceResponse } from '@/types/services/ServiceResponse';
import { formatCurrency, formatDate } from '@/utils/helper';

const { t } = useI18n();
const router = useRouter();
const purchaseOrderPaymentRefundService = new PurchaseOrderPaymentRefundService();
...
const lists = ref<Collection<Array<PurchaseOrderPaymentRefund>> | null>({
  data: [],
...
const getLists = async (search: string, refresh: boolean, page: number, perPage: number) => {
  ...
  const request: PurchaseOrderPaymentRefundReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result = (await purchaseOrderPaymentRefundService.readAnyPaginate(
    request,
  )) as ServiceResponse<Collection<Array<PurchaseOrderPaymentRefund>> | null>;
          ...
          <div class="col-span-12 md:col-span-12 lg:col-span-4 self-start">
            <div class="space-y-2">
              <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                {{ t('views.purchase_order.field_groups.refunded_payments') }}
              </div>
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as PurchaseOrderPaymentRefund).code }}</div>
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderPaymentRefund).date ? formatDate((item as PurchaseOrderPaymentRefund).date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.supplier_id') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderPaymentRefund).purchase_order?.supplier?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-6 lg:col-span-4 self-start md:pr-3 lg:pr-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-5 text-right text-primary font-medium">{{ formatCurrency((item as PurchaseOrderPaymentRefund).amount ?? 0) }}</div>
                ...
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderPaymentRefund).cash_account?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-5 lg:col-span-3 self-start md:pl-3 lg:pl-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderPaymentRefund).purchase_order?.code ?? '-' }}
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              <div class="col-span-12 lg:col-span-6 space-y-3">
                ...
                <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                  ...
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as PurchaseOrderPaymentRefund).company?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.supplier_id') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as PurchaseOrderPaymentRefund).purchase_order?.supplier?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.code') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as PurchaseOrderPaymentRefund).purchase_order?.code ?? '-' }}</div>
                </div>
              </div>
              <div class="col-span-12 lg:col-span-6 space-y-3">
                <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                ...
                <div class="rounded-md border border-slate-200/60 dark:border-darkmode-400 px-3 py-2 text-slate-700 dark:text-slate-200 break-words min-h-[72px]">
                  {{ (item as PurchaseOrderPaymentRefund).remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
