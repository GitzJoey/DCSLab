...
import PurchaseOrderPaymentRefundList from './PurchaseOrderPaymentRefundList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.purchase_order_payment_refund.page_title');
...
const purchaseOrderPaymentRefundNotification = ref<NotificationElement>();

provide('bind[purchaseOrderPaymentRefundNotification]', (el: NotificationElement) => {
  purchaseOrderPaymentRefundNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.purchase_order_payment_refund.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseOrderPaymentRefundNotification.value) {
    purchaseOrderPaymentRefundNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <PurchaseOrderPaymentRefundList
    ...
    <Notification ref-key="purchaseOrderPaymentRefundNotification" :options="{ duration: 3000 }" class="flex">
