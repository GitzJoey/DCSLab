...
const productServiceNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('')
// #endregion

// #region Provide/Inject
provide("bind[productServiceNotification]", (el: NotificationElement) => {
    productServiceNotification.value = el;
});
// #endregion
...
const createNew = () => {
    ...
    router.push({ name: 'side-menu-product-product-service-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
        case ViewMode.FORM_CREATE:
            titleView.value = 'views.product_service.actions.create';
            break;
        case ViewMode.FORM_EDIT:
            titleView.value = 'views.product_service.actions.edit';
            break;
        case ViewMode.INDEX:
        case ViewMode.LIST:
        default:
            titleView.value = 'views.product_service.page_title';
            break;
    }
};
...
const clearCache = (mode: ViewMode) => {
    switch (mode) {
        case ViewMode.FORM_CREATE:
            cacheService.removeLastEntity('PRODUCT_SERVICE_CREATE');
            break;
        case ViewMode.FORM_EDIT:
            cacheService.removeLastEntity('PRODUCT_SERVICE_EDIT');
            break;
        default:
            break;
    }
