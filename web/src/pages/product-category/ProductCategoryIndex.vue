...
const titleView = ref<string>("views.product_category.page_title");
...
const productCategoryNotification = ref<NotificationElement>();
...
provide("bind[productCategoryNotification]", (el: NotificationElement) => {
  productCategoryNotification.value = el;
});
// #endregion
...
const createNew = () => {
  ...
  router.push({ name: "side-menu-product-product-category-create" });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = "views.product_category.actions.create";
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = "views.product_category.actions.edit";
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = "views.product_category.page_title";
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (productCategoryNotification.value) productCategoryNotification.value.showToast();
};

const clearCache = (mode: ViewMode) => {
  switch (mode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity("PRODUCT_CATEGORY_CREATE");
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity("PRODUCT_CATEGORY_EDIT");
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="productCategoryNotification" :options="{ duration: 3000 }" class="flex">
