...
import type { IncomePaymentReadAnyPaginateRequest } from '@/types/services/income-payment/IncomePaymentRequest';
...
const incomePaymentLists = ref<Collection<Array<IncomePayment>> | null>({
  data: [],
...
onMounted(async () => {
  ...
  await getIncomePayments('', true, 1, 10);
});

const getIncomePayments = async (search: string, refresh: boolean, page: number, per_page: number) => {
  emits('loading-state', true);
  const searchReq: IncomePaymentReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<IncomePayment>> | null> =
    await incomePaymentService.readAnyPaginate(searchReq);

  if (result.success && result.data) {
    incomePaymentLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getIncomePayments(data.search.text, false, data.pagination.page, data.pagination.per_page);
};
...
const editSelected = (idx: number) => {
  if (!incomePaymentLists.value) return;
  router.push({
    name: 'side-menu-income-payment-edit',
    params: { ulid: incomePaymentLists.value.data[idx].ulid },
  });
};

const deleteSelected = (idx: number) => {
  if (!incomePaymentLists.value) return;
  deleteUlid.value = incomePaymentLists.value.data[idx].ulid;
...
const confirmDelete = async () => {
  ...
  emits('loading-state', true);
  const result = await incomePaymentService.delete(deleteUlid.value);
  emits('loading-state', false);
  ...
  if (result.success) {
    ...
    await getIncomePayments('', true, 1, 10);
    showNotification(
      t('views.income_payment.alert.delete_income_payment.title'),
      t('views.income_payment.alert.delete_income_payment.content'),
    );
  } else {
    showAlertPlaceholder('danger', '', result.errors as Record<string, Array<string>>);
  }
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12">
      <DataListFlex
        :data="incomePaymentLists"
        :title="t('views.income_payment.table.title')"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="incomePaymentLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="incomePaymentLists ? incomePaymentLists.meta : null"
          ...
          <div class="col-span-12 md:col-span-5 self-start">
            <div class="space-y-2">
              <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                {{ (item as IncomePayment).code }}
              </div>
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                <div class="col-span-4 text-slate-500">{{ t('views.income_payment.fields.income') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ (item as IncomePayment).income?.code ?? '-' }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income_payment.fields.amount') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as IncomePayment).amount ?? 0)) }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income_payment.fields.cash_account') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ (item as IncomePayment).cash_account?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          <div class="col-span-12 md:col-span-5 self-start md:pl-3">
            <div v-if="expandDetail === index" class="rounded-md border border-slate-200/60 p-4 text-xs dark:border-darkmode-400">
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2">
                <div class="col-span-4 text-slate-500">{{ t('views.income_payment.fields.date') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ formatDate((item as IncomePayment).date, 'YYYY-MM-DD HH:mm:ss') }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income_payment.fields.remarks') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ (item as IncomePayment).remarks ?? '-' }}
                </div>
              </div>
            </div>
          </div>
