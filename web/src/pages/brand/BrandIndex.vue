<script setup lang="ts">
  ...
  const titleView = ref<string>('views.brand.page_title');
  ...
  const brandNotification = ref<NotificationElement>();
  ...
  provide('bind[brandNotification]', (el: NotificationElement) => {
    brandNotification.value = el;
  });
  // #endregion
  ...
  const createNew = () => {
    ...
    router.push({ name: 'side-menu-product-brand-create' });
  };
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.FORM_CREATE:
        titleView.value = 'views.brand.actions.create';
        break;
      case ViewMode.FORM_EDIT:
        titleView.value = 'views.brand.actions.edit';
        break;
      case ViewMode.INDEX:
      case ViewMode.LIST:
      default:
        titleView.value = 'views.brand.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (brandNotification.value) brandNotification.value.showToast();
  };

  const clearCache = (mode: ViewMode) => {
    switch (mode) {
      case ViewMode.FORM_CREATE:
        cacheService.removeLastEntity('BRAND_CREATE');
        break;
      case ViewMode.FORM_EDIT:
        cacheService.removeLastEntity('BRAND_EDIT');
        break;
      default:
        break;
    }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="brandNotification" :options="{ duration: 3000 }" class="flex">
