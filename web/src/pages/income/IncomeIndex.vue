...
const titleView = ref<string>('views.income.page_title');
...
const incomeNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[incomeNotification]', (el: NotificationElement) => {
  incomeNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-income-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.income.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.income.actions.edit';
      break;
    default:
      titleView.value = 'views.income.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  incomeNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('INCOME_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('INCOME_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="incomeNotification" :options="{ duration: 3000 }" class="flex">
