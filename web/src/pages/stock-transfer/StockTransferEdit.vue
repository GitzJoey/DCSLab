...
const stockTransferData = ref<StockTransfer | null>(null);
...
const stockTransferForm = stockTransferService.useStockTransferEditForm(route.params.ulid as string);
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('STOCK_TRANSFER_EDIT', newValue.data());
  }, 500),
  { deep: true },
);
...
const loadData = async () => {
  ...
  const result = await stockTransferService.read(ulid);
  ...
  if (result.success && result.data) {
    ...
    const productUnits: StockTransferItemFormItem[] = (data.items || []).map((item: any) => {
      ...
      return {
        ...
        serials: item.serials ? item.serials.map((serial: any) => ({ id: serial.id ?? null, serial: serial.serial })) : [],
        delete_serial_ids: [],
      };
    });
    ...
    stockTransferForm.setData({
      ...
      source_warehouse_id: data.source_warehouse?.id ?? '',
...
const loadSourceWarehouseDDL = async (search = '') => {
  ...
  const result = await warehouseService.readAnyGet({
    ...
    search,
    include_id: stockTransferData.value?.source_warehouse?.id as string | undefined,
    status: undefined,
    refresh: false,
    limit: 20,
  });
...
const loadDestinationWarehouseDDL = async (search = '') => {
  ...
  const result = await warehouseService.readAnyGet({
    ...
    branch_id: selectedUserLocation.value.branch.id,
    search,
    include_id: stockTransferData.value?.destination_warehouse?.id as string | undefined,
    status: undefined,
    refresh: false,
    limit: 20,
  });
        ...
        <div class="p-5">
          ...
          <div v-else class="space-y-5">
            <div
              ...
              :key="item.id ? `${item.id}` : `${item.product_unit_id}-${index}`"
                ...
                <div v-if="item.is_use_serial_number" class="col-span-12">
                  ...
                  <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
                    <div
                      ...
                      :key="serialItem.id ? `${serialItem.id}` : `${index}-${serialIndex}`"
                      class="flex gap-2"
                    >
                      <FormInput
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-3 py-2">[{{ option.product_code }}] {{ option.product_name }}</td>
                <td class="px-3 py-2">
                  {{ option.unit_name }}
                </td>
