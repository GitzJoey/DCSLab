...
import type { IncomeImage } from '@/types/models/IncomeImage';
...
import type { IncomeReadAnyPaginateRequest } from '@/types/services/income/IncomeRequest';
...
const previewImages = ref<IncomeImage[]>([]);
const previewImageIndex = ref(0);
const previewIncomeCode = ref('');
const incomeLists = ref<Collection<Array<Income>> | null>({
  data: [],
...
onMounted(async () => {
  ...
  await getIncomes('', true, 1, 10);
});

const getIncomes = async (search: string, refresh: boolean, page: number, per_page: number) => {
  emits('loading-state', true);
  const searchReq: IncomeReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<Income>> | null> = await incomeService.readAnyPaginate(searchReq);

  if (result.success && result.data) {
    incomeLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getIncomes(data.search.text, false, data.pagination.page, data.pagination.per_page);
};

const editSelected = (idx: number) => {
  if (!incomeLists.value) return;
  router.push({
    name: 'side-menu-income-edit',
    params: { ulid: incomeLists.value.data[idx].ulid },
  });
};
...
const viewSelected = (idx: number) => {
  ...
  const income = incomeLists.value.data[idx];
  expandDetail.value = expandDetail.value === income.ulid ? null : income.ulid;
};

const deleteSelected = (idx: number) => {
  if (!incomeLists.value) return;
  deleteUlid.value = incomeLists.value.data[idx].ulid;
...
const confirmDelete = async () => {
  ...
  emits('loading-state', true);
  const result = await incomeService.delete(deleteUlid.value);
  emits('loading-state', false);

  if (result.success) {
    emits('update-profile');
    await getIncomes('', true, 1, 10);
    showNotification(t('views.income.alert.delete_income.title'), t('views.income.alert.delete_income.content'));
  } else {
    showAlertPlaceholder('danger', '', result.errors as Record<string, Array<string>>);
  }
};
...
const getCategoryLabel = (income: Income) => {
  if (!income.category) return '-';

  return income.category.display_code ? `${income.category.display_code} - ${income.category.name}` : income.category.name;
};

const getCashAccountLabel = (income: Income) => {
  if (!income.paid_immediately_cash_account) return '-';

  return income.paid_immediately_cash_account.code
    ? `${income.paid_immediately_cash_account.code} - ${income.paid_immediately_cash_account.name}`
    : income.paid_immediately_cash_account.name;
};

const getPaidOffLabel = (income: Income) =>
  income.is_amount_receivable_paid_off ? t('views.income.status.paid_off') : t('views.income.status.not_paid_off');

const getIncomeImages = (income: Income): IncomeImage[] => {
  const images = income.income_images?.filter((image) => !!image.url) ?? [];

  if (images.length > 0) {
    return images;
  }

  return income.main_income_image?.url ? [income.main_income_image] : [];
};

const getIncomePayments = (income: Income) => income.payments ?? [];

const getPaymentCashAccountLabel = (payment: IncomePayment) => {
...
const openImagePreview = (income: Income) => {
  const images = getIncomeImages(income);
  ...
  previewIncomeCode.value = income.code;
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12">
      <DataListFlex
        :data="incomeLists"
        :title="t('views.income.table.title')"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="incomeLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="incomeLists ? incomeLists.meta : null"
      ...
      >
        <template #row="{ item, index }">
          <div class="col-span-12 md:col-span-6 lg:col-span-6 self-start">
            <div class="space-y-2">
              <div class="flex items-start justify-between gap-3">
                ...
                <div
                  v-if="(item as Income).main_income_image?.url"
                  ...
                  @click="openImagePreview(item as Income)"
                >
                  <img
                    :src="(item as Income).main_income_image?.url"
              ...
              <div class="grid grid-cols-12 items-start gap-x-3 gap-y-2 text-xs">
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.code') }}</div>
                <div class="col-span-8 break-words font-medium text-slate-700 dark:text-slate-200">
                  {{ (item as Income).code }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.date') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ formatDate((item as Income).date, 'YYYY-MM-DD HH:mm:ss') }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.category') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ getCategoryLabel(item as Income) }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.paid_immediately_cash_account') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ getCashAccountLabel(item as Income) }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.amount_paid_immediately') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  <div class="w-full max-w-[8rem] font-semibold">
                    {{ formatCurrency(Number((item as Income).amount_paid_immediately ?? 0)) }}
                  </div>
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.income.fields.remarks') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ (item as Income).remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-6 lg:col-span-4 self-start lg:pr-4">
            <div class="space-y-2">
              <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                {{ t('views.income.field_groups.receivable_summary') }}
              </div>
              <div class="grid grid-cols-12 items-start gap-x-3 gap-y-2 text-xs">
                <div class="col-span-5 text-slate-500">{{ t('views.income.fields.amount_receivable') }}</div>
                <div class="col-span-7 text-right font-semibold text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Income).amount_receivable ?? 0)) }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.income.fields.due_days') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ (item as Income).due_days ?? 0 }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.income.fields.amount_receivable_paid') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Income).amount_receivable_paid ?? 0)) }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.income.fields.amount_receivable_due') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Income).amount_receivable_due ?? 0)) }}
                </div>
                <div class="col-span-7 text-slate-500">{{ t('views.income.fields.amount_total') }}</div>
                <div class="col-span-5 text-right font-semibold text-primary">
                  {{ formatCurrency(Number((item as Income).amount_total ?? 0)) }}
                </div>
                <div class="col-span-7 text-slate-500">{{ t('views.income.fields.is_amount_receivable_paid_off') }}</div>
                <div class="col-span-5 text-right">
                  <span
                    class="inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium"
                    :class="
                      (item as Income).is_amount_receivable_paid_off
                        ? 'bg-success/20 text-success'
                        : 'bg-warning/20 text-warning'
                    "
                  >
                    {{ getPaidOffLabel(item as Income) }}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div
            v-if="expandDetail === (item as Income).ulid"
          ...
          >
            <div class="grid grid-cols-12 gap-4">
              <div class="col-span-12 space-y-3">
                ...
                <div v-if="!getIncomePayments(item as Income).length" class="text-xs text-slate-500">
                  {{ t('components.data-list.data_not_found') }}
                </div>

                <div v-else class="space-y-3 rounded-md border border-slate-200/60 p-3 dark:border-darkmode-400">
                  <div
                    v-for="payment in getIncomePayments(item as Income)"
                    :key="payment.ulid"
