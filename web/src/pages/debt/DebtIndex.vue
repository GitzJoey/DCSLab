...
const titleView = ref<string>('views.debt.page_title');
...
const debtNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[debtNotification]', (el: NotificationElement) => {
  debtNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-debt-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.debt.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.debt.actions.edit';
      break;
    default:
      titleView.value = 'views.debt.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  debtNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('DEBT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('DEBT_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="debtNotification" :options="{ duration: 3000 }" class="flex">
