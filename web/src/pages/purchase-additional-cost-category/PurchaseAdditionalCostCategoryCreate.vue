...
const purchaseAdditionalCostCategoryForm =
  purchaseAdditionalCostCategoryService.usePurchaseAdditionalCostCategoryCreateForm();
...
const loadFromCache = () => {
  const data = cacheServices.getLastEntity('PURCHASE_ADDITIONAL_COST_CATEGORY_CREATE') as Record<string, unknown>;
  if (!data) return;
  purchaseAdditionalCostCategoryForm.setData(data);
};
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('PURCHASE_ADDITIONAL_COST_CATEGORY_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
</script>
