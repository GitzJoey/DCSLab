...
const debtCategoryLists = ref<Resource<Array<DebtCategory>> | null>({
  data: [],
});
// #endregion
...
onMounted(async () => {
  ...
  await getDebtCategories('', true);
});
// #endregion

// #region Methods
const getDebtCategories = async (search: string, refresh: boolean) => {
  emits('loading-state', true);

  const result: ServiceResponse<Resource<Array<DebtCategory>> | null> = await debtCategoryService.readAnyGet({
    with_trashed: false,
  ...
  if (result.success && result.data) {
    debtCategoryLists.value = result.data;
...
const editSelected = (item: DebtCategory) => {
  router.push({
...
const deleteSelected = (item: DebtCategory) => {
...
const confirmDelete = async () => {
  ...
  const result: ServiceResponse<boolean | null> = await debtCategoryService.delete(deleteUlid.value);

  if (result.success) {
    emits('update-profile');
    await getDebtCategories('', true);
    showNotification(
      t('views.debt_category.alert.delete_debt_category.title'),
      t('views.debt_category.alert.delete_debt_category.content'),
    );
  } else {
...
<template>
  <DataList
    :title="t('views.debt_category.table.title')"
    :enable-search="false"
    :can-print="true"
    :can-export="true"
    :pagination="null"
  ...
  >
    <template #content>
      <Table class="mt-5" :hover="true">
        ...
        <Table.Tbody v-if="debtCategoryLists !== null">
          <template v-if="debtCategoryLists.data.length == 0">
          ...
          <template v-else v-for="item in debtCategoryLists.data" :key="item.ulid">
            <Table.Tr class="intro-x">
              <Table.Td>
              ...
              <Table.Td>
                <div class="flex justify-end gap-1">
                  ...
                  <Button variant="outline-secondary" @click="editSelected(item as DebtCategory)">
                    <Lucide icon="Pen" class="w-4 h-4" />
                  </Button>
                  <Button variant="outline-secondary" @click="deleteSelected(item as DebtCategory)">
