<script setup lang="ts">
  ...
  import StockAdjustmentInProductSerialList from './StockAdjustmentInProductSerialList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_in_product_serial.page_title');
  ...
  const stockAdjustmentInProductSerialNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentInProductSerialNotification]', (el: NotificationElement) => {
    stockAdjustmentInProductSerialNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_in_product_serial.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentInProductSerialNotification.value) {
      stockAdjustmentInProductSerialNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <StockAdjustmentInProductSerialList
    ...
    <Notification ref-key="stockAdjustmentInProductSerialNotification" :options="{ duration: 3000 }" class="flex">
