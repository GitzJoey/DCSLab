<script setup lang="ts">
  ...
  const titleView = ref<string>('views.cash_account.page_title');
  ...
  const cashAccountNotification = ref<NotificationElement>();
  ...
  provide('bind[cashAccountNotification]', (el: NotificationElement) => {
    cashAccountNotification.value = el;
  });
  // #endregion
  ...
  const createNew = () => {
    ...
    router.push({ name: 'side-menu-finance-cash-account-create' });
  };
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.FORM_CREATE:
        titleView.value = 'views.cash_account.actions.create';
        break;
      case ViewMode.FORM_EDIT:
        titleView.value = 'views.cash_account.actions.edit';
        break;
      case ViewMode.INDEX:
      case ViewMode.LIST:
      default:
        titleView.value = 'views.cash_account.page_title';
        break;
    }
  };
  ...
  const clearCache = (mode: ViewMode) => {
    switch (mode) {
      case ViewMode.FORM_CREATE:
        cacheService.removeLastEntity('CASH_ACCOUNT_CREATE');
        break;
      case ViewMode.FORM_EDIT:
        cacheService.removeLastEntity('CASH_ACCOUNT_EDIT');
        break;
      default:
        break;
    }
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (cashAccountNotification.value) cashAccountNotification.value.showToast();
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="cashAccountNotification" :options="{ duration: 3000 }" class="flex">
