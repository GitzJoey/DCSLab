...
const liabilityForm = liabilityService.useLiabilityEditForm(route.params.ulid.toString());
const liabilityData = ref<Liability | null>(null);
...
const loadData = async () => {
  ...
  const result = await liabilityService.read(route.params.ulid.toString());
  ...
  liabilityData.value = result.data;
  ...
  liabilityForm.setData({
    ...
    delete_payment_ids: [],
    payments: (result.data.payments ?? []).map((payment: LiabilityPayment) => ({
      id: payment.id,
      code: payment.code,
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await liabilityCategoryService.readAnyGet({
    ...
    search,
    include_id: liabilityData.value?.category?.id,
    refresh: false,
    limit: 20,
  });
...
const loadCreditorDDL = async (search = '') => {
  ...
  const result = await liabilityCreditorService.readAnyGet({
    ...
    company_id: selectedUserLocation.value.company.id,
    search,
    include_id: liabilityData.value?.creditor?.id ?? undefined,
    refresh: false,
    limit: 20,
  });
...
const loadSupplierDDL = async (search = '') => {
  ...
  const result = await supplierService.readAnyGet({
    ...
    search,
    status: undefined,
    include_id: liabilityData.value?.supplier?.id ?? undefined,
    refresh: false,
    limit: 20,
  });
...
const loadCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: liabilityData.value?.cash_account?.id ?? undefined,
...
const removePayment = (index: number) => {
  ...
  if (payment?.id && !liabilityForm.delete_payment_ids.includes(payment.id)) {
    liabilityForm.delete_payment_ids.push(payment.id);
  }
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <FormErrorMessages :messages="(liabilityForm.errors as any).delete_payment_ids" />
          ...
          <div v-else class="space-y-4">
            <div
              ...
              :key="payment.id ?? `liability-payment-${index}`"
