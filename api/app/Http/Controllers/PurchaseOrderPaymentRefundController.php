...
use App\Http\Resources\PurchaseOrderPaymentRefundResource;
...
class PurchaseOrderPaymentRefundController extends BaseController
{
    public function __construct(
        private readonly PurchaseOrderPaymentRefundActions $purchaseOrderPaymentRefundActions,
    ) {
        parent::__construct();
    }
    ...
    public function readAny(Request $request)
    {
        ...
        $this->authorize('viewAny', PurchaseOrderPaymentRefund::class);
        ...
        try {
            $result = $this->purchaseOrderPaymentRefundActions->readAny(
        ...
        return PurchaseOrderPaymentRefundResource::collection($result);
    }

    public function read(PurchaseOrderPaymentRefund $purchaseOrderPaymentRefund)
    {
        if (! Auth::check()) return response()->error(trans('auth.unauthenticated'), 401);
        $this->authorize('view', $purchaseOrderPaymentRefund);

        $result = null;
        $errorMsg = '';

        try {
            $result = $this->purchaseOrderPaymentRefundActions->read($purchaseOrderPaymentRefund);
        } catch (Exception $e) {
            $errorMsg = app()->environment('production') ? '' : $e->getMessage();
        }
        ...
        return new PurchaseOrderPaymentRefundResource($result);
    }
}
