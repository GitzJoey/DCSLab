<script setup lang="ts">
  ...
  import StockAdjustmentOutItemService from '@/services/StockAdjustmentOutItemService';
  ...
  import { StockAdjustmentOutItemReadAnyPaginateRequest } from '@/types/services/stock-adjustment-out-item/StockAdjustmentOutItemRequest';
  ...
  const stockAdjustmentOutItemService = new StockAdjustmentOutItemService();
  ...
  const stockAdjustmentOutItemLists = ref<Collection<Array<StockAdjustmentOutItem>> | null>({
    data: [],
    meta: {
      current_page: 0,
      from: null,
  ...
  onMounted(async () => {
    ...
    await getStockAdjustmentOutItems('', true, 1, 10);
  });

  const getStockAdjustmentOutItems = async (search: string, refresh: boolean, page: number, per_page: number) => {
    emits('loading-state', true);
    searchText.value = search;

    const request: StockAdjustmentOutItemReadAnyPaginateRequest = {
      with_trashed: false,
    ...
    const result: ServiceResponse<Collection<Array<StockAdjustmentOutItem>> | null> =
      await stockAdjustmentOutItemService.readAnyPaginate(request);

    if (result.success && result.data) {
      stockAdjustmentOutItemLists.value = result.data;
  ...
  const handleDataListChange = async (data: DataListEmittedData) => {
    await getStockAdjustmentOutItems(
      data.search.text, 
      true, 
  ...
  const handleDateFilterChange = async () => {
    const perPage = stockAdjustmentOutItemLists.value?.meta.per_page || 10;
    await getStockAdjustmentOutItems(searchText.value, true, 1, perPage);
  };
  ...
  const deleteSelected = (idx: number) => {
    if (!stockAdjustmentOutItemLists.value) return;

    const ulid = stockAdjustmentOutItemLists.value.data[idx].ulid;
  ...
  const confirmDelete = async () => {
    ...
    const result = await stockAdjustmentOutItemService.delete(deleteUlid.value);
  ...
  const getProductMainImageUrl = (item: StockAdjustmentOutItem): string | null => {
  ...
  const getBaseUnitName = (item: StockAdjustmentOutItem): string => {
  ...
  const getQtyDisplay = (item: StockAdjustmentOutItem): string => {
  ...
  const getConversionValueDisplay = (item: StockAdjustmentOutItem): string => {
  ...
  const getSerialsDisplay = (item: StockAdjustmentOutItem): string => {
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12 intro-y lg:col-span-12">
      ...
      <DataListFlex
        :data="stockAdjustmentOutItemLists"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="stockAdjustmentOutItemLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="stockAdjustmentOutItemLists ? stockAdjustmentOutItemLists.meta : null"
      ...
      >
        <template #row="{ item, index }">
            <div class="col-span-12 lg:col-span-1 md:col-span-12 flex items-center justify-center md:justify-start">
              <ProductImagePreview
                :image-url="getProductMainImageUrl(item as StockAdjustmentOutItem)"
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.date') }}:
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.date ? formatDate((item as StockAdjustmentOutItem).stock_adjustment.date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.category_id') }}:
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.category?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.in_warehouse_id') }}:
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.in_warehouse?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.out_warehouse_id') }}:
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.out_warehouse?.name ?? '-' }}
              </div>
              <div v-if="(item as StockAdjustmentOutItem).stock_adjustment?.remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment.fields.remarks') }}:
                {{ (item as StockAdjustmentOutItem).stock_adjustment?.remarks }}
              </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-3 md:col-span-4 self-start">
              ...
              <div class="text-slate-500 text-xs whitespace-nowrap">
                ...
                {{ (item as StockAdjustmentOutItem).product_unit?.product?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.name') }}:
                {{ (item as StockAdjustmentOutItem).product_unit?.product?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.category_id') }}:
                {{ (item as StockAdjustmentOutItem).product_unit?.product?.category?.name ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.brand_id') }}:
                {{ (item as StockAdjustmentOutItem).product_unit?.product?.brand?.name ?? '-' }}
              </div>
            </div>

            <div class="col-span-12 lg:col-span-3 md:col-span-3 self-start">
              <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
                {{ t('views.stock_adjustment_out_item.table.cols.product_unit') }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.product.fields.unit_code') }}:
                {{ (item as StockAdjustmentOutItem).product_unit?.code ?? '-' }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_out_item.table.cols.qty') }}:
                {{ getQtyDisplay(item as StockAdjustmentOutItem) }}
              </div>
              <div class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_out_item.table.cols.product_unit_conversion_value') }}:
                {{ getConversionValueDisplay(item as StockAdjustmentOutItem) }}
              </div>
              <div v-if="(item as StockAdjustmentOutItem).remarks?.trim()" class="text-slate-500 text-xs whitespace-nowrap">
                {{ t('views.stock_adjustment_out_item.table.cols.remarks') }}:
                {{ (item as StockAdjustmentOutItem).remarks }}
              </div>
              <div v-if="((item as StockAdjustmentOutItem).serials?.length ?? 0) > 0" class="text-slate-500 text-xs mt-1">
                <span class="font-medium">{{ t('views.product.fields.serial_number') }}:</span>
                <span class="break-all"> {{ getSerialsDisplay(item as StockAdjustmentOutItem) }}</span>
              </div>
            </div>
