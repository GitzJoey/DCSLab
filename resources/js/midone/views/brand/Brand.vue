<template>
    ...
    <div class="intro-y" v-if="mode === 'list'">
        <DataList
            :title="t('views.brand.table.title')"
            :data="brandList"
            v-on:createNew="createNew"
            v-on:dataListChange="onDataListChange"
            :enableSearch="true"
        >
                                ...
                                <td class="table-report__action w-12">
                                    ...
                                    >
                                        <Tippy
                                            tag="a"
                                            href="javascript:;"
                                            class="tooltip p-2 hover:border"
                                            :content="
                                                t('components.data-list.view')
                                            "
                                            @click.prevent="
                                                showSelected(itemIdx)
                                            "
                                        >
                                        ...
                                        <Tippy
                                            ...
                                            class="tooltip p-2 hover:border"
                                            :content="
                                                t('components.data-list.delete')
                                            "
                                            @click.prevent="
                                                deleteSelected(itemIdx)
                                            "
                                        >
                                            <Trash2Icon
                    ...
                    <ModalBody class="p-0">
                        <div class="p-5 text-center">
                            ...
                            <div class="text-3xl mt-5">
                                {{
                                    t(
                                        "components.data-list.delete_confirmation.title"
                                    )
                                }}
                            </div>
                            <div class="text-slate-600 mt-2">
                                {{
                                    t(
                                        "components.data-list.delete_confirmation.desc_1"
                                    )
                                }}<br />{{
                                    t(
                                        "components.data-list.delete_confirmation.desc_2"
                                    )
                                }}
                            </div>
                        </div>
    ...
    <div class="intro-y box" v-if="mode !== 'list'">
        ...
        >
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'create'">
                {{ t("views.brand.actions.create") }}
            </h2>
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'edit'">
                {{ t("views.brand.actions.edit") }}
            </h2>
        </div>
        <div class="loader-container">
            ...
            >
                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex items-center">
                            <VeeField
                                ...
                                rules="required"
                                @blur="reValidate(errors)"
                                v-model="brand.code"
                                :readonly="brand.code === '[AUTO]'"
                            />
                            <button
                                type="button"
                ...
                <div class="pl-5" v-if="mode === 'create' || mode === 'edit'">
                    <button type="submit" class="btn btn-primary w-24 mr-3">
        ...
        <div>
            <button
                type="button"
                class="btn btn-secondary w-15 m-3"
                @click="backToList"
            >
...
const selectedUserCompany = computed(
    () => userContextStore.selectedUserCompany
);
//#endregion
...
const getAllProductGroups = (args) => {
    ...
    axios
        .get(
            route("api.get.db.product.brand.read", {
                companyId: companyId,
                page: args.page,
...
const getDDL = () => {
    if (getCachedDDL("productCategoryDDL") == null) {
        axios
            .get(route("api.get.db.common.ddl.list.productcategory"))
            .then((response) => {
...
const getDDLSync = () => {
    axios
        .get(
            route("api.get.db.company.company.read.all_active", {
...
const onSubmit = (values, actions) => {
    ...
    if (mode.value === "create") {
        axios
            .post(route("api.post.db.product.brand.save"), formData)
            .then((response) => {
                backToList();
            })
            .catch((e) => {
        ...
        axios
            .post(
                route("api.post.db.product.brand.edit", brand.value.hId),
                formData
            )
            .then((response) => {
                actions.resetForm();
...
const invalidSubmit = (e) => {
    ...
    if (dom(".border-danger").length !== 0)
        dom(".border-danger")[0].scrollIntoView({ behavior: "smooth" });
};
...
const confirmDelete = () => {
    ...
    axios
        .post(route("api.post.db.product.brand.delete", deleteId.value))
        .then((response) => {
            backToList();
        })
        .catch((e) => {
...
const backToList = () => {
    ...
    mode.value = "list";
    getAllProductGroups({
        page: brandList.value.meta.current_page,
        pageSize: brandList.value.meta.per_page,
    });
};
...
watch(
    ...
    (newV) => {
        if (mode.value == "create")
            sessionStorage.setItem("DCSLAB_LAST_ENTITY", JSON.stringify(newV));
    },
    { deep: true }
);
//#endregion
