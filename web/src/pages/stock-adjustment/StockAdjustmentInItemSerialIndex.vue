<script setup lang="ts">
  ...
  import StockAdjustmentInItemSerialList from './StockAdjustmentInItemSerialList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_in_item_serial.page_title');
  ...
  const stockAdjustmentInItemSerialNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentInItemSerialNotification]', (el: NotificationElement) => {
    stockAdjustmentInItemSerialNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_in_item_serial.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentInItemSerialNotification.value) {
      stockAdjustmentInItemSerialNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <StockAdjustmentInItemSerialList
    ...
    <Notification ref-key="stockAdjustmentInItemSerialNotification" :options="{ duration: 3000 }" class="flex">
