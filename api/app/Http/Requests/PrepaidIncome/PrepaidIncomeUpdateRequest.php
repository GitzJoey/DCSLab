...
namespace App\Http\Requests\PrepaidIncome;
...
class PrepaidIncomeUpdateRequest extends FormRequest
{
    public function authorize()
    {
        ...
        /** @var \App\User */
        $user = Auth::user();
        $prepaidIncome = $this->route('prepaid_income');

        return $user->can('update', $prepaidIncome);
    }
    ...
    public function rules()
    {
        ...
        return [
            ...
            'delete_image_ids.*' => [
                'required',
                'integer',
                Rule::exists('prepaid_income_images', 'id')->where(function ($query) use ($prepaidIncome) {
                    $query->where('prepaid_income_id', $prepaidIncome->id);
                }),
            ],

            'image_hashes' => ['present', 'array'],
            'image_hashes.*.hash' => ['required', 'string', Rule::exists('prepaid_income_images', 'hash')],
            ...
            'delete_payment_ids.*' => [
                ...
                'integer',
                'distinct',
                Rule::exists('prepaid_income_payments', 'id')->where(function ($query) use ($prepaidIncome) {
                    $query->where('company_id', $this->company_id)
                        ->where('prepaid_income_id', $prepaidIncome->id);
                }),
            ],
        ...
        $validator->after(function ($validator) {
            ...
            if ($amountPaidImmediately > 0 && empty($this->input('paid_immediately_cash_account_id'))) {
                $validator->errors()->add('paid_immediately_cash_account_id', trans('validation.required', [
                    'attribute' => trans('validation_attributes.prepaid_income.paid_immediately_cash_account_id'),
                ]));
            }

            if ($amountPaidImmediately <= 0 && $amountReceivable <= 0) {
                $validator->errors()->add('amount_total', trans('rules.prepaid_income.amount_total_must_be_positive'));
            }

            if ($paymentTotal > $amountReceivable) {
                $validator->errors()->add('payments', trans('rules.prepaid_income.payments_exceed_amount_receivable'));
            }

            if (! $validator->errors()->has('income_category_id')) {
                $prepaidIncomeCategory = IncomeCategory::query()
                    ->select('id')
                    ->find($this->input('income_category_id'));

                if ($prepaidIncomeCategory && $prepaidIncomeCategory->children()->exists()) {
    ...
    {
        return array_merge(
            trans('validation_attributes.prepaid_income'),
            [
                'payments.*.id' => trans('validation_attributes.prepaid_income_payment.id'),
                'payments.*.code' => trans('validation_attributes.prepaid_income_payment.code'),
                'payments.*.date' => trans('validation_attributes.prepaid_income_payment.date'),
                'payments.*.cash_account_id' => trans('validation_attributes.prepaid_income_payment.cash_account_id'),
                'payments.*.amount' => trans('validation_attributes.prepaid_income_payment.amount'),
                'payments.*.remarks' => trans('validation_attributes.prepaid_income_payment.remarks'),
            ],
        );
    }
    ...
    {
        return [
            'delete_payment_ids.*.exists' => trans('rules.prepaid_income.invalid_payment_reference'),
            'payments.*.id.exists' => trans('rules.prepaid_income.invalid_payment_reference'),
        ];
    }
}
