...
import StockTransferItemService from '@/services/StockTransferItemService';
...
import { StockTransferItemReadAnyPaginateRequest } from '@/types/services/stock-transfer-item/StockTransferItemRequest';
...
const stockTransferItemService = new StockTransferItemService();
...
const stockTransferItemLists = ref<Collection<Array<StockTransferItem>> | null>({
  data: [],
  meta: {
    current_page: 1,
    from: null,
...
onMounted(async () => {
  ...
  await getStockTransferItems('', true, 1, 10);
});

const getStockTransferItems = async (search: string, refresh: boolean, page: number, per_page: number) => {
  setLoadingState(true);
  searchText.value = search;
  const safePage = page > 0 ? page : 1;
  const safePerPage = per_page > 0 ? per_page : 10;

  const request: StockTransferItemReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<StockTransferItem>> | null> =
    await stockTransferItemService.readAnyPaginate(request);

  if (result.success && result.data) {
    stockTransferItemLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getStockTransferItems(data.search.text, true, data.pagination.page, data.pagination.per_page);
};

const handleDateFilterChange = async () => {
  const perPage = stockTransferItemLists.value?.meta.per_page || 10;
  await getStockTransferItems(searchText.value, true, 1, perPage);
};
...
const editSelected = (idx: number) => {
  if (!stockTransferItemLists.value) return;

  const ulid = stockTransferItemLists.value.data[idx].stock_transfer?.ulid;
  if (!ulid) return;
...
const getProductMainImageUrl = (item: StockTransferItem): string | null => {
...
const getBaseUnitName = (item: StockTransferItem): string => {
...
const getQtyDisplay = (item: StockTransferItem): string => {
...
const getConversionValueDisplay = (item: StockTransferItem): string => {
...
const getSerialsDisplay = (item: StockTransferItem): string => {
...
<template>
  <div>
    <LoadingOverlay :visible="loading">
      <TitleLayout>
        <template #title>
          {{ t('views.stock_transfer_item.page_title') }}
        </template>
      </TitleLayout>
      ...
      <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 intro-y lg:col-span-12">
          ...
          <DataListFlex :data="stockTransferItemLists" :enable-search="true" :can-print="true"
              :can-export="true" :rows="stockTransferItemLists?.data ?? []"
              row-class="bg-white dark:bg-darkmode-600"
              :pagination="stockTransferItemLists ? stockTransferItemLists.meta : null"
              ...
              <template #row="{ item, index }">
                <div class="col-span-12 lg:col-span-1 md:col-span-12 flex items-center justify-center md:justify-start">
                  <ProductImagePreview :image-url="getProductMainImageUrl(item as StockTransferItem)"
                ...
                <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
                  ...
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    ...
                    {{ (item as StockTransferItem).stock_transfer?.code ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer.fields.date') }}:
                    {{ formatStockTransferDate((item as StockTransferItem).stock_transfer?.date) }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer.fields.source_warehouse_id') }}:
                    {{ (item as StockTransferItem).stock_transfer?.source_warehouse?.name ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer.fields.destination_warehouse_id') }}:
                    {{ (item as StockTransferItem).stock_transfer?.destination_warehouse?.name ?? '-' }}
                  </div>
                  <div v-if="(item as StockTransferItem).stock_transfer?.remarks?.trim()"
                    class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer.fields.remarks') }}:
                    {{ (item as StockTransferItem).stock_transfer?.remarks }}
                  </div>
                </div>
                ...
                <div class="col-span-12 lg:col-span-3 md:col-span-4 self-start">
                  ...
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    ...
                    {{ (item as StockTransferItem).product_unit?.product?.code ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.product.fields.name') }}:
                    {{ (item as StockTransferItem).product_unit?.product?.name ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.product.fields.category_id') }}:
                    {{ (item as StockTransferItem).product_unit?.product?.category?.name ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.product.fields.brand_id') }}:
                    {{ (item as StockTransferItem).product_unit?.product?.brand?.name ?? '-' }}
                  </div>
                </div>

                <div class="col-span-12 lg:col-span-3 md:col-span-3 self-start">
                  <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
                    {{ t('views.stock_transfer_item.table.cols.product_unit') }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.product.fields.unit_code') }}:
                    {{ (item as StockTransferItem).product_unit?.code ?? '-' }}
                  </div>
                  <div class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer_item.table.cols.qty') }}:
                    {{ getQtyDisplay(item as StockTransferItem) }}
                  </div>
                  <div v-if="Number((item as StockTransferItem).product_unit_conversion_value ?? 1) > 1"
                    class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer_item.table.cols.product_unit_conversion_value') }}:
                    {{ getConversionValueDisplay(item as StockTransferItem) }}
                  </div>
                  <div v-if="(item as StockTransferItem).remarks?.trim()"
                    class="text-slate-500 text-xs whitespace-nowrap">
                    {{ t('views.stock_transfer_item.table.cols.remarks') }}:
                    {{ (item as StockTransferItem).remarks }}
                  </div>
                  <div v-if="((item as StockTransferItem).serials?.length ?? 0) > 0"
                    class="text-slate-500 text-xs mt-1">
                    <span class="font-medium">{{ t('views.product.fields.serial_number') }}:</span>
                    <span class="break-all"> {{ getSerialsDisplay(item as StockTransferItem) }}</span>
                  </div>
                </div>
                ...
                <div v-if="expandDetail === index"
                  ...
                  <div v-else class="text-slate-500 text-xs break-all">
                    {{(item as StockTransferItem).serials.map((serial) => serial.serial).join(', ')}}
                  </div>
                </div>
              </template>
          </DataListFlex>
