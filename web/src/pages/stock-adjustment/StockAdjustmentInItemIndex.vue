<script setup lang="ts">
  ...
  import StockAdjustmentInItemList from './StockAdjustmentInItemList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_in_item.page_title');
  ...
  const stockAdjustmentInItemNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentInItemNotification]', (el: NotificationElement) => {
    stockAdjustmentInItemNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_in_item.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentInItemNotification.value) {
      stockAdjustmentInItemNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="stockAdjustmentInItemNotification" :options="{ duration: 3000 }" class="flex">
