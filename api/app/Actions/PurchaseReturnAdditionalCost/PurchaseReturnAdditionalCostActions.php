...
class PurchaseReturnAdditionalCostActions
{
    ...
    public function readAny(
        ...
        $query = PurchaseReturnAdditionalCost::select('purchase_return_additional_costs.*')
            ->with(['company'])
            ->join('companies', 'companies.id', '=', 'purchase_return_additional_costs.company_id')
            ->whereCompanyId('purchase_return_additional_costs', $companyId)
            ->withTrashed();

        $query->where(function ($query) use ($withTrashed, $search, $branchId, $purchaseId, $categoryId) {
            ...
            if ($branchId) {
                $query->where('purchase_return_additional_costs.branch_id', $branchId);
            }

            if ($purchaseId) {
                $query->where('purchase_return_additional_costs.purchase_id', $purchaseId);
            }

            if ($categoryId) {
                $query->where('purchase_return_additional_costs.category_id', $categoryId);
            }
        });
        ...
        if ($execute) {
            ...
            try {
                ...
                $cacheKey = 'read_any_purchase_return_additional_cost_'.implode('_', $cacheParams);
    ...
    {
        return $purchaseReturnAdditionalCost->load('company');
    }
