...
export default {
    ...
    methods: {
        ...
        onSubmit(values, actions) {
            this.loading = true;
            if (this.mode === 'create') {
                axios.post(route('api.post.dashboard.company.branches.save'), new FormData($('#branchForm')[0])). then(response => {
                    this.backToList();
                }).catch(e => {
                    this.handleError(e, actions);
                    this.loading = false;
                ...
                axios.post(route('api.post.dashboard.company.branches.edit', this.branch.hId), new FormData($('#branchForm')[0])).then(response => {
                    this.backToList();
                }).catch(e => {
                    this.handleError(e, actions);
                    this.loading = false;
