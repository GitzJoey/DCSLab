...
const titleView = ref<string>('views.asset_category.page_title');
...
const assetCategoryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[assetCategoryNotification]', (el: NotificationElement) => {
  assetCategoryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-asset-category-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.asset_category.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.asset_category.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.asset_category.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (assetCategoryNotification.value) {
    assetCategoryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('ASSET_CATEGORY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('ASSET_CATEGORY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="assetCategoryNotification" :options="{ duration: 3000 }" class="flex">
