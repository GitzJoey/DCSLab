...
class PurchaseAdditionalCostPaymentController extends BaseController
{
    ...
    public function store(PurchaseAdditionalCostPaymentStoreRequest $request)
    {
        ...
        try {
            ...
            DB::beginTransaction();
            $dto = new PurchaseAdditionalCostPaymentCreateDTO(
                companyId: $validatedRequest['company_id'],
                branchId: $validatedRequest['branch_id'],
                code: $validatedRequest['code'],
                date: $validatedRequest['date'],
                purchaseAdditionalCostId: $validatedRequest['purchase_additional_cost_id'],
                cashAccountId: $validatedRequest['cash_account_id'],
                amount: $validatedRequest['amount'],
                remarks: $validatedRequest['remarks'],
            );
            $result = $this->purchaseAdditionalCostPaymentActions->create($dto);
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
            $errorMsg = app()->environment('production') ? '' : $e->getMessage();
    ...
    public function update(PurchaseAdditionalCostPayment $purchase_additional_cost_payment, PurchaseAdditionalCostPaymentUpdateRequest $request)
    {
        ...
        try {
            ...
            DB::beginTransaction();
            $result = $this->purchaseAdditionalCostPaymentActions->update($purchase_additional_cost_payment, new PurchaseAdditionalCostPaymentUpdateDTO(
                code: $validatedRequest['code'],
                date: $validatedRequest['date'],
                cashAccountId: $validatedRequest['cash_account_id'],
                amount: $validatedRequest['amount'],
