...
const titleView = ref<string>('views.purchase_additional_cost_payment.page_title');
...
const purchaseAdditionalCostPaymentNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[purchaseAdditionalCostPaymentNotification]', (el: NotificationElement) => {
  purchaseAdditionalCostPaymentNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-purchase-additional-cost-payment-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.purchase_additional_cost_payment.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.purchase_additional_cost_payment.actions.edit';
      break;
    default:
      titleView.value = 'views.purchase_additional_cost_payment.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  purchaseAdditionalCostPaymentNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_PAYMENT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_PAYMENT_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseAdditionalCostPaymentNotification" :options="{ duration: 3000 }" class="flex">
