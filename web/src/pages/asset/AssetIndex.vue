...
const titleView = ref<string>('views.asset.page_title');
...
const assetNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[assetNotification]', (el: NotificationElement) => {
  assetNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-asset-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.asset.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.asset.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.asset.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (assetNotification.value) {
    assetNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('ASSET_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('ASSET_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="assetNotification" :options="{ duration: 3000 }" class="flex">
