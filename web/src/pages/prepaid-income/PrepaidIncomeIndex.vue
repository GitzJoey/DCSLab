...
const titleView = ref<string>('views.prepaid_income.page_title');
...
const prepaidIncomeNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[prepaidIncomeNotification]', (el: NotificationElement) => {
  prepaidIncomeNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-prepaid-income-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.prepaid_income.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.prepaid_income.actions.edit';
      break;
    default:
      titleView.value = 'views.prepaid_income.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  prepaidIncomeNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('PREPAID_INCOME_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PREPAID_INCOME_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="prepaidIncomeNotification" :options="{ duration: 3000 }" class="flex">
