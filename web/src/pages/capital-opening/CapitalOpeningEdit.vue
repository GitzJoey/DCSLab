...
const capitalOpeningForm = capitalOpeningService.useCapitalOpeningEditForm(route.params.ulid as string);
const capitalOpeningData = ref<CapitalOpening | null>(null);
...
const loadData = async (ulid: string) => {
  ...
  const result: ServiceResponse<CapitalOpening | null> = await capitalOpeningService.read(ulid);

  if (result.success && result.data) {
    capitalOpeningData.value = result.data;
...
const loadInvestorDDL = async (search = '') => {
  ...
  const result = await investorService.readAnyGet({
    ...
    search,
    include_id: capitalOpeningData.value?.investor?.id,
    refresh: false,
    limit: 20,
  });
...
const loadCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    search,
    include_id: capitalOpeningData.value?.cash_account?.id,
    refresh: false,
    limit: 20,
  });
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('CAPITAL_OPENING_EDIT', newValue.data());
  }, 500),
  { deep: true },
);
</script>
