...
class StockTransferUpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'delete_item_ids.*' => ['required', 'integer', new ExistsForCompany('stock_transfer_items', $this->company_id)],

            'items' => ['required', 'array', 'min:1'],
            'items.*.id' => ['present', 'nullable', 'integer', new ExistsForCompany('stock_transfer_items', $this->company_id)],
            ...
            'items.*.delete_serial_ids.*' => ['required', 'integer', new ExistsForCompany('stock_transfer_item_serials', $this->company_id)],
            'items.*.serials' => ['present', 'array'],
            'items.*.serials.*.id' => ['present', 'nullable', 'integer', new ExistsForCompany('stock_transfer_item_serials', $this->company_id)],
    ...
    public function prepareForValidation()
    {
        ...
        if (is_array($this->input('delete_item_ids'))) {
            ...
            foreach ($this->input('delete_item_ids') as $item) {
                $deleteProductUnitIds[] = HashidsHelper::decodeId($item);
            }
            $this->merge(['delete_item_ids' => $deleteProductUnitIds]);
        }
