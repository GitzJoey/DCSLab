...
const incomeForm = incomeService.useIncomeEditForm(route.params.ulid.toString());
const incomeData = ref<Income | null>(null);
const uploadedImages = ref<IncomeImage[]>([]);
...
const loadData = async () => {
  ...
  const result = await incomeService.read(route.params.ulid.toString());
  ...
  if (result.success && result.data) {
    ...
    incomeForm.setData({
      ...
      image_hashes: [],
      delete_payment_ids: [],
      payments: (result.data.payments ?? []).map((payment: IncomePayment) => ({
        id: payment.id,
        code: payment.code,
    ...
    if (result.data.income_images) {
      const images = result.data.income_images.map((img: IncomeImage) => ({
        id: img.id,
        income_id: img.income_id,
        path: img.path,
        url: img.url,
        hash: img.hash,
        is_main: img.is_main,
      ...
      incomeForm.image_hashes = images.map((img: IncomeImage) => ({
        hash: img.hash,
        is_main: img.is_main,
      }));
    } else {
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await incomeCategoryService.readAnyGet({
    ...
    has_children: false,
    include_id: incomeData.value?.category?.id,
    refresh: false,
    limit: 20,
  });
...
const loadPaidImmediatelyCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: incomeData.value?.paid_immediately_cash_account?.id,
...
const removePayment = (index: number) => {
  ...
  if (payment?.id && !incomeForm.delete_payment_ids.includes(payment.id)) {
    incomeForm.delete_payment_ids.push(payment.id);
  }
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <FormErrorMessages :messages="(incomeForm.errors as any).delete_payment_ids" />
          ...
          <div v-else class="space-y-4">
            <div v-for="(payment, index) in incomePaymentsForm" :key="payment.id ?? `income-payment-${index}`" class="space-y-3">
              <div class="grid grid-cols-12 gap-4 gap-y-3">
      ...
      <template #card-items-3>
        <div class="p-5">
          ...
          <IncomeImagesField
            v-model="incomeForm.image_hashes"
            v-model:existing-images="uploadedImages"
            v-model:delete-image-ids="incomeForm.delete_image_ids"
          />
          <FormErrorMessages :messages="(incomeForm.errors as any).image_hashes" />
          <FormErrorMessages :messages="(incomeForm.errors as any).delete_image_ids" />
        </div>
      </template>
