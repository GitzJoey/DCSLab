...
import {
  SalesReturnItemNestedUpdateRequest,
  SalesReturnRefundNestedUpdateRequest,
...
// #region Declarations
type SalesReturnItemFormItem = SalesReturnItemNestedUpdateRequest & {
...
type SalesReturnRefundFormItem = SalesReturnRefundNestedUpdateRequest;
...
const salesReturnForm = salesReturnService.useSalesReturnEditForm(route.params.ulid as string);

const isUserLocationSelected = computed(() => selectedUserLocationStore.isUserLocationSelected);
const selectedUserLocation = computed(() => selectedUserLocationStore.selectedUserLocation);

const salesReturnData = ref<SalesReturn | null>(null);
...
const appendDDL = (
  ...
  if (!item?.id) return;
  appendDropDownOption(target, { code: item.id, name: item.name ?? item.code ?? item.id });
};
...
const loadCustomerDDL = async (search = '') => {
  ...
  const result = await customerService.readAnyGet({
    ...
    search,
    status: undefined,
    include_id: (salesReturnForm.customer_id as string | null)
      ?? salesReturnData.value?.customer?.id
      ?? undefined,
    refresh: false,
    limit: 100,
  });
  ...
  appendDDL(customerDDL, salesReturnData.value?.customer);
};
...
const loadWarehouseDDL = async (search = '') => {
  ...
  const result = await warehouseService.readAnyGet({
    ...
    search,
    status: undefined,
    include_id: (salesReturnForm.warehouse_id as string | null)
      ?? salesReturnData.value?.warehouse?.id
      ?? undefined,
    refresh: false,
    limit: 100,
  });
  ...
  appendDDL(warehouseDDL, salesReturnData.value?.warehouse);
};
...
const loadSalesInvoiceDDL = async (search = '') => {
  ...
  const options: Array<SalesInvoiceOption> = [];

  if (result.success && result.data) {
    (result.data.data as Array<any>).forEach((item: any) => {
      options.push({
        code: item.id,
  ...
  const linkedInvoice = salesReturnData.value?.sales_invoice;
  if (linkedInvoice?.id && !options.some((option) => option.code === linkedInvoice.id)) {
    options.push({
      code: linkedInvoice.id,
      name: linkedInvoice.code,
      customer_id: linkedInvoice.customer?.id ?? null,
      sales_order_id: linkedInvoice.sales_order?.id ?? null,
    });
  }

  salesInvoiceDDL.value = options;
};
...
const loadVatProfileDDL = async (search = '') => {
  ...
  (salesReturnData.value?.items ?? []).forEach((item) => appendVatProfileOption({
...
const loadCashAccountDDL = async (search = '') => {
  ...
  (salesReturnData.value?.refunds ?? []).forEach((refund) => appendDDL(cashAccountDDL, refund.cash_account));
};
// #endregion
...
const loadData = async () => {
  ...
  const result = await salesReturnService.read(ulid);
  if (!result.success || !result.data) return;

  const data = result.data;
  salesReturnData.value = data;

  const items: SalesReturnItemFormItem[] = (data.items ?? []).map((item: any) => ({
    id: item.id ?? null,
    sales_order_delivery_item_id: item.sales_order_delivery_item?.id ?? null,
    qty: item.qty,
    ...
    product_unit_product_name: item.product_unit?.product?.name ?? item.product?.name ?? '',
    ...
    is_use_serial_number: Boolean(item.product_unit?.product?.is_use_serial_number ?? item.product?.is_use_serial_number),
    sales_order_delivery_item_label: item.sales_order_delivery_item?.id
      ? `${item.sales_order_delivery_item.sales_order_delivery?.code ?? '-'} - ${item.product_unit?.code
        ? `[${item.product_unit.code}] ` : ''}${item.product_unit?.product?.name ?? '-'}`
      : null,
    sales_order_delivery_item_qty_base: item.sales_order_delivery_item?.product_unit_qty_base
      ? Number(item.sales_order_delivery_item.product_unit_qty_base)
      : null,
    base_unit_cogs: Number(item.base_unit_cogs ?? 0),
    total_cogs: Number(item.total_cogs ?? 0),
  }));

  const refunds: SalesReturnRefundFormItem[] = (data.refunds ?? []).map((refund: any) => ({
    id: refund.id ?? null,
    code: refund.code,
    date: formatDate(refund.date, 'YYYY-MM-DD HH:mm:ss'),
    cash_account_id: refund.cash_account?.id ?? '',
  ...
  salesReturnForm.setData({
    ...
    sales_invoice_id: data.sales_invoice?.id ?? null,
  ...
  salesReturnItemDetailsExpanded.value = items.map(() => false);
};
// #endregion
...
const removeProductUnit = (index: number) => {
  const item = salesReturnItemsForm.value[index];
  if (item?.id) {
    salesReturnForm.delete_item_ids.push(item.id);
  }
...
const getAmountAllocatedToInvoicePreview = () =>
  Number(salesReturnData.value?.amount_allocated_to_invoice ?? 0);
...
const removeRefund = (index: number) => {
  ...
  if (refund?.id) {
    salesReturnForm.delete_refund_ids.push(refund.id);
  }
...
const onSubmit = async () => {
  ...
  const cleanedItems: SalesReturnItemNestedUpdateRequest[] = salesReturnItemsForm.value.map((item) => ({
    id: item.id,
    ...
    serials: (item.serials ?? []).map((serial) => ({ id: serial.id, serial: serial.serial })),
  }));

  const cleanedRefunds: SalesReturnRefundNestedUpdateRequest[] = salesReturnRefundsForm.value.map((refund) => ({
    id: refund.id,
    code: refund.code,
    date: refund.date,
  ...
  try {
    ...
    showNotification(t('views.sales_return.alert.update.title'), t('views.sales_return.alert.update.message'));
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <div v-if="salesReturnItemsForm.length === 0" class="text-slate-500 text-sm">
        ...
        <div class="p-5 space-y-4">
          <div class="rounded-md border border-slate-200/60 dark:border-darkmode-400 p-4 space-y-4">
            ...
            <div class="grid grid-cols-12 gap-4 gap-y-3 items-end">
              <div class="col-span-12 lg:col-span-9"></div>
              <div class="col-span-12 lg:col-span-3">
                <FormLabel>{{ t('views.sales_return.fields.amount_allocated_to_invoice') }}</FormLabel>
                <FormInputCurrency :model-value="getAmountAllocatedToInvoicePreview()" readonly />
              </div>
            </div>
