        ...
        <div class="block-content">
            <transition name="fade">
                <div id="list" v-if="this.mode === 'list'">
                    <table class="table table-vcenter">
                        ...
                        <tbody>
                            <tr v-for="(c, cIdx) in houseList.data">
                                <td>{{ c.code }}</td>
                                <td>{{ c.name }}</td>
                                <td>
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-sm justify-content-end">
                            <li :class="{'page-item':true, 'disabled': this.houseList.prev_page_url == null}">
                                <a class="page-link" href="#" aria-label="Previous" v-on:click="onPaginationChangePage('prev')">
                                    <span aria-hidden="true">&laquo;</span>
                                    <span class="sr-only">Previous</span>
                                </a>
                            </li>
                            <li :class="{'page-item':true, 'disabled': this.houseList.next_page_url == null}">
            ...
            <transition name="fade">
                <div id="crud" v-if="this.mode !== 'list'">
                    <Form id="houseForm" @submit="onSubmit" :validation-schema="schema" v-slot="{ handleReset, errors }">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <Field id="inputCode" name="code" as="input" :class="{'form-control':true, 'is-invalid': errors['code']}" :placeholder="$t('fields.code')" :label="$t('fields.code')" v-model="house.code" v-show="this.mode === 'create' || this.mode === 'edit'"/>
                                <ErrorMessage name="code" class="invalid-feedback" />
                                <div class="form-control-plaintext" v-show="this.mode === 'show'">{{ house.code }}</div>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="inputName" class="col-2 col-form-label">{{ $t('fields.name') }}</label>
                            <div class="col-md-10">
                                <Field id="inputName" name="name" as="input" :class="{'form-control':true, 'is-invalid': errors['name']}" :placeholder="$t('fields.name')" :label="$t('fields.name')" v-model="house.name" v-show="this.mode === 'create' || this.mode === 'edit'"/>
                                <ErrorMessage name="name" class="invalid-feedback" />
                                <div class="form-control-plaintext" v-show="this.mode === 'show'">{{ house.name }}</div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-10 d-flex align-items-center">
                                <div>
                                    <select class="form-control" id="inputStatus" name="status" v-model="house.status" v-if="this.mode === 'create' || this.mode === 'edit'">
                                        <option value="1">{{ $t('statusDDL.active') }}</option>
                                        <option value="0">{{ $t('statusDDL.inactive') }}</option>
                                    </select>
                                    <div class="form-control-plaintext" v-show="this.mode === 'show'">
                                        <span v-if="house.status === 1">{{ $t('statusDDL.active') }}</span>
                                        <span v-if="house.status === 0">{{ $t('statusDDL.inactive') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        getAllHouse(page) {
            this.loading = true;
            axios.get('/api/get/admin/house/read?page=' + page).then(response => {
        ...
        onPaginationChangePage(page) {
            if (page === 'next') {
                this.getAllHouse(this.houseList.current_page + 1);
            } else if (page === 'prev') {
                this.getAllHouse(this.houseList.current_page - 1);
            } else {
                this.getAllHouse(page);
            }
        },
        emptyHouse() {
            return {
        ...
        createNew() {
            this.mode = 'create';
            this.house = this.emptyHouse();
        },
        editSelected(idx) {
            this.mode = 'edit';
            this.house = this.houseList.data[idx];
        },
        showSelected(idx) {
            this.mode = 'show';
            this.house = this.houseList.data[idx];
        ...
        onSubmit(values, actions) {
            this.loading = true;
            if (this.mode === 'create') {
                axios.post('/api/post/admin/house/save', new FormData($('#houseForm')[0])).then(response => {
                    this.backToList();
                }).catch(e => {
                ...
                axios.post('/api/post/admin/house/edit/' + this.house.hId, new FormData($('#houseForm')[0])).then(response => {
                    this.backToList();
                }).catch(e => {
                    this.handleError(e, actions);
                    this.loading = false;
        ...
        handleUpload(e) {
            ...
            fileReader.addEventListener('load', () => {
                this.house.profile.img_path = fileReader.result
            })
            fileReader.readAsDataURL(files[0])
        },
        backToList() {
            this.mode = 'list';
            this.getAllHouse(this.houseList.current_page);
            this.house = this.emptyHouse();
        },
        toggleFullScreen() {
    ...
    computed: {
        getPages() {
            if (this.houseList.current_page == null) return 0;

            return Math.ceil(this.houseList.total / this.houseList.per_page);
        },
        retrieveImage()
        {
            if (this.house.profile.img_path && this.house.profile.img_path !== '') {
                if (this.house.profile.img_path.includes('data:image')) {
                    return this.house.profile.img_path;
                } else {
                    return '/storage/' + this.house.profile.img_path;
                }
            } else {
                return '/images/def-house.png';
            }
        }
    }
}
