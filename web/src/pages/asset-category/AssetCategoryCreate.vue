...
const assetCategoryForm = assetCategoryService.useAssetCategoryCreateForm();
// #endregion
...
watch(
  ...
  debounce((newValue): void => {
    cacheService.setLastEntity('ASSET_CATEGORY_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
// #endregion
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('ASSET_CATEGORY_CREATE') as Record<string, unknown> | null;
  if (!data) return;
  ...
  if (
    data.estimated_useful_life_months === undefined ||
    data.estimated_useful_life_months === null
  ) {
    data.estimated_useful_life_months = '';
  }
  ...
  assetCategoryForm.setData(data);
};
