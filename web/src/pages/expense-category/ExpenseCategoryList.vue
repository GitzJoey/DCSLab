...
const expenseCategoryLists = ref<Resource<Array<ExpenseCategory>> | null>({
  data: [],
});
// #endregion
...
onMounted(async () => {
  ...
  await getExpenseCategories('', true);
});
// #endregion

// #region Methods
const getExpenseCategories = async (search: string, refresh: boolean) => {
  emits('loading-state', true);

  const result: ServiceResponse<Resource<Array<ExpenseCategory>> | null> = await expenseCategoryService.readAnyGet({
    with_trashed: false,
  ...
  if (result.success && result.data) {
    expenseCategoryLists.value = result.data;
...
const onDataListChanged = async () => {
  await getExpenseCategories('', false);
};
...
const editSelected = (item: ExpenseCategory) => {
  router.push({
...
const deleteSelected = (item: ExpenseCategory) => {
...
const confirmDelete = async () => {
  ...
  const result: ServiceResponse<boolean | null> = await expenseCategoryService.delete(deleteUlid.value);

  if (result.success) {
    emits('update-profile');
    await getExpenseCategories('', true);
    showNotification(
      t('views.expense_category.alert.delete_expense_category.title'),
      t('views.expense_category.alert.delete_expense_category.content'),
    );
  } else {
...
<template>
  <DataList
    :title="t('views.expense_category.table.title')"
    :enable-search="false"
    :can-print="true"
    :can-export="true"
    :pagination="null"
  ...
  >
    ...
    <template #content>
      <Table class="mt-5" :hover="true">
        ...
        <Table.Tbody v-if="expenseCategoryLists !== null">
          <template v-if="expenseCategoryLists.data.length == 0">
          ...
          <TreeList v-else ref="treeListRef" :items="expenseCategoryLists.data">
            ...
            <Table.Tr class="intro-x">
              ...
              <Table.Td>
                <div class="flex justify-end gap-1">
                  ...
                  <Button variant="outline-secondary" @click="editSelected(item as ExpenseCategory)">
                    <Lucide icon="Pen" class="w-4 h-4" />
                  </Button>
                  <Button variant="outline-secondary" @click="deleteSelected(item as ExpenseCategory)">
            ...
            >
              <Table.Td colspan="3">
                ...
                <div class="mt-1 flex flex-row">
                  <div class="ml-5 w-48 pr-5 text-right font-medium">
                    {{ t('views.expense_category.fields.child_count') }}
                  </div>
