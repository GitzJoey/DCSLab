...
const debtForm = debtService.useDebtCreateForm();
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('DEBT_CREATE') as Record<string, unknown> | null;
  if (!data) return;

  debtForm.setData(data);
};
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await debtCategoryService.readAnyGet({
    ...
    company_id: selectedUserLocation.value.company.id,
    search,
    include_id: debtForm.category_id || undefined,
    refresh: false,
    limit: 20,
  });
...
const loadCreditorDDL = async (search = '') => {
  ...
  const result = await debtCreditorService.readAnyGet({
    ...
    company_id: selectedUserLocation.value.company.id,
    search,
    include_id: debtForm.creditor_id || undefined,
    refresh: false,
    limit: 20,
  });
...
const loadSupplierDDL = async (search = '') => {
  ...
  const result = await supplierService.readAnyGet({
    ...
    search,
    status: undefined,
    include_id: debtForm.supplier_id || undefined,
    refresh: false,
    limit: 20,
  });
...
const loadCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: debtForm.cash_account_id || undefined,
...
watch(
  ...
  debounce((newValue): void => {
    cacheService.setLastEntity('DEBT_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
