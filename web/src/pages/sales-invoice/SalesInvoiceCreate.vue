...
import {
  SalesInvoiceItemNestedStoreRequest,
  SalesInvoicePaymentNestedStoreRequest,
...
// #region Declarations
type SalesInvoiceItemFormItem = SalesInvoiceItemNestedStoreRequest & {
...
type SalesInvoicePaymentFormItem = SalesInvoicePaymentNestedStoreRequest;
...
const salesInvoiceForm = salesInvoiceService.useSalesInvoiceCreateForm();
...
watch(
  salesInvoiceForm,
  debounce((newValue) => {
    cacheService.setLastEntity('SALES_INVOICE_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
...
onMounted(async () => {
  ...
  await restoreSalesOrderFromCache();
});
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('SALES_INVOICE_CREATE') as Record<string, unknown>;
  if (!data) return;
  salesInvoiceForm.setData(data);
  salesInvoiceItemDetailsExpanded.value = salesInvoiceItemsForm.value.map(() => false);
};

const restoreSalesOrderFromCache = async () => {
  const salesOrderId = salesInvoiceForm.sales_order_id;
  if (!salesOrderId) return;

  const option = salesOrderDDL.value.find((item) => item.code === salesOrderId);
  if (!option) return;

  const result = await salesOrderService.read(option.ulid);
...
const loadSalesReturnDDL = async (search = '') => {
  ...
  if (result.success && result.data) {
    ...
    salesReturnDDL.value = (result.data.data as Array<any>)
...
const onSubmit = async () => {
  ...
  const cleanedItems: SalesInvoiceItemNestedStoreRequest[] = salesInvoiceItemsForm.value.map((item) => ({
    qty: item.qty,
  ...
  const cleanedPayments: SalesInvoicePaymentNestedStoreRequest[] = salesInvoicePaymentsForm.value.map((payment) => ({
    code: payment.code,
    date: payment.date,
  ...
  try {
    ...
    cacheService.removeLastEntity('SALES_INVOICE_CREATE');
    showAlertPlaceholder('hidden', '', null);
    emits('update-profile');
    showNotification(t('views.sales_invoice.alert.create.title'), t('views.sales_invoice.alert.create.message'));
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-1>
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            ...
            <!-- sales order (required, drives customer + items) -->
            ...
            <!-- customer (inherited from the sales order) -->
            <div class="col-span-12 md:col-span-6 lg:col-span-6">
              <FormLabel :class="{ 'text-danger': salesInvoiceForm.invalid('customer_id') }">
                {{ t('views.sales_invoice.fields.customer_id') }}
              </FormLabel>
              <FormInput :model-value="selectedSalesOrderData?.customer?.name ?? '-'" readonly />
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <div v-if="!salesInvoiceForm.sales_order_id && salesInvoiceItemsForm.length === 0"
            ...
            {{ t('views.sales_invoice.fields.sales_order_required_hint') }}
          </div>
          <div v-else-if="salesInvoiceItemsForm.length === 0" class="text-slate-500 text-sm">
          ...
          <div v-else>
            <div v-for="(item, index) in salesInvoiceItemsForm" :key="`${item.product_unit_id}-${index}`"
              ...
              <!-- item content: responsive summary row -->
              <div class="grid grid-cols-12 gap-4 gap-y-3">
                <!-- item image -->
                ...
                <!-- item subtotal after discount + expand/remove -->
      ...
      <template #card-items-3>
        <div class="p-5 space-y-4">
          <div class="rounded-md border border-slate-200/60 dark:border-darkmode-400 p-4 space-y-4">
            ...
            <!-- summary: header global discount -->
            <div class="grid grid-cols-12 gap-4 gap-y-3 items-end">
              <div class="col-span-12 lg:col-span-9"></div>
              <div class="col-span-12 lg:col-span-3">
            ...
            <!-- summary: payments editor and aggregates -->
            <div class="space-y-4">
