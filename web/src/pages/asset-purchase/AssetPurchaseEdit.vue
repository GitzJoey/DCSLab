...
import type { AssetPurchaseItemNestedUpdateRequest } from '@/types/services/asset-purchase/AssetPurchaseRequest';
import { convertErrorTypeToAlertListType, formatDate } from '@/utils/helper';
import type { AlertPlaceholderProps } from '@/components/AlertPlaceholder/AlertPlaceholder.vue';

type AssetPurchaseItemFormItem = {
  id: string | null;
  qty: AssetPurchaseItemNestedUpdateRequest['qty'];
  asset_id: AssetPurchaseItemNestedUpdateRequest['asset_id'];
  asset_code?: string | null;
  asset_name?: string | null;
  asset_category_name?: string | null;
  asset_unit_name?: string | null;
  unit_price: AssetPurchaseItemNestedUpdateRequest['unit_price'];
  remarks: AssetPurchaseItemNestedUpdateRequest['remarks'];
...
const assetPurchaseForm = assetPurchaseService.useAssetPurchaseEditForm(route.params.ulid as string);
...
watch(
  ...
  debounce((newValue): void => {
    cacheService.setLastEntity('ASSET_PURCHASE_EDIT', newValue.data());
  }, 500),
  { deep: true },
);
...
const loadData = async () => {
  ...
  const result = await assetPurchaseService.read(ulid);
  if (!result.success || !result.data) return;

  const data = result.data as AssetPurchase;
  const items: AssetPurchaseItemFormItem[] = (data.items || []).map((item) => ({
    id: item.id ?? null,
    qty: item.qty,
...
const handleAssetModalAfterLeave = () => {
  ...
  nextTick(() => {
    const el = document.getElementById(`asset-purchase-edit-qty-${index}`) as HTMLInputElement | null;
...
const removeAsset = (index: number) => {
  const item = itemsForm.value[index];
  if (item?.id) {
    assetPurchaseForm.delete_item_ids.push(item.id);
    assetPurchaseForm.delete_item_ids = Array.from(new Set(assetPurchaseForm.delete_item_ids));
  }
...
<template>
  <form id="assetPurchaseEditForm" @submit.prevent="onSubmit">
            ...
            >
              ...
              <div class="grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 lg:col-span-2">
                  ...
                  <FormInputCurrency
                    :id="`asset-purchase-edit-qty-${index}`"
                    v-model="item.qty"
                ...
                <div class="col-span-12">
                  ...
                  <FormErrorMessages
                    ...
                    :key="`asset-purchase-edit-serial-error-${index}-${serialIndex}`"
