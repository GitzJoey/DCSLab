...
const titleView = ref<string>('views.receivable.page_title');
...
const receivableNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[receivableNotification]', (el: NotificationElement) => {
  receivableNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-receivable-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.receivable.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.receivable.actions.edit';
      break;
    default:
      titleView.value = 'views.receivable.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  receivableNotification.value?.showToast();
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      ...
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('RECEIVABLE_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="receivableNotification" :options="{ duration: 3000 }" class="flex">
