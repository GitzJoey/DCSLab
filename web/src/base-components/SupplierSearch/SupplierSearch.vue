...
<template>
  <div class="grid grid-cols-12">
    <select class="form-select col-span-11 py-3 px-4 w-full mt-3 lg:mt-0 ml-auto border-none focus:border-none rounded"
      @change="handleChangeOptions($event)" placeholder="Select Supplier"
      :value="selectedSupplier ? selectedSupplier : false">
      <option :value="false" selected>Select Supplier</option>
      <option v-for="(supplier, index) in supplierList" :key="index" :value="supplier?.ulid"
  ...
  <div class="box mt-3" v-if="(selectedSupplier ? true : false) || isAddNewValue">
    <div class="p-4">
      <FormLabel html-for="name"> Code </FormLabel>
      <FormInput v-model="detailSupplier.code" :disabled="selectedSupplier ? true : false" name="code" type="text"
        :value="detailSupplier.code" class="w-full" />
    </div>

    <div class="p-4">
      <FormLabel html-for="name"> Name </FormLabel>
      <FormInput :disabled="selectedSupplier ? true : false" name="name" v-model="detailSupplier.name"
        :value="detailSupplier.name" type="text" class="w-full" />
    </div>
