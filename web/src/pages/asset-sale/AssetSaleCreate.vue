...
import type { AssetSaleItemNestedStoreRequest } from '@/types/services/asset-sale/AssetSaleRequest';
import { convertErrorTypeToAlertListType } from '@/utils/helper';
import type { AlertPlaceholderProps } from '@/components/AlertPlaceholder/AlertPlaceholder.vue';

type AssetSaleItemFormItem = {
  qty: AssetSaleItemNestedStoreRequest['qty'];
  asset_id: AssetSaleItemNestedStoreRequest['asset_id'];
  asset_code?: string | null;
  asset_name?: string | null;
  asset_category_name?: string | null;
  asset_unit_name?: string | null;
  unit_price: AssetSaleItemNestedStoreRequest['unit_price'];
  remarks: AssetSaleItemNestedStoreRequest['remarks'];
...
const assetSaleForm = assetSaleService.useAssetSaleCreateForm();
...
watch(
  ...
  debounce((newValue): void => {
    cacheService.setLastEntity('ASSET_SALE_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('ASSET_SALE_CREATE') as Record<string, unknown>;
  if (!data) return;
  assetSaleForm.setData(data);
};
...
const selectAsset = (option: AssetOption) => {
  ...
  if (editingAssetIndex.value === null) {
    ...
    assetSaleForm.items[index] = { ...current, ...baseData };
    targetIndex = index;
  }
...
const handleAssetModalAfterLeave = () => {
  ...
  nextTick(() => {
    const el = document.getElementById(`asset-sale-qty-${index}`) as HTMLInputElement | null;
...
<template>
  <form id="assetSaleForm" @submit.prevent="onSubmit">
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            <div class="col-span-12 lg:col-span-4 md:col-span-6">
              ...
              <FormInputCode
                v-model="assetSaleForm.code"
                :class="{ 'border-danger': assetSaleForm.invalid('code') }"
                :placeholder="t('views.asset_sale.fields.code')"
                @set-auto="setCode"
                @change="assetSaleForm.validate('code')"
              />
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-6">
              ...
              <FormInputDateTimeAuto
                v-model="assetSaleForm.date"
                :class="{ 'border-danger': assetSaleForm.invalid('date') }"
                :placeholder="t('views.asset_sale.fields.date')"
                @change="assetSaleForm.validate('date')"
              />
            ...
            <div class="col-span-12 lg:col-span-6">
              ...
              <FormTextarea
                v-model="assetSaleForm.remarks"
                rows="2"
                :class="{ 'border-danger': assetSaleForm.invalid('remarks') }"
                :placeholder="t('views.asset_sale.fields.remarks')"
                @change="assetSaleForm.validate('remarks')"
              />
            ...
            <div v-for="(item, index) in itemsForm" :key="index" class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-4">
              ...
              <div class="grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 lg:col-span-2">
                  ...
                  <FormInputCurrency
                    :id="`asset-sale-qty-${index}`"
                    v-model="item.qty"
                    :class="['text-right', { 'border-danger': assetSaleForm.invalid(`items.${index}.qty` as any) }]"
                    @change="assetSaleForm.validate(`items.${index}.qty` as any); assetSaleForm.validate(`items.${index}.serials` as any);"
                  />
                ...
                <div class="col-span-12 lg:col-span-3">
                  ...
                  <FormInputCurrency
                    ...
                    :class="['text-right', { 'border-danger': assetSaleForm.invalid(`items.${index}.unit_price` as any) }]"
                    @change="assetSaleForm.validate(`items.${index}.unit_price` as any)"
                  />
                ...
                <div class="col-span-12">
                  ...
                  <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
                    <div v-for="(serial, serialIndex) in item.serials" :key="serialIndex" class="flex gap-2">
                      <FormInput
                        ...
                        :class="{ 'border-danger': assetSaleForm.invalid(`items.${index}.serials.${serialIndex}.serial` as any) }"
                        @change="assetSaleForm.validate(`items.${index}.serials.${serialIndex}.serial` as any); assetSaleForm.validate(`items.${index}.serials` as any);"
                      />
                  ...
                  <FormErrorMessages
                    ...
                    :key="`asset-sale-serial-error-${index}-${serialIndex}`"
                    :messages="(assetSaleForm.errors as any)[`items.${index}.serials.${serialIndex}.serial`]"
                  />
                </div>
            ...
            <table class="min-w-full text-sm">
              ...
              <tbody>
                <tr v-if="assetOptions.length === 0">
                  <td colspan="5" class="px-3 py-4 text-center text-slate-500">{{ t('components.data-list.data_not_found') }}</td>
                </tr>
                <tr v-for="(option, optionIndex) in assetOptions" :key="optionIndex" class="border-t border-slate-200/60 dark:border-darkmode-400">
                  ...
                  <td class="px-3 py-2 text-right">
                    <Button type="button" variant="primary" size="sm" @click="selectAsset(option)">{{ t('components.buttons.select') }}</Button>
                  </td>
                </tr>
              </tbody>
            </table>
