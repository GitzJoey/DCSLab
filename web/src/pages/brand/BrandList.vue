...
const getBrands = async (search: string, refresh: boolean, paginate: boolean, page: number, per_page: number) => {
  ...
  let result: ServiceResponse<Collection<Array<Brand>> | Resource<Array<Brand>> | null> = await brandServices.readAny(searchReq);

  if (result.success && result.data) {
    brandLists.value = result.data as Collection<Array<Brand>>;
  } else {
...
const onDataListChanged = async (data: DataListEmittedData) => {
  await getBrands(data.search.text, false, true, data.pagination.page, data.pagination.per_page);
}
