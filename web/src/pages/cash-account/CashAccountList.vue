...
const getCashAccounts = async (
  ...
  let result: ServiceResponse<Collection<Array<CashAccount>> | null> = await cashAccountServices.readAnyPaginate(searchReq);
...
const confirmDelete = async () => {
  ...
  if (result.success) {
    ...
    await getCashAccounts("", true, 1, 10);
    showNotification(t("views.cash_account.alert.delete_cash_account.title"), t("views.cash_account.alert.delete_cash_account.content"));
  } else {
    showAlertPlaceholder("danger", "", result.errors as Record<string, Array<string>>);
  }
            ...
            >
              <Table.Td colspan="5">
                ...
                <div class="flex flex-row">
                  ...
                  <div class="flex-1">
                    <span v-if="item.is_bank">{{ t("components.dropdown.values.switch.on") }}</span>
