...
const emit = defineEmits<{
  ...
  (e: 'update:existingImages', value: ExpenseImage[]): void;
...
const existingImages = computed({
  ...
  set: (value: ExpenseImage[]) => emit('update:existingImages', value),
});
...
const newImages = ref<ExpenseImage[]>([]);
...
const isImageMain = (image: ExpenseImage): boolean => {
...
const handleUpload = async (file: File) => {
  ...
  try {
    const uploadResponse: ServiceResponse<ExpenseImage | null> = await expenseImageService.upload(file);
...
const setMainImage = (image: ExpenseImage) => {
...
const capturePhoto = () => {
  ...
  canvas.toBlob((blob) => {
    ...
    const file = new File([blob], `expense_${Date.now()}.jpg`, { type: 'image/jpeg' });
    handleUpload(file);
    stopCamera();
  }, 'image/jpeg');
};
