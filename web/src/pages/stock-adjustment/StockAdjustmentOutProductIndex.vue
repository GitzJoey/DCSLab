<script setup lang="ts">
  ...
  import StockAdjustmentOutProductList from './StockAdjustmentOutProductList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_out_product.page_title');
  ...
  const stockAdjustmentOutProductNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentOutProductNotification]', (el: NotificationElement) => {
    stockAdjustmentOutProductNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_out_product.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentOutProductNotification.value) {
      stockAdjustmentOutProductNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="stockAdjustmentOutProductNotification" :options="{ duration: 3000 }" class="flex">
