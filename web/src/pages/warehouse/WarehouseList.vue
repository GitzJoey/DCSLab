...
const getWarehouses = async (
  ...
  let result: ServiceResponse<Collection<Array<Warehouse>> | null> =
    await warehouseServices.readAnyPaginate(searchReq);
...
const confirmDelete = async () => {
  ...
  let result: ServiceResponse<boolean | null> = await warehouseServices.delete(
    deleteUlid.value,
  );
  ...
  if (result.success) {
    ...
    await getWarehouses("", true, 1, 10);
    showNotification(
      t("views.warehouse.alert.delete_warehouse.title"),
      t("views.warehouse.alert.delete_warehouse.content"),
    );
  } else {
    showAlertPlaceholder(
      "danger",
...
<template>
  <DataList
    :title="t('views.warehouse.table.title')"
    :enable-search="true"
    :can-print="true"
    :can-export="true"
    :pagination="warehouseLists ? warehouseLists.meta : null"
    ...
    <template #content>
      <Table class="mt-5" :hover="true">
        ...
        <Table.Tbody v-if="warehouseLists !== null">
          ...
          <template
            v-for="(item, itemIdx) in warehouseLists.data"
            :key="item.ulid"
          >
            <Table.Tr class="intro-x">
