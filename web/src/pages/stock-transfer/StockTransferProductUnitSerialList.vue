...
import StockTransferProductUnitSerialService from '@/services/StockTransferProductUnitSerialService';
...
import { StockTransferProductUnitSerialReadAnyPaginateRequest } from '@/types/services/stock-transfer-product-unit-serial/StockTransferProductUnitSerialRequest';
...
const stockTransferProductUnitSerialService = new StockTransferProductUnitSerialService();
...
const stockTransferProductUnitSerialLists = ref<Collection<Array<StockTransferProductUnitSerial>> | null>({
    data: [],
    meta: {
        current_page: 1,
        from: null,
...
onMounted(async () => {
    ...
    await getStockTransferProductUnitSerials('', true, 1, 10);
});

const getStockTransferProductUnitSerials = async (
    search: string,
    refresh: boolean,
    page: number,
    per_page: number,
    ...
    const request: StockTransferProductUnitSerialReadAnyPaginateRequest = {
        with_trashed: false,
    ...
    const result: ServiceResponse<Collection<Array<StockTransferProductUnitSerial>> | null> =
        await stockTransferProductUnitSerialService.readAnyPaginate(request);

    if (result.success && result.data) {
        stockTransferProductUnitSerialLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
    await getStockTransferProductUnitSerials(data.search.text, true, data.pagination.page, data.pagination.per_page);
};

const handleDateFilterChange = async () => {
    const perPage = stockTransferProductUnitSerialLists.value?.meta.per_page || 10;
    await getStockTransferProductUnitSerials(searchText.value, true, 1, perPage);
};
...
const editSelected = (idx: number) => {
    if (!stockTransferProductUnitSerialLists.value) return;

    const ulid = stockTransferProductUnitSerialLists.value.data[idx].stock_transfer?.ulid;
    if (!ulid) return;
...
const formatStockTransferDate = (item: StockTransferProductUnitSerial) => {
...
const getProductMainImageUrl = (item: StockTransferProductUnitSerial): string | null => {
    const images = item.stock_transfer_product_unit?.product_unit?.product?.product_images ?? [];
...
<template>
    <div>
        <LoadingOverlay :visible="loading">
            <TitleLayout>
                <template #title>
                    {{ t('views.stock_transfer_product_unit_serial.page_title') }}
                </template>
            </TitleLayout>
            ...
            <div class="grid grid-cols-12 gap-6 mt-5">
                <div class="col-span-12 intro-y lg:col-span-12">
                    ...
                    <DataListFlex :data="stockTransferProductUnitSerialLists" :enable-search="true"
                            :can-print="true" :can-export="true" :rows="stockTransferProductUnitSerialLists?.data ?? []"
                            row-class="bg-white dark:bg-darkmode-600"
                            :pagination="stockTransferProductUnitSerialLists ? stockTransferProductUnitSerialLists.meta : null"
                            ...
                            <template #row="{ item, index }">
                                <div
                                    ...
                                    <ProductImagePreview
                                        :image-url="getProductMainImageUrl(item as StockTransferProductUnitSerial)"
                                ...
                                <div class="col-span-12 lg:col-span-4 md:col-span-5 self-start">
                                    ...
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        ...
                                        {{ (item as StockTransferProductUnitSerial).stock_transfer?.code ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.stock_transfer.fields.date') }}:
                                        {{ formatStockTransferDate(item as StockTransferProductUnitSerial) }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.stock_transfer.fields.source_warehouse_id') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer?.source_warehouse?.name ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.stock_transfer.fields.destination_warehouse_id') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer?.destination_warehouse?.name ??
                                        '-' }}
                                    </div>
                                    <div v-if="(item as StockTransferProductUnitSerial).stock_transfer?.remarks?.trim()"
                                        class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.stock_transfer.fields.remarks') }}:
                                        {{ (item as StockTransferProductUnitSerial).stock_transfer?.remarks }}
                                    </div>
                                </div>
                                ...
                                <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
                                    ...
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        ...
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.product?.code
                                        ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.product.fields.name') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.product?.name
                                        ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.product.fields.category_id') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.product?.category?.name
                                        ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.product.fields.brand_id') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.product?.brand?.name
                                        ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.product.fields.unit_code') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.code
                                        ?? '-' }}
                                    </div>
                                    <div class="text-slate-500 text-xs whitespace-nowrap">
                                        {{ t('views.stock_transfer_product_unit.fields.product_unit_id') }}:
                                        {{ (item as
                                            StockTransferProductUnitSerial).stock_transfer_product_unit?.product_unit?.unit?.name
                                        ?? '-' }}
                                    </div>
                                </div>
                                ...
                                <div class="col-span-12 lg:col-span-2 md:col-span-2 self-start">
                                    ...
                                    <div class="text-slate-500 text-xs break-all">
                                        {{ (item as StockTransferProductUnitSerial).serial || '-' }}
                                    </div>
                                </div>
