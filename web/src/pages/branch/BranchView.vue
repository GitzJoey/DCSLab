...
import { BranchRequest } from "../../types/requests/BranchRequests";
...
//#region Data - Views
const branch = ref<BranchRequest>({
  id: '',
  ulid: '',
  company: { 
      id: '',
...
const getBranches = async (search: string, refresh: boolean, paginate: boolean, page: number, per_page: number) => {
  let result: ServiceResponse<Collection<Branch[]> | Resource<Branch[]> | null> = await branchServices.readAny(
    search,
    refresh,
    paginate,
    page,
            ...
            <Table class="mt-5" :hover="true">
              <Table.Thead variant="light">
                <Table.Tr>
                  ...
                  <Table.Th class="whitespace-nowrap">{{
                    t("views.branch.table.cols.remarks")
                  }}</Table.Th>
                  <Table.Th class="whitespace-nowrap">{{
              ...
              <Table.Tbody v-if="branchLists !== null">
                ...
                <template v-for="( item, itemIdx ) in branchLists.data" :key="item.ulid">
                  <Table.Tr class="intro-x">
                    ...
                    <Table.Td>{{ item.is_main }}</Table.Td>
                  ...
                  <Table.Tr :class="{ 'intro-x': true, 'hidden transition-all': expandDetail !== itemIdx }">
                    <Table.Td colspan="5">
                      ...
                      <div class="flex flex-row">
                        ...
                        <div class="flex-1">{{ item.is_main }}</div>
                      </div>
                      <div class="flex flex-row">
