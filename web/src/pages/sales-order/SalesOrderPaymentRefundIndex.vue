...
import SalesOrderPaymentRefundList from './SalesOrderPaymentRefundList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.sales_order_payment_refund.page_title');
...
const salesOrderPaymentRefundNotification = ref<NotificationElement>();

provide('bind[salesOrderPaymentRefundNotification]', (el: NotificationElement) => {
  salesOrderPaymentRefundNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.sales_order_payment_refund.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (salesOrderPaymentRefundNotification.value) {
    salesOrderPaymentRefundNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesOrderPaymentRefundNotification" :options="{ duration: 3000 }" class="flex">
