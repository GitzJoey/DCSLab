<?php

namespace Database\Factories;

use Illuminate\Database\Eloquent\Factories\Factory;

class PurchaseInvoiceItemFactory extends Factory
{
    public function definition(): array
    {
        $qty = fake()->randomFloat(8, 1, 100);
        $conversionValue = fake()->randomFloat(8, 1, 100);
        $price = fake()->randomFloat(8, 1000, 1000000);

        return [
            'qty' => $qty,
            'product_unit_conversion_value' => $conversionValue,
            'product_unit_qty_base' => bcmul((string) $qty, (string) $conversionValue, 8),
            'product_unit_price' => $price,
            'product_unit_is_price_include_vat' => false,
            'price_discount' => 0,
            'price_after_discount' => $price,
            'subtotal' => bcmul((string) $qty, (string) $price, 8),
            'subtotal_discount' => 0,
            'subtotal_after_discount' => bcmul((string) $qty, (string) $price, 8),
            'vat_rate' => 0,
            'vat_base_numerator' => 1,
            'vat_base_denominator' => 1,
            'remarks' => fake()->sentence(),
        ];
    }
}
