...
class StockTransferItemUpdateRequest extends FormRequest
{
    public function authorize()
    {
        ...
        /** @var \App\User */
        $user = Auth::user();
        $resource = $this->route('stock_transfer_item');

        return $user->can('update', StockTransferItem::class, $resource) ? true : false;
    }
    ...
    public function rules()
    {
        return [
            ...
            'delete_serial_ids.*' => ['required', 'integer', 'distinct', new ExistsForCompany('stock_transfer_item_serials', $this->company_id)],
            'serials' => ['present', 'array'],
            'serials.*.id' => ['present', 'nullable', 'integer', new ExistsForCompany('stock_transfer_item_serials', $this->company_id)],
