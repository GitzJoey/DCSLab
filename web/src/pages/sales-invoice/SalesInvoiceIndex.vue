...
const titleView = ref<string>('views.sales_invoice.page_title');
...
const salesInvoiceNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[salesInvoiceNotification]', (el: NotificationElement) => {
  salesInvoiceNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-sales-invoice-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.sales_invoice.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.sales_invoice.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.sales_invoice.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  salesInvoiceNotification.value?.showToast();
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      ...
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('SALES_INVOICE_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesInvoiceNotification" :options="{ duration: 3000 }" class="flex">
