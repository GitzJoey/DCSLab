...
const titleView = ref<string>('views.purchase_additional_cost_category.page_title');
...
const purchaseAdditionalCostCategoryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[purchaseAdditionalCostCategoryNotification]', (el: NotificationElement) => {
  purchaseAdditionalCostCategoryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-purchase-additional-cost-category-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.purchase_additional_cost_category.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.purchase_additional_cost_category.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.purchase_additional_cost_category.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseAdditionalCostCategoryNotification.value) {
    purchaseAdditionalCostCategoryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_CATEGORY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_CATEGORY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseAdditionalCostCategoryNotification" :options="{ duration: 3000 }" class="flex">
