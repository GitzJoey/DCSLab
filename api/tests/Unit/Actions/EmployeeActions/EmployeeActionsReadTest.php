...
class EmployeeActionsReadTest extends ActionsTestCase
{
    ...
    {
        $employeeCount = 4;
        $idxTest = random_int(0, $employeeCount - 1);
        $defaultName = Employee::factory()->make()->name;
        $testname = Employee::factory()->insertStringInName('testing')->make()->name;

        $user = User::factory()
            ->has(Company::factory()->setStatusActive()->setIsDefault()
                ->has(Employee::factory()->count($employeeCount)
                    ->state(new Sequence(
