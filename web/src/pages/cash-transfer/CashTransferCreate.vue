...
const cashTransferForm = cashTransferService.useCashTransferCreateForm();
...
const loadFromCache = () => {
  const data = cacheServices.getLastEntity('CASH_TRANSFER_CREATE') as Record<string, unknown>;

  if (!data) return;

  cashTransferForm.setData(data);
};
...
const resetForm = () => {
  ...
  setLocationData();
  sourceCashAccountSearch.value = '';
  destinationCashAccountSearch.value = '';
};
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('CASH_TRANSFER_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
</script>
