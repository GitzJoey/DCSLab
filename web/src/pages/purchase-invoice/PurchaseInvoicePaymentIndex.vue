...
import PurchaseInvoicePaymentList from './PurchaseInvoicePaymentList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.purchase_invoice_payment.page_title');
...
const purchaseInvoicePaymentNotification = ref<NotificationElement>();

provide('bind[purchaseInvoicePaymentNotification]', (el: NotificationElement) => {
  purchaseInvoicePaymentNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.purchase_invoice_payment.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseInvoicePaymentNotification.value) {
    purchaseInvoicePaymentNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseInvoicePaymentNotification" :options="{ duration: 3000 }" class="flex">
