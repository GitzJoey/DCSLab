...
const getDDL = (): void => {
  roleServices
    .readAny()
    .then((result: ServiceResponse<Resource<Array<Role>> | null>) => {
      if (result.success && result.data) {
...
const createNew = () => {
  ...
  userForm.value =
    cachedData == null ? emptyUser() : (cachedData as UserFormRequest);
};
...
const onSubmit = async (
  ...
  actions: FormActions<UserFormFieldValues>
) => {
  loading.value = true;
  ...
  if (mode.value == ViewMode.FORM_CREATE) {
    ...
    if (!values.api_token) delete values.api_token;
    if (!values.reset_password) delete values.reset_password;
...
const onInvalidSubmit = (formResults: InvalidSubmissionContext) => {
  ...
  if (Object.keys(formResults.errors).length > 0) {
    const errorField = document.getElementById(
      Object.keys(formResults.errors)[0]
    );
...
const cards = computed((): Array<TwoColumnsLayoutCards> => {
  switch (mode.value) {
    case ViewMode.FORM_CREATE:
      return [
  ...
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <div v-if="mode == ViewMode.LIST">
        ...
        <DataList
          :title="t('views.user.table.title')"
          :enable-search="true"
          :can-print="true"
          :can-export="true"
          :pagination="userLists ? userLists.meta : null"
          ...
          <template #content>
            <Table class="mt-5" :hover="true">
              ...
              <Table.Tbody v-if="userLists !== null">
                ...
                <template
                  v-for="(item, itemIdx) in userLists.data"
                  :key="item.ulid"
                >
                  <Table.Tr class="intro-x">
                    ...
                    <Table.Td>
                      ...
                      />
                      <Lucide
                        v-if="item.profile.status === 'INACTIVE'"
                        icon="X"
                      />
                    </Table.Td>
                    <Table.Td>
                ...
                <div class="pb-4">
                  ...
                  >
                    <FormInput
                      ...
                      :placeholder="t('views.user.fields.email')"
            ...
            <template #card-items-1>
              <div class="p-5">
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.first_name"
                    name="first_name"
                  >
                ...
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.last_name"
                    name="last_name"
                  >
                ...
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.address"
                    name="address"
                  >
                    <FormLabel html-for="address" class="form-label">{{
                      t("views.user.fields.address")
                    }}</FormLabel>
                    <FormInput
                      id="address"
                ...
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.city"
                    name="city"
                  >
                ...
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.postal_code"
                    name="postal_code"
                  >
                ...
                <div class="pb-4">
                  <FormLabel
                    html-for="country"
                    :class="{ 'text-danger': errors['country'] }"
                  >
                    {{ t("views.user.fields.country") }}
                  </FormLabel>
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.country"
                    name="country"
                    rules="required"
                ...
                <div class="pb-4">
                  <FormLabel
                    html-for="img_path"
                    :class="{ 'text-danger': errors['img_path'] }"
                  >
                    {{ t("views.user.fields.picture") }}
                  </FormLabel>
                  <FormFileUpload
                    id="img_path"
                    v-model="userForm.data.profile.img_path"
                    name="img_path"
                    type="text"
                    :class="{ 'border-danger': errors['img_path'] }"
                ...
                <div class="pb-4">
                  <FormLabel
                    html-for="tax_id"
                    :class="{ 'text-danger': errors['tax_id'] }"
                  >
                    {{ t("views.user.fields.tax_id") }}
                  </FormLabel>
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.tax_id"
                    name="tax_id"
                    rules="required"
                    :placeholder="t('views.user.fields.tax_id')"
                ...
                <div class="pb-4">
                  <FormLabel
                    html-for="ic_num"
                    :class="{ 'text-danger': errors['ic_num'] }"
                  >
                    {{ t("views.user.fields.ic_num") }}
                  </FormLabel>
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.ic_num"
                    name="ic_num"
                    rules="required"
                    :placeholder="t('views.user.fields.ic_num')"
                ...
                <div class="pb-4">
                  <FormLabel
                    html-for="status"
                    :class="{ 'text-danger': errors['status'] }"
                  >
                    {{ t("views.user.fields.status") }}
                  </FormLabel>
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.status"
                    name="status"
                    rules="required"
                ...
                <div class="pb-4">
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.profile.remarks"
                    name="remarks"
                  >
                    <FormLabel
                      html-for="remarks"
                      :class="{ 'text-danger': errors['remarks'] }"
                    >
                      {{ t("views.user.fields.remarks") }}
                    </FormLabel>
                    <FormTextarea
                      ...
                      v-bind="field"
                      type="text"
                      :placeholder="t('views.user.fields.remarks')"
                      rows="3"
                    />
                  </VeeField>
                </div>
            ...
            <template #card-items-2>
              <div class="p-5">
                <div class="pb-4">
                  <FormLabel
                    html-for="roles"
                    :class="{ 'text-danger': errors['roles[]'] }"
                  >
                    {{ t("views.user.fields.roles") }}
                  </FormLabel>
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.roles"
                    name="roles"
                    rules="required"
            ...
            <template #card-items-3>
              <div class="p-5">
                <div class="pb-4">
                  ...
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.settings.theme"
                    name="theme"
                  >
                    <FormSelect
                      v-show="
                        mode == ViewMode.FORM_CREATE ||
                        mode == ViewMode.FORM_EDIT
                      "
                      id="theme"
                      name="theme"
                      v-bind="field"
                    >
                      ...
                      <option value="side-menu-dark-mini">
                        Mini Menu Dark
                      </option>
                    </FormSelect>
                  </VeeField>
                ...
                <div class="pb-4">
                  ...
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.settings.date_format"
                    name="date_format"
                  >
                    <FormSelect
                      v-show="
                ...
                <div class="pb-4">
                  ...
                  <VeeField
                    v-slot="{ field }"
                    v-model="userForm.data.settings.time_format"
                    name="time_format"
                  >
                    <FormSelect
                      v-show="
                  ...
                  >
                    <FormSwitch>
                      <FormSwitch.Input
                        ...
                        v-bind="field"
                        type="checkbox"
                        :class="{ 'border-danger': errors['api_token'] }"
                      />
                    </FormSwitch>
                  </VeeField>
                </div>
                  ...
                  >
                    <FormSwitch>
                      <FormSwitch.Input
                        ...
                        v-bind="field"
                        type="checkbox"
                        :class="{ 'border-danger': errors['reset_password'] }"
                      />
                    </FormSwitch>
                  </VeeField>
                </div>
