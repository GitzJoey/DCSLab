<script setup lang="ts">
  ...
  import StockAdjustmentOutItemSerialList from './StockAdjustmentOutItemSerialList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_out_item_serial.page_title');
  ...
  const stockAdjustmentOutItemSerialNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentOutItemSerialNotification]', (el: NotificationElement) => {
    stockAdjustmentOutItemSerialNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_out_item_serial.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentOutItemSerialNotification.value) {
      stockAdjustmentOutItemSerialNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <StockAdjustmentOutItemSerialList
    ...
    <Notification ref-key="stockAdjustmentOutItemSerialNotification" :options="{ duration: 3000 }" class="flex">
