...
const titleView = ref<string>('views.chart_of_account.page_title');
...
const chartOfAccountNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[chartOfAccountNotification]', (el: NotificationElement) => {
  chartOfAccountNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-chart-of-account-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.chart_of_account.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.chart_of_account.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.chart_of_account.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (chartOfAccountNotification.value) {
    chartOfAccountNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('CHART_OF_ACCOUNT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('CHART_OF_ACCOUNT_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="chartOfAccountNotification" :options="{ duration: 3000 }" class="flex">
