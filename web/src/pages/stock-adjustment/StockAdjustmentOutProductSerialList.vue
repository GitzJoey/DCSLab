<script setup lang="ts">
  ...
  const handleDateFilterChange = async () => {
    const perPage = stockAdjustmentOutProductSerialLists.value?.meta.per_page || 10;
    await getStockAdjustmentOutProductSerials(searchText.value, true, 1, perPage);
  };
  ...
  const getProductMainImageUrl = (item: StockAdjustmentOutProductSerial): string | null => {
    const images = item.stock_adjustment_out_product?.product_unit?.product?.product_images ?? [];
