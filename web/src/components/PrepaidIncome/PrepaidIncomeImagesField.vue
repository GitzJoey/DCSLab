...
import PrepaidIncomeImageService from '@/services/PrepaidIncomeImageService';
...
const props = withDefaults(
  defineProps<{
    ...
    existingImages?: PrepaidIncomeImage[];
...
const emit = defineEmits<{
  ...
  (e: 'update:existingImages', value: PrepaidIncomeImage[]): void;
  (e: 'update:deleteImageIds', value: Array<number | string>): void;
}>();

const { t } = useI18n();
const prepaidIncomeImageService = new PrepaidIncomeImageService();
...
const existingImages = computed({
  ...
  set: (value: PrepaidIncomeImage[]) => emit('update:existingImages', value),
});
...
const newImages = ref<PrepaidIncomeImage[]>([]);
...
const isImageMain = (image: PrepaidIncomeImage): boolean => {
...
const handleUpload = async (file: File) => {
  ...
  try {
    const uploadResponse: ServiceResponse<PrepaidIncomeImage | null> = await prepaidIncomeImageService.upload(file);
...
const setMainImage = (image: PrepaidIncomeImage) => {
...
const capturePhoto = () => {
  ...
  canvas.toBlob((blob) => {
    ...
    const file = new File([blob], `prepaid_income_${Date.now()}.jpg`, { type: 'image/jpeg' });
    handleUpload(file);
    stopCamera();
  }, 'image/jpeg');
};
