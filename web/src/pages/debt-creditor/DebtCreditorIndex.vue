<script setup lang="ts">
  ...
  const titleView = ref<string>('views.debt_creditor.page_title');
  ...
  const debtCreditorNotification = ref<NotificationElement>();
  ...
  provide('bind[debtCreditorNotification]', (el: NotificationElement) => {
    debtCreditorNotification.value = el;
  });
  // #endregion
  ...
  const createNew = () => {
    ...
    router.push({ name: 'side-menu-debt-creditor-create' });
  };
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.FORM_CREATE:
        titleView.value = 'views.debt_creditor.actions.create';
        break;
      case ViewMode.FORM_EDIT:
        titleView.value = 'views.debt_creditor.actions.edit';
        break;
      case ViewMode.INDEX:
      case ViewMode.LIST:
      default:
        titleView.value = 'views.debt_creditor.page_title';
        break;
    }
  };
  ...
  const clearCache = (mode: ViewMode) => {
    switch (mode) {
      case ViewMode.FORM_CREATE:
        cacheService.removeLastEntity('DEBT_CREDITOR_CREATE');
        break;
      case ViewMode.FORM_EDIT:
        cacheService.removeLastEntity('DEBT_CREDITOR_EDIT');
        break;
      default:
        break;
    }
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (debtCreditorNotification.value) debtCreditorNotification.value.showToast();
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="debtCreditorNotification" :options="{ duration: 3000 }" class="flex">
