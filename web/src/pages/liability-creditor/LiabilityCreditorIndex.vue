<script setup lang="ts">
  ...
  const titleView = ref<string>('views.liability_creditor.page_title');
  ...
  const liabilityCreditorNotification = ref<NotificationElement>();
  ...
  provide('bind[liabilityCreditorNotification]', (el: NotificationElement) => {
    liabilityCreditorNotification.value = el;
  });
  // #endregion
  ...
  const createNew = () => {
    ...
    router.push({ name: 'side-menu-liability-creditor-create' });
  };
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.FORM_CREATE:
        titleView.value = 'views.liability_creditor.actions.create';
        break;
      case ViewMode.FORM_EDIT:
        titleView.value = 'views.liability_creditor.actions.edit';
        break;
      case ViewMode.INDEX:
      case ViewMode.LIST:
      default:
        titleView.value = 'views.liability_creditor.page_title';
        break;
    }
  };
  ...
  const clearCache = (mode: ViewMode) => {
    switch (mode) {
      case ViewMode.FORM_CREATE:
        cacheService.removeLastEntity('LIABILITY_CREDITOR_CREATE');
        break;
      case ViewMode.FORM_EDIT:
        cacheService.removeLastEntity('LIABILITY_CREDITOR_EDIT');
        break;
      default:
        break;
    }
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (liabilityCreditorNotification.value) liabilityCreditorNotification.value.showToast();
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="liabilityCreditorNotification" :options="{ duration: 3000 }" class="flex">
