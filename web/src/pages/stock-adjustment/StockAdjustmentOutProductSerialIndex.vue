<script setup lang="ts">
  ...
  import StockAdjustmentOutProductSerialList from './StockAdjustmentOutProductSerialList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_out_product_serial.page_title');
  ...
  const stockAdjustmentOutProductSerialNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentOutProductSerialNotification]', (el: NotificationElement) => {
    stockAdjustmentOutProductSerialNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_out_product_serial.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentOutProductSerialNotification.value) {
      stockAdjustmentOutProductSerialNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <StockAdjustmentOutProductSerialList
    ...
    <Notification ref-key="stockAdjustmentOutProductSerialNotification" :options="{ duration: 3000 }" class="flex">
