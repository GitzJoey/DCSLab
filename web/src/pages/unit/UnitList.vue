...
import { UnitReadAnyPaginateRequest } from "@/types/services/unit/UnitRequest";
...
const getUnitTypesDDL = async () => {
    unitTypesDDL.value = await unitServices.getTypes();
};

const getUnitTypeLabel = (code: number | string) => {
    if (!unitTypesDDL.value) return code;
    const option = unitTypesDDL.value.find(x => x.code == code);
...
const getUnits = async (search: string, refresh: boolean, page: number, per_page: number) => {
    ...
    const searchReq: UnitReadAnyPaginateRequest = {
        with_trashed: false,
                            ...
                            <Table.Td colspan="4">
                                <div class="flex flex-row">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.unit.fields.code') }}</div>
                                    <div class="flex-1">{{ item.code }}</div>
                                </div>
                                <div class="flex flex-row mt-1">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.unit.fields.name') }}</div>
                                    <div class="flex-1">{{ item.name }}</div>
                                </div>
                                <div class="flex flex-row mt-1">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.unit.fields.type') }}</div>
                                    <div class="flex-1">{{ getUnitTypeLabel(item.type) }}</div>
                                </div>
                                <div class="flex flex-row mt-1">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.unit.fields.description') }}</div>
