...
const titleView = ref<string>('views.sales_order.page_title');
...
const salesOrderNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[salesOrderNotification]', (el: NotificationElement) => {
  salesOrderNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-sales-order-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.sales_order.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.sales_order.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.sales_order.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  salesOrderNotification.value?.showToast();
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      ...
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('SALES_ORDER_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesOrderNotification" :options="{ duration: 3000 }" class="flex">
