<script setup lang="ts">
  ...
  import StockAdjustmentOutItemList from './StockAdjustmentOutItemList.vue';
  ...
  const titleView = ref<string>('views.stock_adjustment_out_item.page_title');
  ...
  const stockAdjustmentOutItemNotification = ref<NotificationElement>();
  const notificationTitle = ref<string>('');
  const notificationContent = ref<string>('');

  provide('bind[stockAdjustmentOutItemNotification]', (el: NotificationElement) => {
    stockAdjustmentOutItemNotification.value = el;
  });
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.LIST:
      default:
        titleView.value = 'views.stock_adjustment_out_item.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (stockAdjustmentOutItemNotification.value) {
      stockAdjustmentOutItemNotification.value.showToast();
    }
  };
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="stockAdjustmentOutItemNotification" :options="{ duration: 3000 }" class="flex">
