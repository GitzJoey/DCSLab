...
const titleView = ref<string>("views.unit.page_title");
...
const unitNotification = ref<NotificationElement>();
...
provide("bind[unitNotification]", (el: NotificationElement) => {
  unitNotification.value = el;
});
// #endregion
...
const createNew = () => {
  ...
  router.push({ name: "side-menu-product-unit-create" });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = "views.unit.actions.create";
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = "views.unit.actions.edit";
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = "views.unit.page_title";
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (unitNotification.value) unitNotification.value.showToast();
};

const clearCache = (mode: ViewMode) => {
  switch (mode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity("UNIT_CREATE");
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity("UNIT_EDIT");
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="unitNotification" :options="{ duration: 3000 }" class="flex">
