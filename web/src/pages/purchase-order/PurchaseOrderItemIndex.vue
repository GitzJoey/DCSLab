...
import PurchaseOrderItemList from './PurchaseOrderItemList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.purchase_order_item.page_title');
...
const purchaseOrderItemNotification = ref<NotificationElement>();

provide('bind[purchaseOrderItemNotification]', (el: NotificationElement) => {
  purchaseOrderItemNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.purchase_order_item.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseOrderItemNotification.value) {
    purchaseOrderItemNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseOrderItemNotification" :options="{ duration: 3000 }" class="flex">
