...
import type {
  PurchaseReturnItemNestedStoreRequest,
  PurchaseReturnRefundNestedStoreRequest,
...
type PurchaseReturnItemFormItem = PurchaseReturnItemNestedStoreRequest & {
...
type PurchaseReturnRefundFormItem = PurchaseReturnRefundNestedStoreRequest;
...
const purchaseReturnForm = purchaseReturnService.usePurchaseReturnCreateForm();
...
/** Without a purchase invoice link the backend rejects any VAT on the items. */
...
watch(
  purchaseReturnForm,
  debounce((newValue) => {
    cacheService.setLastEntity('PURCHASE_RETURN_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('PURCHASE_RETURN_CREATE') as Record<string, unknown>;
  if (!data) return;
  purchaseReturnForm.setData(data);
};
// #endregion
...
const setCode = () => {
  ...
  purchaseReturnForm.setData({ code: purchaseReturnForm.code === '_AUTO_' ? '' : '_AUTO_' });
};
...
/** Force every item to be VAT free (the backend rejects VAT on uninvoiced returns). */
...
const clearSupplier = async () => {
  purchaseReturnForm.setData({ supplier_id: null });
...
const clearPurchaseInvoice = async () => {
  purchaseReturnForm.setData({ purchase_invoice_id: null });
...
const clearWarehouse = () => {
  purchaseReturnForm.setData({ warehouse_id: null });
...
/** On create nothing is allocated to an invoice yet, so the whole return is refundable. */
const getRefundCapPreview = () => Math.max(getAmountPayablePreview(), 0);

const getAmountAvailablePreview = () => Math.max(getRefundCapPreview() - getRefundsTotalPreview(), 0);

/** Client-side cap: a refund row may never push the total past the available amount. */
...
const onSubmit = async () => {
  ...
  const cleanedItems: PurchaseReturnItemNestedStoreRequest[] = purchaseReturnItemsForm.value.map((item) => ({
  ...
  const cleanedRefunds: PurchaseReturnRefundNestedStoreRequest[] = purchaseReturnRefundsForm.value.map((refund) => ({
    code: refund.code,
    date: refund.date,
  ...
  try {
    ...
    cacheService.removeLastEntity('PURCHASE_RETURN_CREATE');
    showAlertPlaceholder('hidden', '', null);
    emits('update-profile');
    showNotification(t('views.purchase_return.alert.create.title'), t('views.purchase_return.alert.create.message'));
...
<template>
  <form v-if="selectedUserLocation" id="purchaseReturnForm" @submit.prevent="onSubmit">
      ...
      <template #card-items-2>
        <div class="space-y-4 p-5">
          ...
          <div
            ...
            :key="`return-item-${item.product_unit_id}-${index}`"
