...
import type { AssetPurchaseReadAnyPaginateRequest } from '@/types/services/asset-purchase/AssetPurchaseRequest';
...
const assetPurchaseLists = ref<Collection<Array<AssetPurchase>> | null>({
  data: [],
  meta: {
    current_page: 0,
    from: null,
...
onMounted(async () => {
  ...
  await getAssetPurchases('', true, 1, 10);
});

const getAssetPurchases = async (search: string, refresh: boolean, page: number, perPage: number) => {
  emits('loading-state', true);
  searchText.value = search;

  const request: AssetPurchaseReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<AssetPurchase>> | null> =
    await assetPurchaseService.readAnyPaginate(request);

  if (result.success && result.data) {
    assetPurchaseLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getAssetPurchases(data.search.text, true, data.pagination.page, data.pagination.per_page);
};

const handleDateFilterChange = async () => {
  const perPage = assetPurchaseLists.value?.meta.per_page || 10;
  await getAssetPurchases(searchText.value, true, 1, perPage);
};
...
const editSelected = (idx: number) => {
  if (!assetPurchaseLists.value) return;

  const ulid = assetPurchaseLists.value.data[idx].ulid;
...
const deleteSelected = (idx: number) => {
  ...
  deleteUlid.value = assetPurchaseLists.value.data[idx].ulid;
...
const confirmDelete = async () => {
  ...
  const result = await assetPurchaseService.delete(deleteUlid.value);

  emits('loading-state', false);

  if (result.success) {
    emits('update-profile');
    await getAssetPurchases(searchText.value, true, 1, assetPurchaseLists.value?.meta.per_page || 10);
    showNotification(
      t('views.asset_purchase.alert.delete.title'),
      t('views.asset_purchase.alert.delete.message'),
    );
  } else {
    showAlertPlaceholder('danger', '', result.errors as Record<string, Array<string>>);
  }
    ...
    <div class="col-span-12 intro-y lg:col-span-12">
      <div class="grid grid-cols-12 gap-4 gap-y-3 mb-3">
        <div class="col-span-12 lg:col-span-3">
          <FormLabel>
            {{ t('views.asset_purchase.fields.start_date') }}
          </FormLabel>
          <FormInputDateTime v-model="startDate" @change="handleDateFilterChange" />
        </div>
        <div class="col-span-12 lg:col-span-3">
          <FormLabel>
            {{ t('views.asset_purchase.fields.end_date') }}
          </FormLabel>
      ...
      <DataListFlex
        :data="assetPurchaseLists"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="assetPurchaseLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="assetPurchaseLists ? assetPurchaseLists.meta : null"
        ...
        <template #row="{ item, index }">
          <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_purchase.page_title') }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_purchase.fields.code') }}:
              {{ (item as AssetPurchase).code ?? '-' }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_purchase.fields.date') }}:
              {{ (item as AssetPurchase).date ? formatDate((item as AssetPurchase).date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_purchase.fields.due_days') }}:
              {{ (item as AssetPurchase).due_days ?? 0 }}
            </div>
          </div>
          ...
          <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
            ...
            <div class="text-slate-500 text-xs whitespace-nowrap">
              [{{ (item as AssetPurchase).supplier?.code ?? '-' }}] {{ (item as AssetPurchase).supplier?.name ?? '-' }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_purchase.fields.item_total') }}:
              {{ formatCurrency((item as AssetPurchase).item_total ?? 0) }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_purchase.fields.amount_payable') }}:
              {{ formatCurrency((item as AssetPurchase).amount_payable ?? 0) }}
            </div>
          </div>
          ...
          <div class="col-span-12 lg:col-span-3 md:col-span-2 self-start">
            ...
            <div class="text-slate-500 text-xs whitespace-nowrap mb-1">
              <span class="inline-flex items-center">
                <Lucide v-if="(item as AssetPurchase).is_posted" icon="CheckCircle" class="w-4 h-4 text-success" />
                <Lucide v-else icon="X" class="w-4 h-4 text-danger" />
              </span>
            </div>
            <div v-if="(item as AssetPurchase).remarks?.trim()" class="text-slate-500 text-xs break-all">
              {{ t('views.asset_purchase.fields.remarks') }}:
              {{ (item as AssetPurchase).remarks }}
            </div>
          </div>
          ...
          >
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_purchase.field_groups.items') }}
            </div>
            <div v-if="((item as AssetPurchase).items?.length ?? 0) === 0" class="text-slate-500 text-xs italic">
              {{ t('components.data-list.data_not_found') }}
            </div>
            <div v-else class="space-y-2 text-xs">
              <div v-for="(assetItem, assetIdx) in (item as AssetPurchase).items" :key="assetItem.ulid" class="flex gap-2">
                ...
                <div class="flex-1">
                  ...
                  <div class="text-slate-500">
                    {{ t('views.asset_purchase.table.cols.qty') }}:
                    <span class="font-medium">{{ formatCurrency(assetItem.qty) }}</span>
                  </div>
                  <div class="text-slate-500">
                    {{ t('views.asset_purchase.table.cols.unit_price') }}:
                    <span class="font-medium">{{ formatCurrency(assetItem.unit_price) }}</span>
                  </div>
                  <div class="text-slate-500">
                    {{ t('views.asset_purchase.table.cols.subtotal') }}:
                    <span class="font-medium">{{ formatCurrency(assetItem.subtotal_after_additional_cost) }}</span>
                  </div>
