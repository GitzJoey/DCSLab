...
const getProducts = async (search: string, refresh: boolean, paginate: boolean, page: number, per_page: number) => {
  ...
  let result: ServiceResponse<Collection<Array<Product>> | Resource<Array<Product>> | null> = await productServices.readAny(searchReq);
...
const onDataListChanged = async (data: DataListEmittedData) => {
  await getProducts(data.search.text, false, true, data.pagination.page, data.pagination.per_page);
}
...
const editSelected = (itemIdx: number) => {
  ...
  let ulid = productLists.value.data[itemIdx].ulid;
  router.push({ name: 'side-menu-product-product-edit', params: { ulid: ulid } });
}

const deleteSelected = (itemIdx: number) => {
  if (!productLists.value) return;

  let itemUlid = productLists.value.data[itemIdx].ulid;
      ...
      <Table class="mt-5" :hover="true">
        <Table.Thead variant="light">
          <Table.Tr>
            ...
            <Table.Th class="whitespace-nowrap">
              {{ t("views.product.table.cols.remarks") }}
            </Table.Th>
            <Table.Th class="whitespace-nowrap">
