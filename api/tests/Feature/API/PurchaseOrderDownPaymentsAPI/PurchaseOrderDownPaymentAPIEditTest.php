...
class PurchaseOrderDownPaymentAPIEditTest extends APITestCase
{
    ...
    public function test_purchase_order_down_payments_api_call_update_without_authorization_expect_unauthorized_message()
    {
        ...
        $api = $this->json('POST', route('api.post.db.purchase_order.purchase_order_down_payments.edit', $purchaseOrderDownPayment->ulid), $purchaseOrderDownPaymentArr);
    ...
    public function test_purchase_order_down_payments_api_call_update_and_use_existing_code_in_same_company_expect_failed()
    {
        ...
        PurchaseOrderDownPayment::factory()->for($company)->count(2)->create();

        $purchaseOrderDownPayment = $company->purchaseOrderDownPayment()->inRandomOrder()->take(2)->get();
        $purchaseOrderDownPayment_1 = $purchaseOrderDownPayment[0];
        $purchaseOrderDownPayment_2 = $purchaseOrderDownPayment[1];

        $purchaseOrderDownPaymentArr = PurchaseOrderDownPayment::factory()->make([
            'company_id' => Hashids::encode($company->id),
            'code' => $purchaseOrderDownPayment_1->code,
        ])->toArray();

        $api = $this->json('POST', route('api.post.db.purchase_order.purchase_order_down_payments.edit', $purchaseOrderDownPayment_2->ulid), $purchaseOrderDownPaymentArr);
    ...
    public function test_purchase_order_down_payments_api_call_update_and_use_existing_code_in_different_company_expect_successful()
    {
        ...
        $purchaseOrderDownPayment_2 = PurchaseOrderDownPayment::factory()->for($company_2)->create([
            'code' => 'test2',
        ]);
