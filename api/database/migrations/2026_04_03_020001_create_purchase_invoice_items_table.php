...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('purchase_invoice_items', function (Blueprint $table) {
            ...
            $table->foreign('company_id', 'fk_pii_company_id')->references('id')->on('companies');
            $table->foreignId('branch_id');
            $table->foreign('branch_id', 'fk_pii_branch_id')->references('id')->on('branches');
            $table->foreignId('purchase_invoice_id');
            $table->foreign('purchase_invoice_id', 'fk_pii_purchase_invoice_id')->references('id')->on('purchase_invoices');
            $table->foreignId('purchase_order_item_id')->nullable();
            $table->foreign('purchase_order_item_id', 'fk_pii_purchase_order_item_id')->references('id')->on('purchase_order_items');
            $table->decimal('qty', 30, 8)->default(0);
            $table->foreignId('product_unit_id');
            $table->foreign('product_unit_id', 'fk_pii_product_unit_id')->references('id')->on('product_units');
            $table->foreignId('product_id');
            $table->foreign('product_id', 'fk_pii_product_id')->references('id')->on('products');
            ...
            $table->foreign('vat_profile_id', 'fk_pii_vat_profile_id')->references('id')->on('vat_profiles');
