...
const titleView = ref<string>('views.capital_transaction.page_title');
...
const capitalTransactionNotification = ref<NotificationElement>();

const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[capitalTransactionNotification]', (el: NotificationElement) => {
  capitalTransactionNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-finance-capital-transaction-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.capital_transaction.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.capital_transaction.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.capital_transaction.page_title';
      break;
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('CAPITAL_TRANSACTION_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('CAPITAL_TRANSACTION_EDIT');
      break;
    default:
      break;
  }
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (capitalTransactionNotification.value) {
    capitalTransactionNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="capitalTransactionNotification" :options="{ duration: 3000 }" class="flex">
