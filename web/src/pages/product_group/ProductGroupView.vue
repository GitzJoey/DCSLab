...
//#region Data - Views
const productGroupForm = ref<ProductGroupFormRequest>({
  data: {
    id: '',
    ulid: '',
    company: {
...
const productGroupLists = ref<Collection<ProductGroup[]> | null>({
  data: [],
  meta: {
    current_page: 0,
    from: null,
...
const productGroupCategoryDDL = ref<Array<DropDownOption> | null>(null);
//#endregion

//#region onMounted
onMounted(async () => {
  await getProductGroups('', true, true, 1, 10);
...
const getProductGroups = async (search: string, refresh: boolean, paginate: boolean, page: number, per_page: number) => {  
  ...
  let result: ServiceResponse<Collection<Array<ProductGroup>> | Resource<Array<ProductGroup>> | null> = await productGroupServices.readAny(company_id, searchReq);

  if (result.success && result.data) {
    productGroupLists.value = result.data as Collection<ProductGroup[]>;
  } else {
...
const onDataListChanged = (data: DataListEmittedData) => {
  getProductGroups(data.search.text, false, true, data.pagination.page, data.pagination.per_page);
}

const createNew = () => {
  mode.value = ViewMode.FORM_CREATE;

  let cachedData: unknown | null = cacheServices.getLastEntity('ProductGroup');

  productGroupForm.value = cachedData == null ? emptyProductGroup() : cachedData as ProductGroupFormRequest;
}
...
const editSelected = (itemIdx: number) => {
  ...
  productGroupForm.value.data = productGroupLists.value?.data[itemIdx] as ProductGroup;
}
...
const confirmDelete = async () => {
  ...
  let result: ServiceResponse<boolean | null> = await productGroupServices.delete(deleteUlid.value);
...
const onSubmit = async (values: ProductGroupFormFieldValues, actions: FormActions<ProductGroupFormFieldValues>) => {
  loading.value = true;

  let result: ServiceResponse<ProductGroup | null> = {
    success: false,
  }
...
const backToList = async () => {
  ...
  cacheServices.removeLastEntity('ProductGroup');
...
const titleView = computed(() => {
  switch (mode.value) {
    case ViewMode.FORM_CREATE:
      return t('views.product_group.actions.create');
    case ViewMode.FORM_EDIT:
      return t('views.product_group.actions.edit');
    case ViewMode.LIST:
    default:
      return t('views.product_group.page_title');
  }
});
//#endregion
...
watch(
  ...
  debounce((newValue): void => {
    ...
    cacheServices.setLastEntity('ProductGroup', newValue)
  }, 500),
  { deep: true }
);
//#endregion
  ...
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <div v-if="mode == ViewMode.LIST">
        <AlertPlaceholder :errors="datalistErrors" :title="t('views.product_group.table.title')" />
        <DataList :title="t('views.product_group.table.title')" :enable-search="true" :can-print="true" :can-export="true"
          :pagination="productGroupLists ? productGroupLists.meta : null" @dataListChanged="onDataListChanged">
          <template #content>
            <Table class="mt-5" :hover="true">
              <Table.Thead variant="light">
                <Table.Tr>
                <Table.Th class="whitespace-nowrap">
                {{ t("views.product_group.table.cols.code") }}
                </Table.Th>
                <Table.Th class="whitespace-nowrap">
                  {{ t("views.product_group.table.cols.name") }}
                </Table.Th>
                <Table.Th class="whitespace-nowrap">
                  {{ t("views.product_group.table.cols.category") }}
                </Table.Th>
                <Table.Th class="whitespace-nowrap"></Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody v-if="productGroupLists !== null">
                <template v-if="productGroupLists.data.length == 0">
                ...
                <template v-for="( item, itemIdx ) in productGroupLists.data" :key="item.ulid">
                  <Table.Tr class="intro-x">
                      ...
                      <div class="flex flex-row">
                        ...
                        <div class="flex-1">
                          <span v-if="item.category === 'PRODUCTS'">
                            {{ t('components.dropdown.values.productGroupCategoryDDL.product') }}
                          </span>
                          <span v-if="item.category === 'SERVICES'">
                            {{ t('components.dropdown.values.productGroupCategoryDDL.service') }}
                          </span>
                        </div>
                      </div>
                    </Table.Td>
      ...
      <div v-else>
        <VeeForm id="productGroupForm" v-slot="{ errors }" @submit="onSubmit">
            ...
            <template #card-items-0>
              <div class="p-5">
                ...
                <div class="pb-4">
                  ...
                  <VeeField v-slot="{ field }" name="category" rules="required" :label="t('views.product_group.fields.category')">
                    <FormSelect id="category" v-model="productGroupForm.data.category" v-bind="field" name="category"
