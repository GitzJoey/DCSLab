...
const titleView = ref<string>('views.expense_category.page_title');
...
const expenseCategoryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[expenseCategoryNotification]', (el: NotificationElement) => {
  expenseCategoryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-expense-category-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.expense_category.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.expense_category.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.expense_category.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (expenseCategoryNotification.value) {
    expenseCategoryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('EXPENSE_CATEGORY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('EXPENSE_CATEGORY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="expenseCategoryNotification" :options="{ duration: 3000 }" class="flex">
