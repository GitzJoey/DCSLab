...
const expensePaymentForm = expensePaymentService.useExpensePaymentEditForm(route.params.ulid.toString());
const expensePaymentData = ref<ExpensePayment | null>(null);
...
const expenseLabel = computed(() => {
  const expense = expensePaymentData.value?.expense;
  ...
  return [
    ...
    `Sisa ${formatCurrency(Number(expense.amount_payable_due ?? 0))}`,
  ]
    .filter(Boolean)
    .join(' - ');
});
...
const loadData = async () => {
  ...
  const result = await expensePaymentService.read(route.params.ulid.toString());
  emits('loading-state', false);

  if (result.success && result.data) {
    expensePaymentData.value = result.data;

    expensePaymentForm.setData({
      company_id: result.data.company?.id ?? '',
      branch_id: result.data.branch?.id ?? '',
      expense_id: result.data.expense?.id ?? '',
...
const loadCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: expensePaymentData.value?.cash_account?.id,
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            <div class="col-span-12">
              ...
              <FormInput type="hidden" v-model="expensePaymentForm.expense_id" />
