        ...
        >
          <template v-if="userContext.companies && userContext.companies.length != 0">
            <template v-for="(c, cIdx) in userContext.companies" :key="cIdx">
              ...
              <Menu.Item v-for="(b, bIdx) in c.branches" :key="bIdx" @click="setNewUserLocation(c.id, b == null ? '' : b.id)" class="pl-6 relative">
          ...
          <template v-else>
            <Menu.Item>
              <span class="text-primary">{{ t("components.user-location.data_not_found") }}</span>
            </Menu.Item>
          </template>
        </Menu.Items>
      </Menu>
