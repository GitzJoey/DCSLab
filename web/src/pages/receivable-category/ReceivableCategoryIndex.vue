...
const titleView = ref<string>('views.receivable_category.page_title');
...
const receivableCategoryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[receivableCategoryNotification]', (el: NotificationElement) => {
  receivableCategoryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-receivable-category-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.receivable_category.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.receivable_category.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.receivable_category.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (receivableCategoryNotification.value) {
    receivableCategoryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('RECEIVABLE_CATEGORY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('RECEIVABLE_CATEGORY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="receivableCategoryNotification" :options="{ duration: 3000 }" class="flex">
