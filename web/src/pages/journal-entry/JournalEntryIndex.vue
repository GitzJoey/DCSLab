...
const titleView = ref<string>('views.journal_entry.page_title');
...
const journalEntryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[journalEntryNotification]', (el: NotificationElement) => {
  journalEntryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-journal-entry-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.journal_entry.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.journal_entry.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.journal_entry.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (journalEntryNotification.value) {
    journalEntryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('JOURNAL_ENTRY_CREATE');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="journalEntryNotification" :options="{ duration: 3000 }" class="flex">
