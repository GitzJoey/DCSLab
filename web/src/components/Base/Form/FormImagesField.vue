...
import ExpenseImageService from '@/services/ExpenseImageService';
import type { ProductImage } from '@/types/models/ProductImage';
...
const emit = defineEmits<{
  ...
  (e: 'update:existingImages', value: ImageItem[]): void;
  (e: 'update:deleteImageIds', value: Array<number | string>): void;
}>();

const { t } = useI18n();
const productImageService = new ProductImageService();
const expenseImageService = new ExpenseImageService();
...
const existingImages = computed({
  ...
  set: (value: ImageItem[]) => emit('update:existingImages', value),
});
...
const isImageMain = (image: ImageItem): boolean => {
  const item = imageHashes.value.find((hash) => hash.hash === image.hash);
  return item ? item.is_main : false;
};

const uploadImage = async (file: File): Promise<ServiceResponse<ProductImage | ExpenseImage | null>> => {
  if (props.entity === 'expense') {
    return expenseImageService.upload(file);
  }

  return productImageService.upload(file);
};
...
const handleUpload = async (file: File) => {
  ...
  try {
    ...
    const image: ImageItem = {
      ...
      is_main: uploadResponse.data.is_main,
    };

    newImages.value = [...newImages.value, image];
...
const setMainImage = (image: ImageItem) => {
...
const capturePhoto = () => {
  ...
  canvas.toBlob((blob) => {
    ...
    const filenamePrefix = props.entity === 'expense' ? 'expense' : 'product';
    const file = new File([blob], `${filenamePrefix}_${Date.now()}.jpg`, { type: 'image/jpeg' });
    handleUpload(file);
    stopCamera();
  }, 'image/jpeg');
};
