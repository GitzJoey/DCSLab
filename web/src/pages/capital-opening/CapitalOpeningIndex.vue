...
const titleView = ref<string>('views.capital_opening.page_title');
...
const capitalOpeningNotification = ref<NotificationElement>();

const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[capitalOpeningNotification]', (el: NotificationElement) => {
  capitalOpeningNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-finance-capital-opening-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.capital_opening.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.capital_opening.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.capital_opening.page_title';
      break;
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('CAPITAL_OPENING_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('CAPITAL_OPENING_EDIT');
      break;
    default:
      break;
  }
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (capitalOpeningNotification.value) {
    capitalOpeningNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="capitalOpeningNotification" :options="{ duration: 3000 }" class="flex">
