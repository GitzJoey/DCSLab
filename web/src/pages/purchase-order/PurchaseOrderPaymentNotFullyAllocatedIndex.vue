...
import PurchaseOrderPaymentList from './PurchaseOrderPaymentList.vue';
import { AllocationStatus } from '@/types/enums/AllocationStatus';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.purchase_order_payment.page_title_not_fully_allocated');
...
const purchaseOrderPaymentNotification = ref<NotificationElement>();

provide('bind[purchaseOrderPaymentNotification]', (el: NotificationElement) => {
  purchaseOrderPaymentNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.purchase_order_payment.page_title_not_fully_allocated';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseOrderPaymentNotification.value) {
    purchaseOrderPaymentNotification.value.showToast();
  }
};
  ...
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <PurchaseOrderPaymentList
        :preset-allocation-status="AllocationStatus.NOT_FULLY_ALLOCATED"
        :hide-allocation-status-filter="true"
    ...
    <Notification ref-key="purchaseOrderPaymentNotification" :options="{ duration: 3000 }" class="flex">
