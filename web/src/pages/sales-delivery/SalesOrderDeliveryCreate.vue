...
import type {
  SalesOrderDeliveryCostNestedStoreRequest,
  SalesOrderDeliveryItemNestedStoreRequest,
...
type SalesOrderDeliveryItemFormItem = SalesOrderDeliveryItemNestedStoreRequest & {
...
const salesOrderDeliveryForm: any = salesOrderDeliveryService.useSalesOrderDeliveryCreateForm();
...
const getCosts = () => salesOrderDeliveryForm.costs as SalesOrderDeliveryCostNestedStoreRequest[];
...
const loadCustomerDDL = async (search = '') => {
  ...
  const result = await customerService.readAnyGet({
    ...
    search,
    status: undefined,
    include_id: (salesOrderDeliveryForm.customer_id as string | null | undefined) ?? undefined,
    refresh: false,
    limit: 100,
  });
...
const loadWarehouseDDL = async (search = '') => {
  ...
  const result = await warehouseService.readAnyGet({
    ...
    branch_id: selectedUserLocation.value.branch.id,
    search,
    include_id: (salesOrderDeliveryForm.warehouse_id as string | null | undefined) ?? undefined,
    status: undefined,
    refresh: false,
    limit: 100,
  });
...
const onSubmit = async () => {
  ...
  const cleanedItems: SalesOrderDeliveryItemNestedStoreRequest[] = originalItems.map((item) => ({
  ...
  const cleanedCosts: SalesOrderDeliveryCostNestedStoreRequest[] = originalCosts.map((cost) => ({
    code: cost.code,
    date: cost.date,
    name: cost.name,
  ...
  try {
    ...
    showNotification(
      t('views.sales_order_delivery.alert.create.title'),
      t('views.sales_order_delivery.alert.create.message'),
    );
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            <div class="col-span-12 lg:col-span-4 md:col-span-6">
              ...
              <FormInput v-model="salesOrderDeliveryForm.company_id" type="hidden" />
            </div>
            ...
            <div class="col-span-12 lg:col-span-4 md:col-span-6">
              ...
              <FormInput v-model="salesOrderDeliveryForm.branch_id" type="hidden" />
            </div>
          </div>
        </div>
      </template>
          ...
          <div class="rounded-md border border-slate-200/60 dark:border-darkmode-400 p-4 space-y-4">
            <div class="grid grid-cols-12 gap-4 gap-y-3 items-end">
              ...
              <div class="col-span-12 lg:col-span-3">
                <FormLabel>{{ t('views.sales_order_delivery.fields.total_cogs') }}</FormLabel>
                <FormInputCurrency :model-value="0" readonly />
              </div>
            </div>
