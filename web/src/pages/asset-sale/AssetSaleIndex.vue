...
const titleView = ref<string>('views.asset_sale.page_title');
...
const assetSaleNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[assetSaleNotification]', (el: NotificationElement) => {
  assetSaleNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-asset-sale-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.asset_sale.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.asset_sale.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.asset_sale.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (assetSaleNotification.value) {
    assetSaleNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('ASSET_SALE_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('ASSET_SALE_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="assetSaleNotification" :options="{ duration: 3000 }" class="flex">
