...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('sales_return_items', function (Blueprint $table) {
            ...
            $table->foreign('company_id', 'fk_srti_company_id')->references('id')->on('companies');
            $table->foreignId('branch_id');
            $table->foreign('branch_id', 'fk_srti_branch_id')->references('id')->on('branches');
            $table->foreignId('sales_return_id');
            $table->foreign('sales_return_id', 'fk_srti_sales_return_id')->references('id')->on('sales_returns');
            $table->foreignId('sales_order_delivery_item_id')->nullable();
            $table->foreign('sales_order_delivery_item_id', 'fk_srti_sales_order_delivery_item_id')->references('id')->on('sales_order_delivery_items');
            $table->decimal('qty', 30, 8)->default(0);
            $table->foreignId('product_unit_id');
            $table->foreign('product_unit_id', 'fk_srti_product_unit_id')->references('id')->on('product_units');
            $table->foreignId('product_id');
            $table->foreign('product_id', 'fk_srti_product_id')->references('id')->on('products');
            ...
            $table->foreign('vat_profile_id', 'fk_srti_vat_profile_id')->references('id')->on('vat_profiles');
