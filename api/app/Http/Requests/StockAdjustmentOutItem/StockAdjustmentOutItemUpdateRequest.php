...
namespace App\Http\Requests\StockAdjustmentOutItem;
...
class StockAdjustmentOutItemUpdateRequest extends FormRequest
{
    public function authorize()
    {
        ...
        /** @var \App\User */
        $user = Auth::user();
        $resource = $this->route('stock_adjustment_out_item');

        return $user->can('update', StockAdjustmentOutItem::class, $resource) ? true : false;
    }
    ...
    public function rules()
    {
        return [
            ...
            'delete_serial_ids.*' => ['required', 'integer', 'distinct', new ExistsForCompany('stock_adjustment_out_item_serials', $this->company_id)],
            'serials' => ['present', 'array'],
            'serials.*.id' => ['present', 'nullable', 'integer', new ExistsForCompany('stock_adjustment_out_item_serials', $this->company_id)],
    ...
    {
        return [
            'company_id' => trans('validation_attributes.stock_adjustment_out_item.company_id'),
            'branch_id' => trans('validation_attributes.stock_adjustment_out_item.branch_id'),
            'stock_adjustment_id' => trans('validation_attributes.stock_adjustment_out_item.stock_adjustment_id'),
            'qty' => trans('validation_attributes.stock_adjustment_out_item.qty'),
            'product_unit_id' => trans('validation_attributes.stock_adjustment_out_item.product_unit_id'),
            'product_unit_conversion_value' => trans('validation_attributes.stock_adjustment_out_item.product_unit_conversion_value'),
            'remarks' => trans('validation_attributes.stock_adjustment_out_item.remarks'),
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (str_contains($normalizedBaseQty, '.')) {
                $validator->errors()->add('serials', trans('validation.stock_adjustment_out_item.base_qty_must_be_integer'));
            ...
            if (bccomp($serialCount, $baseQty, 8) !== 0) {
                $validator->errors()->add('serials', trans('validation.stock_adjustment_out_item.serial_count_not_match_qty'));
            }
        });
    }
}
