...
import PurchaseOrderDownPaymentRefundList from './PurchaseOrderDownPaymentRefundList.vue';

const { t } = useI18n();

const loading = ref(false);
const titleView = ref<string>('views.purchase_order_down_payment_refund.page_title');
...
const purchaseOrderDownPaymentRefundNotification = ref<NotificationElement>();

provide('bind[purchaseOrderDownPaymentRefundNotification]', (el: NotificationElement) => {
  purchaseOrderDownPaymentRefundNotification.value = el;
});
...
const onModeStateChanged = (state: ViewMode) => {
  if (state === ViewMode.LIST) {
    titleView.value = 'views.purchase_order_down_payment_refund.page_title';
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (purchaseOrderDownPaymentRefundNotification.value) {
    purchaseOrderDownPaymentRefundNotification.value.showToast();
  }
};
...
<template>
  <div class="mt-8">
    <LoadingOverlay :visible="loading">
      ...
      <PurchaseOrderDownPaymentRefundList
    ...
    <Notification ref-key="purchaseOrderDownPaymentRefundNotification" :options="{ duration: 3000 }" class="flex">
