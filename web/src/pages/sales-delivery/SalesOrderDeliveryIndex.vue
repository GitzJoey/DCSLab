...
const titleView = ref<string>('views.sales_order_delivery.page_title');
...
const salesOrderDeliveryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[salesOrderDeliveryNotification]', (el: NotificationElement) => {
  salesOrderDeliveryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-sales-delivery-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.sales_order_delivery.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.sales_order_delivery.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.sales_order_delivery.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  salesOrderDeliveryNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('SALES_ORDER_DELIVERY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('SALES_ORDER_DELIVERY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="salesOrderDeliveryNotification" :options="{ duration: 3000 }" class="flex">
