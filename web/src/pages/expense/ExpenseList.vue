...
import type { ExpenseReadAnyPaginateRequest } from '@/types/services/expense/ExpenseRequest';
...
const previewImages = ref<ExpenseImage[]>([]);
const previewImageIndex = ref(0);
const previewExpenseCode = ref('');
const expenseLists = ref<Collection<Array<Expense>> | null>({
  data: [],
...
onMounted(async () => {
  ...
  await getExpenses('', true, 1, 10);
});

const getExpenses = async (search: string, refresh: boolean, page: number, per_page: number) => {
  emits('loading-state', true);
  const searchReq: ExpenseReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<Expense>> | null> = await expenseService.readAnyPaginate(searchReq);

  if (result.success && result.data) {
    expenseLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getExpenses(data.search.text, false, data.pagination.page, data.pagination.per_page);
};

const editSelected = (idx: number) => {
  if (!expenseLists.value) return;
  router.push({
    name: 'side-menu-expense-edit',
    params: { ulid: expenseLists.value.data[idx].ulid },
  });
};
...
const viewSelected = (idx: number) => {
  ...
  const expense = expenseLists.value.data[idx];
  expandDetail.value = expandDetail.value === expense.ulid ? null : expense.ulid;
};

const deleteSelected = (idx: number) => {
  if (!expenseLists.value) return;
  deleteUlid.value = expenseLists.value.data[idx].ulid;
...
const confirmDelete = async () => {
  ...
  emits('loading-state', true);
  const result = await expenseService.delete(deleteUlid.value);
  emits('loading-state', false);

  if (result.success) {
    emits('update-profile');
    await getExpenses('', true, 1, 10);
    showNotification(t('views.expense.alert.delete_expense.title'), t('views.expense.alert.delete_expense.content'));
  } else {
    showAlertPlaceholder('danger', '', result.errors as Record<string, Array<string>>);
  }
};
...
const getCategoryLabel = (expense: Expense) => {
  ...
  return expense.category.display_code ? `${expense.category.display_code} - ${expense.category.name}` : expense.category.name;
};

const getCashAccountLabel = (expense: Expense) => {
  if (!expense.paid_immediately_cash_account) return '-';

  return expense.paid_immediately_cash_account.code
    ? `${expense.paid_immediately_cash_account.code} - ${expense.paid_immediately_cash_account.name}`
    : expense.paid_immediately_cash_account.name;
};

const getPaidOffLabel = (expense: Expense) =>
  expense.is_amount_payable_paid_off ? t('views.expense.status.paid_off') : t('views.expense.status.not_paid_off');

const getExpenseImages = (expense: Expense): ExpenseImage[] => {
  const images = expense.expense_images?.filter((image) => !!image.url) ?? [];

  if (images.length > 0) {
    return images;
  }

  return expense.main_expense_image?.url ? [expense.main_expense_image] : [];
};

const getExpensePayments = (expense: Expense) => expense.payments ?? [];

const getPaymentCashAccountLabel = (payment: ExpensePayment) => {
...
const openImagePreview = (expense: Expense) => {
  const images = getExpenseImages(expense);
  ...
  previewExpenseCode.value = expense.code;
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12">
      <DataListFlex
        :data="expenseLists"
        :title="t('views.expense.table.title')"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="expenseLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="expenseLists ? expenseLists.meta : null"
      ...
      >
        <template #row="{ item, index }">
          <div class="col-span-12 md:col-span-6 lg:col-span-6 self-start">
            <div class="space-y-2">
              <div class="flex items-start justify-between gap-3">
                ...
                <div
                  v-if="(item as Expense).main_expense_image?.url"
                  ...
                  @click="openImagePreview(item as Expense)"
                >
                  <img
                    :src="(item as Expense).main_expense_image?.url"
              ...
              <div class="grid grid-cols-12 items-start gap-x-3 gap-y-2 text-xs">
                <div class="col-span-4 text-slate-500">{{ t('views.expense.fields.code') }}</div>
                <div class="col-span-8 break-words font-medium text-slate-700 dark:text-slate-200">
                  {{ (item as Expense).code }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.expense.fields.date') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ formatDate((item as Expense).date, 'YYYY-MM-DD HH:mm:ss') }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.expense.fields.category') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ getCategoryLabel(item as Expense) }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.expense.fields.paid_immediately_cash_account') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ getCashAccountLabel(item as Expense) }}
                </div>
                ...
                <div class="col-span-4 text-slate-500">{{ t('views.expense.fields.remarks') }}</div>
                <div class="col-span-8 break-words text-slate-700 dark:text-slate-200">
                  {{ (item as Expense).remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-start gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-7 text-right font-semibold text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Expense).amount_payable ?? 0)) }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.expense.fields.due_days') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ (item as Expense).due_days ?? 0 }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.expense.fields.amount_payable_paid') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Expense).amount_payable_paid ?? 0)) }}
                </div>
                <div class="col-span-5 text-slate-500">{{ t('views.expense.fields.amount_payable_due') }}</div>
                <div class="col-span-7 text-right text-slate-700 dark:text-slate-200">
                  {{ formatCurrency(Number((item as Expense).amount_payable_due ?? 0)) }}
                </div>
                <div class="col-span-7 text-slate-500">{{ t('views.expense.fields.amount_total') }}</div>
                <div class="col-span-5 text-right font-semibold text-primary">
                  {{ formatCurrency(Number((item as Expense).amount_total ?? 0)) }}
                </div>
                <div class="col-span-7 text-slate-500">{{ t('views.expense.fields.is_amount_payable_paid_off') }}</div>
                <div class="col-span-5 text-right">
                  <span
                    class="inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium"
                    :class="
                      (item as Expense).is_amount_payable_paid_off
                        ? 'bg-success/20 text-success'
                        : 'bg-warning/20 text-warning'
                    "
                  >
                    {{ getPaidOffLabel(item as Expense) }}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div
            v-if="expandDetail === (item as Expense).ulid"
          ...
          >
            <div class="grid grid-cols-12 gap-4">
              <div class="col-span-12 space-y-3">
                ...
                <div v-if="!getExpensePayments(item as Expense).length" class="text-xs text-slate-500">
                  {{ t('components.data-list.data_not_found') }}
                </div>

                <div v-else class="space-y-3 rounded-md border border-slate-200/60 p-3 dark:border-darkmode-400">
                  <div
                    v-for="payment in getExpensePayments(item as Expense)"
                    :key="payment.ulid"
