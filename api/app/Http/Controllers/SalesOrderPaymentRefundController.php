...
use App\Http\Resources\SalesOrderPaymentRefundResource;
...
class SalesOrderPaymentRefundController extends BaseController
{
    public function __construct(
        private SalesOrderPaymentRefundActions $salesOrderPaymentRefundActions,
    ) {
        parent::__construct();
    }
    ...
    public function readAny(Request $request)
    {
        ...
        $this->authorize('viewAny', SalesOrderPaymentRefund::class);
        ...
        try {
            $result = $this->salesOrderPaymentRefundActions->readAny(
        ...
        return SalesOrderPaymentRefundResource::collection($result);
    }

    public function read(SalesOrderPaymentRefund $salesOrderPaymentRefund)
    {
        if (! Auth::check()) return response()->error(trans('auth.unauthenticated'), 401);
        $this->authorize('view', $salesOrderPaymentRefund);

        $result = null;
        $errorMsg = '';

        try {
            $result = $this->salesOrderPaymentRefundActions->read($salesOrderPaymentRefund);
        } catch (Exception $e) {
            $errorMsg = app()->environment('production') ? '' : $e->getMessage();
        }
        ...
        return new SalesOrderPaymentRefundResource($result);
    }
}
