...
import type { AssetAdjustmentReadAnyPaginateRequest } from '@/types/services/asset-adjustment/AssetAdjustmentRequest';
...
const assetAdjustmentLists = ref<Collection<Array<AssetAdjustment>> | null>({
  data: [],
  meta: {
    current_page: 0,
    from: null,
...
onMounted(async () => {
  ...
  await getAssetAdjustments('', true, 1, 10);
});

const getAssetAdjustments = async (search: string, refresh: boolean, page: number, perPage: number) => {
  emits('loading-state', true);
  searchText.value = search;

  const request: AssetAdjustmentReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result: ServiceResponse<Collection<Array<AssetAdjustment>> | null> =
    await assetAdjustmentService.readAnyPaginate(request);

  if (result.success && result.data) {
    assetAdjustmentLists.value = result.data;
...
const handleDataListChange = async (data: DataListEmittedData) => {
  await getAssetAdjustments(data.search.text, true, data.pagination.page, data.pagination.per_page);
};

const handleDateFilterChange = async () => {
  const perPage = assetAdjustmentLists.value?.meta.per_page || 10;
  await getAssetAdjustments(searchText.value, true, 1, perPage);
};
...
const editSelected = (idx: number) => {
  if (!assetAdjustmentLists.value) return;

  const ulid = assetAdjustmentLists.value.data[idx].ulid;
...
const deleteSelected = (idx: number) => {
  ...
  deleteUlid.value = assetAdjustmentLists.value.data[idx].ulid;
...
const confirmDelete = async () => {
  ...
  const result = await assetAdjustmentService.delete(deleteUlid.value);

  emits('loading-state', false);

  if (result.success) {
    emits('update-profile');
    await getAssetAdjustments(searchText.value, true, 1, assetAdjustmentLists.value?.meta.per_page || 10);
    showNotification(
      t('views.asset_adjustment.alert.delete.title'),
      t('views.asset_adjustment.alert.delete.message'),
    );
  } else {
    showAlertPlaceholder('danger', '', result.errors as Record<string, Array<string>>);
  }
    ...
    <div class="col-span-12 intro-y lg:col-span-12">
      <div class="grid grid-cols-12 gap-4 gap-y-3 mb-3">
        <div class="col-span-12 lg:col-span-3">
          <FormLabel>
            {{ t('views.asset_adjustment.fields.start_date') }}
          </FormLabel>
          <FormInputDateTime v-model="startDate" @change="handleDateFilterChange" />
        </div>
        <div class="col-span-12 lg:col-span-3">
          <FormLabel>
            {{ t('views.asset_adjustment.fields.end_date') }}
          </FormLabel>
      ...
      <DataListFlex
        :data="assetAdjustmentLists"
        :enable-search="true"
        :can-print="true"
        :can-export="true"
        :rows="assetAdjustmentLists?.data ?? []"
        row-class="bg-white dark:bg-darkmode-600"
        :pagination="assetAdjustmentLists ? assetAdjustmentLists.meta : null"
        ...
        <template #row="{ item, index }">
          <div class="col-span-12 lg:col-span-5 md:col-span-4 self-start">
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_adjustment.page_title') }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_adjustment.fields.code') }}:
              {{ (item as AssetAdjustment).code ?? '-' }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_adjustment.fields.date') }}:
              {{ (item as AssetAdjustment).date ? formatDate((item as AssetAdjustment).date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
            </div>
            <div v-if="(item as AssetAdjustment).remarks?.trim()" class="text-slate-500 text-xs">
              {{ t('views.asset_adjustment.fields.remarks') }}:
              {{ (item as AssetAdjustment).remarks }}
            </div>
          </div>

          <div class="col-span-12 lg:col-span-4 md:col-span-4 self-start">
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_adjustment.field_groups.company_info') }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ (item as AssetAdjustment).branch?.name ?? '-' }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_adjustment.fields.total_incoming_asset_qty') }}:
              {{ formatCurrency((item as AssetAdjustment).total_incoming_asset_qty ?? 0) }}
            </div>
            <div class="text-slate-500 text-xs whitespace-nowrap">
              {{ t('views.asset_adjustment.fields.total_outgoing_asset_qty') }}:
              {{ formatCurrency((item as AssetAdjustment).total_outgoing_asset_qty ?? 0) }}
            </div>
          </div>
          ...
          <div class="col-span-12 lg:col-span-2 md:col-span-2 self-start">
            ...
            <div class="text-slate-500 text-xs whitespace-nowrap">
              <span class="inline-flex items-center">
                <Lucide v-if="(item as AssetAdjustment).is_posted" icon="CheckCircle" class="w-4 h-4 text-success" />
          ...
          >
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_adjustment.field_groups.in_items') }}
            </div>
            <div v-if="((item as AssetAdjustment).in_items?.length ?? 0) === 0" class="text-slate-500 text-xs italic">
              {{ t('components.data-list.data_not_found') }}
            </div>
            <div v-else class="space-y-2 text-xs mb-3">
              <div v-for="(assetItem, assetIdx) in (item as AssetAdjustment).in_items" :key="assetItem.ulid" class="flex gap-2">
                <div class="w-6 text-right text-slate-500">{{ assetIdx + 1 }}.</div>
                <div class="flex-1">
                  <div class="font-medium break-all">
                    [{{ assetItem.asset.code }}] {{ assetItem.asset.name }}
                  </div>
            ...
            <div class="text-primary text-xs font-semibold uppercase tracking-wide mb-1">
              {{ t('views.asset_adjustment.field_groups.out_items') }}
            </div>
            <div v-if="((item as AssetAdjustment).out_items?.length ?? 0) === 0" class="text-slate-500 text-xs italic">
              {{ t('components.data-list.data_not_found') }}
            </div>
            <div v-else class="space-y-2 text-xs">
              <div v-for="(assetItem, assetIdx) in (item as AssetAdjustment).out_items" :key="assetItem.ulid" class="flex gap-2">
                <div class="w-6 text-right text-slate-500">{{ assetIdx + 1 }}.</div>
                <div class="flex-1">
                  <div class="font-medium break-all">
                    [{{ assetItem.asset.code }}] {{ assetItem.asset.name }}
