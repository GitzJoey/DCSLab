...
const capitalTransactionForm = capitalTransactionService.useCapitalTransactionCreateForm();
...
onMounted(async () => {
  ...
  await Promise.all([loadInvestorDDL(), loadCashAccountDDL(), loadTypeDDL()]);
...
const loadFromCache = () => {
  const data = cacheServices.getLastEntity('CAPITAL_TRANSACTION_CREATE') as Record<string, unknown>;

  if (!data) return;

  capitalTransactionForm.setData(data);
};
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('CAPITAL_TRANSACTION_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
</script>
