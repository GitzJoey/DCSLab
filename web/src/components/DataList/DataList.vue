...
const textInfo = computed(() => {
  ...
  return (
    t("components.data-list.showing") +
    " " +
    props.pagination.from.toString() +
    " " +
    t("components.data-list.to") +
    " " +
    props.pagination.to.toString() +
    " " +
    t("components.data-list.of") +
    " " +
    props.pagination.total.toString() +
    " " +
...
const generatePaginationArray = (currentPage: number, totalRecords: number, perPage: number, maxVisiblePages = 7): number[] => {
...
const searchTextboxChanged = () => {
  if (props.pagination != null) emits("dataListChanged", createDataEmittedPayload(search.value, 1, props.pagination.per_page));
};

const refreshButtonClicked = () => {
  if (props.pagination != null) emits("dataListChanged", createDataEmittedPayload(search.value, props.pagination.current_page, props.pagination.per_page));
};
...
const paginationNumberButtonClicked = (n: number) => {
  if (props.pagination != null) emits("dataListChanged", createDataEmittedPayload(search.value, n, props.pagination.per_page));
};
...
const paginationLastButtonClicked = () => {
  if (props.pagination != null) emits("dataListChanged", createDataEmittedPayload(search.value, props.pagination.last_page, props.pagination.per_page));
};
      ...
      <div v-if="pages.length > 0" class="pb-1 border-b">
        <Pagination class="w-full sm:w-auto sm:mr-auto">
          ...
          <template v-for="n in pages" :key="n">
            <template v-if="n > 0">
              <Pagination.Link :active="pagination ? n == pagination.current_page : false" @click="paginationNumberButtonClicked(n)">
                {{ n }}
              </Pagination.Link>
            </template>
            <template v-else>
