...
class PrepaidExpenseUpdateRequest extends FormRequest
{
    public function authorize()
    {
        ...
        /** @var \App\User */
        $user = Auth::user();
        $prepaidExpense = $this->route('prepaid_expense');

        return $user->can('update', $prepaidExpense);
    }
    ...
    public function rules()
    {
        ...
        return [
            ...
            'delete_image_ids.*' => [
                'required',
                'integer',
                Rule::exists('prepaid_expense_images', 'id')->where(function ($query) use ($prepaidExpense) {
                    $query->where('prepaid_expense_id', $prepaidExpense->id);
                }),
            ],
            ...
            'delete_payment_ids.*' => [
                ...
                'integer',
                'distinct',
                Rule::exists('prepaid_expense_payments', 'id')->where(function ($query) use ($prepaidExpense) {
                    $query->where('company_id', $this->company_id)
                        ->where('prepaid_expense_id', $prepaidExpense->id);
                }),
            ],
    ...
    public function attributes()
    {
        return array_merge(
            ...
            [
                'payments.*.id' => trans('validation_attributes.prepaid_expense_payment.id'),
    ...
    {
        return [
            'delete_payment_ids.*.exists' => trans('rules.prepaid_expense.invalid_payment_reference'),
            'payments.*.id.exists' => trans('rules.prepaid_expense.invalid_payment_reference'),
        ];
    }
}
