...
class SaleOrderProductUnitActions
{
    ...
    public function readAny(
        ...
        $query = SaleOrderProductUnit::select('sale_order_product_units.*')
            ->with([
                'company',
                'branch',
                'saleOrder',
            ...
            ->whereCompanyId('sale_order_product_units', $companyId)
            ->withTrashed();

        $query->where(function ($query) use ($withTrashed, $search, $branchId, $saleOrderId, $productId, $productUnitId) {
            ...
            if ($branchId) {
                $query->where('sale_order_product_units.branch_id', $branchId);
            }

            if ($saleOrderId) {
                $query->where('sale_order_product_units.sale_order_id', $saleOrderId);
            }

            if ($productId) {
                $query->where('sale_order_product_units.product_id', $productId);
            }

            if ($productUnitId) {
                $query->where('sale_order_product_units.product_unit_id', $productUnitId);
            }
        });
        ...
        if ($execute) {
            ...
            try {
                ...
                $cacheKey = 'read_any_sale_order_product_unit_'.implode('_', $cacheParams);
    ...
    {
        return $saleOrderProductUnit->load([
            'company',
            'branch',
            'saleOrder',
            'product',
    ...
    public function create(array $data): SaleOrderProductUnit
    {
        ...
        try {
            ...
            $saleOrderProductUnit->sales_order_id = $data['sales_order_id'];
    ...
    {
        $result = SaleOrderProductUnit::whereCompanyId('sale_order_product_units', $companyId)->where('code', '=', $code);
