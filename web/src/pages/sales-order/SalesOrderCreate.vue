...
import {
  SalesOrderItemNestedStoreRequest,
  SalesOrderPaymentNestedStoreRequest,
  SalesOrderPaymentRefundNestedStoreRequest,
...
type SalesOrderItemFormItem = SalesOrderItemNestedStoreRequest & {
...
const salesOrderForm = salesOrderService.useSalesOrderCreateForm();
...
const salesOrderPaymentsForm = computed<SalesOrderPaymentNestedStoreRequest[]>(
  () => salesOrderForm.payments as SalesOrderPaymentNestedStoreRequest[],
);
const isPaymentEditorExpanded = ref(false);
const salesOrderRefundedPaymentsForm = computed<SalesOrderPaymentRefundNestedStoreRequest[]>(
  () => salesOrderForm.refunded_payments as SalesOrderPaymentRefundNestedStoreRequest[],
);
...
watch(
  salesOrderForm,
  debounce((newValue) => {
    cacheService.setLastEntity('SALES_ORDER_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
...
const loadVatProfileDDL = async (search = '') => {
  ...
  salesOrderItemsForm.value.forEach((item) => {
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('SALES_ORDER_CREATE') as Record<string, unknown>;
  if (!data) return;
  salesOrderForm.setData(data);
  salesOrderItemDetailsExpanded.value = salesOrderItemsForm.value.map(() => false);
};
...
const selectProductUnit = (option: ProductUnitOption) => {
  ...
  const itemData: SalesOrderItemFormItem = {
    qty: 1,
  ...
  if (editingProductUnitIndex.value === null) {
    salesOrderForm.items.push(itemData as any);
    targetIndex = salesOrderForm.items.length - 1;
    salesOrderItemDetailsExpanded.value[targetIndex] = false;
  } else {
    const currentItem = salesOrderItemsForm.value[editingProductUnitIndex.value];
...
const onSubmit = async () => {
  ...
  const cleanedItems: SalesOrderItemNestedStoreRequest[] = salesOrderItemsForm.value.map((item) => ({
    qty: item.qty,
  ...
  try {
    ...
    cacheService.removeLastEntity('SALES_ORDER_CREATE');
    showAlertPlaceholder('hidden', '', null);
    emits('update-profile');
    showNotification(t('views.sales_order.alert.create.title'), t('views.sales_order.alert.create.message'));
