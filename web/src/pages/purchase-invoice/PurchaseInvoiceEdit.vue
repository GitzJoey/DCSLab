...
import type {
  PurchaseInvoiceItemNestedUpdateRequest,
  PurchaseInvoicePaymentNestedUpdateRequest,
...
type PurchaseInvoiceItemFormItem = PurchaseInvoiceItemNestedUpdateRequest & {
...
type PurchaseInvoicePaymentFormItem = PurchaseInvoicePaymentNestedUpdateRequest;
...
const purchaseInvoiceForm = purchaseInvoiceService.usePurchaseInvoiceEditForm(route.params.ulid as string);
...
const purchaseInvoiceData = ref<PurchaseInvoice | null>(null);
...
onMounted(async () => {
  ...
  try {
    await Promise.all([loadPurchaseOrderDDL(), loadVatProfileDDL(), loadCashAccountDDL()]);
...
const appendPurchaseReturnOption = (purchaseReturn: PurchaseReturn | null | undefined) => {
  if (!purchaseReturn?.id) return;

  if (!purchaseReturnDDL.value.some((item) => item.id === purchaseReturn.id)) {
    purchaseReturnDDL.value = [...purchaseReturnDDL.value, purchaseReturn];
  }
};
...
const loadPurchaseOrderDDL = async (search = '') => {
  ...
  appendPurchaseOrderOption(purchaseInvoiceData.value?.purchase_order ?? null);
};
...
const loadCashAccountDDL = async (search = '') => {
  ...
  (purchaseInvoiceData.value?.payments ?? []).forEach((payment) => {
    if (!payment.cash_account) return;
    appendDropDownOption(cashAccountDDL, {
      code: payment.cash_account.id,
      name: `${payment.cash_account.code} - ${payment.cash_account.name}`,
    });
  });
};
...
const loadPurchaseReturnDDL = async (search = '') => {
  ...
  (purchaseInvoiceData.value?.payments ?? []).forEach((payment) => {
    appendPurchaseReturnOption(payment.purchase_return ?? null);
  });
};
// #endregion
...
const loadData = async () => {
  ...
  const result = await purchaseInvoiceService.read(ulid);
  ...
  const items: PurchaseInvoiceItemFormItem[] = (data.items ?? []).map((item: any) => {
    ...
    return {
      ...
      product_unit_base_unit_name:
        conversionValue !== 1 ? (item.product_unit?.product?.base_product_unit?.unit?.name ?? '') : '',
      vat_profile_name: item.vat_profile?.name ?? null,
      purchase_order_item_label: item.purchase_order_item?.product_unit?.code
        ? `[${item.purchase_order_item.product_unit.code}] ${
            item.purchase_order_item.product_unit.product?.name ?? '-'
          }`
        : null,
  ...
  const payments: PurchaseInvoicePaymentFormItem[] = (data.payments ?? []).map((payment: any) => ({
    id: payment.id ?? null,
    code: payment.code,
    ...
    purchase_order_payment_id: payment.purchase_order_payment?.id ?? null,
    purchase_return_id: payment.purchase_return?.id ?? null,
  ...
  purchaseInvoiceForm.setData({
    ...
    due_days: Number(data.due_days ?? 0),
    supplier_id: data.supplier?.id ?? null,
    purchase_order_id: data.purchase_order?.id ?? null,
    tax_invoice_number: data.tax_invoice_number ?? '',
    tax_invoice_vat_base: Number(data.tax_invoice_vat_base ?? 0),
    tax_invoice_vat: Number(data.tax_invoice_vat ?? 0),
    ...
    items: items as any,
    delete_payment_ids: [] as any,
    payments: payments as any,
  } as any);

  appendPurchaseOrderOption(data.purchase_order ?? null);
  isPaymentEditorExpanded.value = payments.length > 0;
...
/** Existing rows that get replaced by a purchase order resync must be deleted server-side. */
const markAllItemsForDeletion = () => {
  purchaseInvoiceItemsForm.value.forEach((item) => {
    if (item.id) purchaseInvoiceForm.delete_item_ids.push(item.id);
  });
};

const syncFromPurchaseOrder = (purchaseOrder: PurchaseOrder) => {
  markAllItemsForDeletion();
...
const handlePurchaseOrderChanged = async (purchaseOrderId: string | number | null) => {
  ...
  if (purchaseOrderId === (purchaseInvoiceData.value?.purchase_order?.id ?? null)) {
    await loadPurchaseOrderDetailById(String(purchaseOrderId));
    return;
  }
...
const setCode = () => {
  ...
  purchaseInvoiceForm.setData({ code: purchaseInvoiceForm.code === '_AUTO_' ? '' : '_AUTO_' } as any);
};
...
const removePayment = (index: number) => {
  const payment = purchaseInvoicePaymentsForm.value[index];
  if (payment?.id) purchaseInvoiceForm.delete_payment_ids.push(payment.id);
...
const onSubmit = async () => {
  ...
  const cleanedItems: PurchaseInvoiceItemNestedUpdateRequest[] = purchaseInvoiceItemsForm.value.map((item) => ({
    id: item.id ?? null,
    qty: Number(item.qty ?? 0),
  ...
  const cleanedPayments: PurchaseInvoicePaymentNestedUpdateRequest[] = purchaseInvoicePaymentsForm.value.map(
    (payment) => ({
      id: payment.id ?? null,
      code: payment.code,
      date: payment.date,
  ...
  try {
    ...
    showNotification(t('views.purchase_invoice.alert.update.title'), t('views.purchase_invoice.alert.update.message'));
...
<template>
  <form v-if="selectedUserLocation && !initializing" id="purchaseInvoiceForm" @submit.prevent="onSubmit">
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-4">
              ...
              <FormInput
                :model-value="
                  selectedPurchaseOrderData?.supplier?.name ??
                  selectedPurchaseOrderOption?.supplier_name ??
      ...
      <template #card-items-3>
        <div class="space-y-4 p-5">
          ...
          <div
            ...
            :key="`invoice-item-${item.id ?? item.product_unit_id}-${index}`"
              ...
              <div v-if="isPaymentEditorExpanded" class="space-y-4">
                ...
                <div v-else class="space-y-4">
                  <div
                    ...
                    :key="`invoice-payment-${payment.id ?? 'new'}-${index}`"
                    class="rounded-md border border-slate-200/70 p-4 dark:border-darkmode-400"
                  >
                    <div class="grid grid-cols-12 gap-4 gap-y-3">
                      <div class="col-span-12 md:col-span-6 lg:col-span-2">
