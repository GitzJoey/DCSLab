...
const titleView = ref<string>('views.asset_unit.page_title');
...
const assetUnitNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[assetUnitNotification]', (el: NotificationElement) => {
  assetUnitNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-asset-unit-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.asset_unit.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.asset_unit.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.asset_unit.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (assetUnitNotification.value) {
    assetUnitNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('ASSET_UNIT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('ASSET_UNIT_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="assetUnitNotification" :options="{ duration: 3000 }" class="flex">
