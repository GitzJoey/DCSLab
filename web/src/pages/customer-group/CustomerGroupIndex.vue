<script setup lang="ts">
  ...
  const titleView = ref<string>('views.customer_group.page_title');
  ...
  const customerGroupNotification = ref<NotificationElement>();
  ...
  provide('bind[customerGroupNotification]', (el: NotificationElement) => {
    customerGroupNotification.value = el;
  });
  // #endregion
  ...
  const createNew = () => {
    ...
    router.push({ name: 'side-menu-customer-group-create' });
  };
  ...
  const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
      case ViewMode.FORM_CREATE:
        titleView.value = 'views.customer_group.actions.create';
        break;
      case ViewMode.FORM_EDIT:
        titleView.value = 'views.customer_group.actions.edit';
        break;
      case ViewMode.INDEX:
      case ViewMode.LIST:
      default:
        titleView.value = 'views.customer_group.page_title';
        break;
    }
  };
  ...
  const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (customerGroupNotification.value) customerGroupNotification.value.showToast();
  };

  const clearCache = (mode: ViewMode) => {
    switch (mode) {
      case ViewMode.FORM_CREATE:
        cacheService.removeLastEntity('CUSTOMER_GROUP_CREATE');
        break;
      case ViewMode.FORM_EDIT:
        cacheService.removeLastEntity('CUSTOMER_GROUP_EDIT');
        break;
      default:
        break;
    }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="customerGroupNotification" :options="{ duration: 3000 }" class="flex">
