...
class ExpensePaymentStoreRequest extends FormRequest
{
    public function authorize()
    {
        ...
        return $user->can('create', ExpensePayment::class);
    }
    ...
    public function prepareForValidation()
    {
        $this->merge([
            ...
            'expense_id' => $this->filled('expense_id') ? HashidsHelper::decodeId($this->expense_id) : null,
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            $expense = Expense::find($this->input('expense_id'));
            if (! $expense) {
                return;
            }

            if ((float) $this->input('amount', 0) > (float) $expense->amount_payable_due) {
                $validator->errors()->add('amount', trans('validation.max.numeric', [
                    'attribute' => trans('validation_attributes.expense_payment.amount'),
                    'max' => $expense->amount_payable_due,
                ]));
            }
        });
    }
