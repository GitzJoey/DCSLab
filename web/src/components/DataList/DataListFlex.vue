<script setup lang="ts">
  ...
  export interface DataListFlexProps<T = unknown> {
    visible?: boolean;
    canPrint?: boolean;
    canExport?: boolean;
  ...
  const props = withDefaults(defineProps<DataListFlexProps>(), {
    visible: true,
    canPrint: false,
    canExport: false,
    enableSearch: false,
  ...
  const resolveRowKey = (item: unknown, index: number): string | number => {
    if (typeof item === 'object' && item !== null) {
      const row = item as Record<string, unknown>;
      if (typeof row.ulid === 'string' || typeof row.ulid === 'number') return row.ulid;
      if (typeof row.id === 'string' || typeof row.id === 'number') return row.id;
      if (typeof row.code === 'string' || typeof row.code === 'number') return row.code;
    }
...
<template>
  <div v-if="visible" class="intro-y box p-5 mt-5">
    ...
    <div v-if="showExportButtonGroups" class="mt-2 flex justify-end">
    ...
    <div class="mt-5 mb-4 space-y-3">
      <template v-if="rows.length > 0">
        <div
          v-for="(item, index) in rows"
          :key="resolveRowKey(item, index)"
          :class="[
            'grid grid-cols-12 gap-3 items-center rounded-md border border-slate-200/60 dark:border-darkmode-400 p-4',
            rowClass,
          ]"
        >
      ...
      <div v-if="pages.length > 0" class="pb-1 border-b">
        <Pagination class="w-full sm:w-auto sm:mr-auto">
          ...
          <template v-for="n in pages" :key="n">
            <template v-if="n > 0">
              <Pagination.Link
                :active="pagination ? n == pagination.current_page : false"
