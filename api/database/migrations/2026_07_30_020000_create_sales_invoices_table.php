...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('sales_invoices', function (Blueprint $table) {
            ...
            $table->foreign('company_id', 'fk_si_company_id')->references('id')->on('companies');
            $table->foreignId('branch_id');
            $table->foreign('branch_id', 'fk_si_branch_id')->references('id')->on('branches');
            $table->string('code');
            $table->dateTime('date');
            $table->integer('due_days')->default(0);
            $table->foreignId('customer_id');
            $table->foreign('customer_id', 'fk_si_customer_id')->references('id')->on('customers');
            $table->foreignId('sales_order_id');
            $table->foreign('sales_order_id', 'fk_si_sales_order_id')->references('id')->on('sales_orders');
