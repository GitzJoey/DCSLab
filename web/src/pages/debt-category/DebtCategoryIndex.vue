...
const titleView = ref<string>('views.debt_category.page_title');
...
const debtCategoryNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[debtCategoryNotification]', (el: NotificationElement) => {
  debtCategoryNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-debt-category-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.debt_category.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.debt_category.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.debt_category.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (debtCategoryNotification.value) {
    debtCategoryNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('DEBT_CATEGORY_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('DEBT_CATEGORY_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="debtCategoryNotification" :options="{ duration: 3000 }" class="flex">
