...
const titleView = ref<string>('views.stock_transfer.page_title');
...
const stockTransferNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[stockTransferNotification]', (el: NotificationElement) => {
  stockTransferNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-stock-transfer-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.stock_transfer.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.stock_transfer.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.stock_transfer.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  stockTransferNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('STOCK_TRANSFER_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('STOCK_TRANSFER_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="stockTransferNotification" :options="{ duration: 3000 }" class="flex">
