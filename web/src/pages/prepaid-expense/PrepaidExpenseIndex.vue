...
const titleView = ref<string>('views.prepaid_expense.page_title');
...
const prepaidExpenseNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[prepaidExpenseNotification]', (el: NotificationElement) => {
  prepaidExpenseNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-prepaid-expense-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.prepaid_expense.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.prepaid_expense.actions.edit';
      break;
    default:
      titleView.value = 'views.prepaid_expense.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  prepaidExpenseNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('PREPAID_EXPENSE_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PREPAID_EXPENSE_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="prepaidExpenseNotification" :options="{ duration: 3000 }" class="flex">
