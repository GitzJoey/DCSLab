...
const productGroupLists = ref<Collection<Array<ProductGroup>> | null>({
  data: [],
  meta: {
    current_page: 0,
    from: null,
...
const getProductGroups = async (search: string, refresh: boolean, paginate: boolean, page: number, per_page: number) => {
  ...
  let result: ServiceResponse<Collection<Array<ProductGroup>> | Resource<Array<ProductGroup>> | null> = await productGroupServices.readAny(searchReq);

  if (result.success && result.data) {
    productGroupLists.value = result.data as Collection<Array<ProductGroup>>;
  } else {
...
const onDataListChanged = async (data: DataListEmittedData) => {
  await getProductGroups(data.search.text, false, true, data.pagination.page, data.pagination.per_page);
}
...
const editSelected = (itemIdx: number) => {
  if (!productGroupLists.value) return;

  let ulid = productGroupLists.value.data[itemIdx].ulid;
  router.push({ name: 'side-menu-product-product_group-edit', params: { ulid: ulid } });
}

const deleteSelected = (itemIdx: number) => {
  if (!productGroupLists.value) return;

  let itemUlid = productGroupLists.value.data[itemIdx].ulid;
            ...
            <Table.Tr class="intro-x">
              ...
              <Table.Td>
                <span v-if="item.category === 'PRODUCTS'">{{
                  t('components.dropdown.values.productGroupCategoryDDL.product') }}</span>
                <span v-if="item.category === 'SERVICES'">{{
                  t('components.dropdown.values.productGroupCategoryDDL.service') }}</span>
              </Table.Td>
              <Table.Td>
