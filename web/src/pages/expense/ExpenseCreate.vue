...
const expenseForm = expenseService.useExpenseCreateForm();
...
const loadFromCache = () => {
  const data = cacheServices.getLastEntity('EXPENSE_CREATE') as Record<string, unknown>;
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await expenseCategoryService.readAnyGet({
    ...
    has_children: false,
    include_id: expenseForm.expense_category_id || undefined,
    refresh: false,
    limit: 20,
  });
...
const loadPaidImmediatelyCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: expenseForm.paid_immediately_cash_account_id || undefined,
...
const onSubmit = async () => {
  ...
  await expenseForm
    .submit()
    .then(() => {
      cacheServices.removeLastEntity('EXPENSE_CREATE');
      resetForm();
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('EXPENSE_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
</script>
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
        ...
        <div class="p-5">
          <div class="grid grid-cols-12 gap-4 gap-y-3">
            <div class="col-span-12 md:col-span-6">
              ...
              <FormInputCurrency type="hidden" v-model="expenseForm.company_id" />
            </div>
            <div class="col-span-12 md:col-span-6">
              <FormLabel>
      ...
      <template #card-items-3>
        <div class="p-5">
          ...
          <ExpenseImagesField v-model="expenseForm.image_hashes" />
