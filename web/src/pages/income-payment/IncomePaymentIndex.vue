...
const titleView = ref<string>('views.income_payment.page_title');
...
const incomePaymentNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[incomePaymentNotification]', (el: NotificationElement) => {
  incomePaymentNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-income-payment-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.income_payment.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.income_payment.actions.edit';
      break;
    default:
      titleView.value = 'views.income_payment.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  incomePaymentNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('INCOME_PAYMENT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('INCOME_PAYMENT_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="incomePaymentNotification" :options="{ duration: 3000 }" class="flex">
