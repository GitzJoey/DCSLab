...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('sales_order_items', function (Blueprint $table) {
            ...
            $table->foreign('company_id', 'fk_soi_company_id')->references('id')->on('companies');
            $table->foreignId('branch_id');
            $table->foreign('branch_id', 'fk_soi_branch_id')->references('id')->on('branches');
            $table->foreignId('sales_order_id');
            $table->foreign('sales_order_id', 'fk_soi_sales_order_id')->references('id')->on('sales_orders');

            $table->decimal('qty', 30, 8)->default(0);
            $table->foreignId('product_unit_id');
            $table->foreign('product_unit_id', 'fk_soi_product_unit_id')->references('id')->on('product_units');
            $table->foreignId('product_id');
            $table->foreign('product_id', 'fk_soi_product_id')->references('id')->on('products');
            $table->decimal('product_unit_conversion_value', 30, 8)->default(0);
            $table->decimal('product_unit_qty_base', 30, 8)->default(0);
            $table->decimal('qty_delivered_base', 30, 8)->default(0);
            $table->decimal('qty_invoiced_base', 30, 8)->default(0);
            ...
            $table->foreign('vat_profile_id', 'fk_soi_vat_profile_id')->references('id')->on('vat_profiles');
