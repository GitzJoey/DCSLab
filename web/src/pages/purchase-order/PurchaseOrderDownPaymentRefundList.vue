...
import PurchaseOrderDownPaymentRefundService from '@/services/PurchaseOrderDownPaymentRefundService';
...
import type { PurchaseOrderDownPaymentRefundReadAnyPaginateRequest } from '@/types/services/purchase-order-down-payment-refund/PurchaseOrderDownPaymentRefundRequest';
import type { ServiceResponse } from '@/types/services/ServiceResponse';
import { formatCurrency, formatDate } from '@/utils/helper';

const { t } = useI18n();
const router = useRouter();
const purchaseOrderDownPaymentRefundService = new PurchaseOrderDownPaymentRefundService();
...
const lists = ref<Collection<Array<PurchaseOrderDownPaymentRefund>> | null>({
  data: [],
...
const getLists = async (search: string, refresh: boolean, page: number, perPage: number) => {
  ...
  const request: PurchaseOrderDownPaymentRefundReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result = (await purchaseOrderDownPaymentRefundService.readAnyPaginate(
    request,
  )) as ServiceResponse<Collection<Array<PurchaseOrderDownPaymentRefund>> | null>;
          ...
          <div class="col-span-12 md:col-span-12 lg:col-span-4 self-start">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as PurchaseOrderDownPaymentRefund).code }}</div>
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderDownPaymentRefund).date ? formatDate((item as PurchaseOrderDownPaymentRefund).date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.supplier_id') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderDownPaymentRefund).purchase_order?.supplier?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-6 lg:col-span-4 self-start md:pr-3 lg:pr-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-5 text-right text-primary font-medium">{{ formatCurrency((item as PurchaseOrderDownPaymentRefund).amount ?? 0) }}</div>
                ...
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderDownPaymentRefund).cash_account?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-5 lg:col-span-3 self-start md:pl-3 lg:pl-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as PurchaseOrderDownPaymentRefund).purchase_order?.code ?? '-' }}
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              <div class="col-span-12 lg:col-span-6 space-y-3">
                ...
                <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                  ...
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as PurchaseOrderDownPaymentRefund).company?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.supplier_id') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as PurchaseOrderDownPaymentRefund).purchase_order?.supplier?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.purchase_order.fields.code') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as PurchaseOrderDownPaymentRefund).purchase_order?.code ?? '-' }}</div>
                </div>
              </div>
              <div class="col-span-12 lg:col-span-6 space-y-3">
                <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                ...
                <div class="rounded-md border border-slate-200/60 dark:border-darkmode-400 px-3 py-2 text-slate-700 dark:text-slate-200 break-words min-h-[72px]">
                  {{ (item as PurchaseOrderDownPaymentRefund).remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
