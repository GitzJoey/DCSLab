...
const vatProfileForm = vatProfileService.useVatProfileEditForm(route.params.ulid as string);
...
const loadData = async (ulid: string) => {
  ...
  const result: ServiceResponse<VatProfile | null> = await vatProfileService.read(ulid);
  ...
  if (result.success && result.data) {
    vatProfileForm.setData({
      ...
      vat_base_numerator: result.data.vat_base_numerator,
      vat_base_denominator: result.data.vat_base_denominator,
...
watch(
  ...
  debounce((newValue): void => {
    cacheServices.setLastEntity('VAT_PROFILE_EDIT', newValue.data());
  }, 500),
  { deep: true },
);
</script>
      ...
      <template #card-items-button>
        <div class="flex gap-4">
          <Button
            ...
            :disabled="vatProfileForm.validating || vatProfileForm.hasErrors"
          >
