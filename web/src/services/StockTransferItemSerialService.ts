...
import {
  type StockTransferItemSerialReadAnyGetRequest,
  type StockTransferItemSerialReadAnyPaginateRequest,
} from '../types/services/stock-transfer-item-serial/StockTransferItemSerialRequest';
import { StatusCode } from '../types/enums/StatusCode';
import { client, useForm } from 'laravel-precognition-vue';

export default class StockTransferItemSerialService {
  ...
  public async readAnyPaginate(
    args: StockTransferItemSerialReadAnyPaginateRequest,
  ): Promise<ServiceResponse<Collection<Array<StockTransferItemSerial>> | null>> {
    ...
    try {
      ...
      const url = route(
        'api.get.stock_transfer_item_serial.read_any',
        {
          _query: queryParams,
        },
        false,
      ...
      const response: AxiosResponse<Collection<Array<StockTransferItemSerial>>> = await axios.get(url);
  ...
  public async readAnyGet(
    args: StockTransferItemSerialReadAnyGetRequest,
  ): Promise<ServiceResponse<Resource<Array<StockTransferItemSerial>> | null>> {
    const result: ServiceResponse<Resource<Array<StockTransferItemSerial>> | null> = {
      success: false,
    };
    ...
    try {
      ...
      const response: AxiosResponse<Resource<Array<StockTransferItemSerial>>> = await axios.get(url);
  ...
  public async read(ulid: string): Promise<ServiceResponse<StockTransferItemSerial | null>> {
    const result: ServiceResponse<StockTransferItemSerial | null> = {
      success: false,
    };

    try {
      const url = route(
        'api.get.stock_transfer_item_serial.read',
        {
          stock_transfer_item_serial: ulid,
        },
        false,
        this.ziggyRoute,
      );

      const response: AxiosResponse<Resource<StockTransferItemSerial>> = await axios.get(url);
  ...
  public useStockTransferItemSerialCreateForm() {
    const url = route('api.post.stock_transfer_item_serial.save', undefined, true, this.ziggyRoute);
  ...
  public useStockTransferItemSerialEditForm(ulid: string) {
    const url = route(
      'api.post.stock_transfer_item_serial.edit',
      {
        stock_transfer_item_serial: ulid,
      },
      true,
  ...
  public async delete(ulid: string): Promise<ServiceResponse<boolean | null>> {
    ...
    try {
      const url = route(
        'api.post.stock_transfer_item_serial.delete',
        {
          stock_transfer_item_serial: ulid,
        },
        false,
