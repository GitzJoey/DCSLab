...
const editSelected = (itemIdx: number) => {
    ...
    router.push({ name: 'side-menu-product-product-category-edit', params: { ulid: ulid } });
};
...
<template>
    <DataList :title="t('views.product_category.table.title')" :enable-search="true" :can-print="true" :can-export="true"
        :pagination="productCategoryLists ? productCategoryLists.meta : null" @dataListChanged="onDataListChanged">
        <template #content>
            <Table class="mt-5" :hover="true">
                ...
                <Table.Tbody v-if="productCategoryLists !== null">
                    <template v-if="productCategoryLists.data.length == 0">
                            ...
                            <Table.Td colspan="4">
                                <div class="flex flex-row">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.product_category.fields.code') }}</div>
                                    <div class="flex-1">{{ item.code }}</div>
                                </div>
                                <div class="flex flex-row mt-1">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.product_category.fields.name') }}</div>
                                    <div class="flex-1">{{ item.name }}</div>
                                </div>
                                <div class="flex flex-row mt-1">
                                    <div class="ml-5 w-48 text-right pr-5 font-medium">{{
                                        t('views.product_category.fields.type') }}</div>
