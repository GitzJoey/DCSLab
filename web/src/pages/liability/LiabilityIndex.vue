...
const titleView = ref<string>('views.liability.page_title');
...
const liabilityNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[liabilityNotification]', (el: NotificationElement) => {
  liabilityNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-liability-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.liability.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.liability.actions.edit';
      break;
    default:
      titleView.value = 'views.liability.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  liabilityNotification.value?.showToast();
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      ...
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('LIABILITY_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="liabilityNotification" :options="{ duration: 3000 }" class="flex">
