...
const titleView = ref<string>('views.expense.page_title');
...
const expenseNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[expenseNotification]', (el: NotificationElement) => {
  expenseNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-expense-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.expense.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.expense.actions.edit';
      break;
    default:
      titleView.value = 'views.expense.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  expenseNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('EXPENSE_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('EXPENSE_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="expenseNotification" :options="{ duration: 3000 }" class="flex">
