...
const prepaidExpenseForm = prepaidExpenseService.usePrepaidExpenseCreateForm();
...
const loadFromCache = () => {
  const data = cacheService.getLastEntity('PREPAID_EXPENSE_CREATE') as Record<string, unknown>;

  if (!data) return;

  prepaidExpenseForm.setData(data);
};
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await expenseCategoryService.readAnyGet({
    ...
    has_children: false,
    include_id: prepaidExpenseForm.expense_category_id || undefined,
    refresh: false,
    limit: 20,
  });
...
const loadPaidImmediatelyCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: prepaidExpenseForm.paid_immediately_cash_account_id || undefined,
...
watch(
  ...
  debounce((newValue): void => {
    cacheService.setLastEntity('PREPAID_EXPENSE_CREATE', newValue.data());
  }, 500),
  { deep: true },
);
</script>
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <div v-else class="space-y-4">
            <div v-for="(payment, index) in prepaidExpensePaymentsForm" :key="`prepaid-expense-payment-${index}`" class="space-y-3">
              <div class="grid grid-cols-12 gap-4 gap-y-3">
      ...
      <template #card-items-3>
        <div class="p-5">
          ...
          <PrepaidExpenseImagesField v-model="prepaidExpenseForm.image_hashes" />
