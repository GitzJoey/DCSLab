...
const receivableForm = receivableService.useReceivableEditForm(route.params.ulid.toString());
const receivableData = ref<Receivable | null>(null);
...
const loadData = async () => {
  ...
  const result = await receivableService.read(route.params.ulid.toString());
  ...
  receivableForm.setData({
    ...
    delete_payment_ids: [],
    payments: (result.data.payments ?? []).map((payment: ReceivablePayment) => ({
      id: payment.id,
      code: payment.code,
...
const loadCategoryDDL = async (search = '') => {
  ...
  const result = await receivableCategoryService.readAnyGet({
    ...
    search,
    include_id: receivableData.value?.category?.id,
    refresh: false,
    limit: 20,
  });
...
const loadCustomerDDL = async (search = '') => {
  ...
  const result = await customerService.readAnyGet({
    ...
    company_id: selectedUserLocation.value.company.id,
    search,
    include_id: receivableData.value?.customer?.id ?? undefined,
    refresh: false,
    limit: 20,
  });
...
const loadCashAccountDDL = async (search = '') => {
  ...
  const result = await cashAccountService.readAnyGet({
    ...
    is_bank: null,
    search,
    include_id: receivableData.value?.cash_account?.id ?? undefined,
...
const removePayment = (index: number) => {
  ...
  if (payment?.id && !receivableForm.delete_payment_ids.includes(payment.id)) {
    receivableForm.delete_payment_ids.push(payment.id);
  }
    ...
    <TwoColumnsLayout :cards="cards" :using-side-tab="false" @handle-expand-card="handleExpandCard">
      ...
      <template #card-items-2>
        <div class="p-5 space-y-4">
          ...
          <FormErrorMessages :messages="(receivableForm.errors as any).delete_payment_ids" />
          ...
          <div v-else class="space-y-4">
            <div
              ...
              :key="payment.id ?? `receivable-payment-${index}`"
