...
const titleView = ref<string>('views.purchase_invoice.page_title');
...
const purchaseInvoiceNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[purchaseInvoiceNotification]', (el: NotificationElement) => {
  purchaseInvoiceNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-purchase-invoice-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.purchase_invoice.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.purchase_invoice.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.purchase_invoice.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  purchaseInvoiceNotification.value?.showToast();
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      ...
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PURCHASE_INVOICE_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseInvoiceNotification" :options="{ duration: 3000 }" class="flex">
