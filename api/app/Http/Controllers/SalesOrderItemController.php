...
class SalesOrderItemController extends BaseController
{
    public function __construct(
        private SalesOrderItemActions $salesOrderItemActions,
    ) {
        parent::__construct();
    }
    ...
    public function readAny(Request $request)
    {
        ...
        $this->authorize('viewAny', SalesOrderItem::class);

        $request->merge([
            'company_id' => $request->filled('company_id') ? HashidsHelper::decodeId($request->company_id) : null,
            'branch_id' => $request->filled('branch_id') ? HashidsHelper::decodeId($request->branch_id) : null,
            'sales_order_customer_id' => $request->filled('sales_order_customer_id') ? HashidsHelper::decodeId($request->sales_order_customer_id) : null,
        ...
        $validated = $request->validate([
            ...
            'sales_order_code' => ['nullable', 'string'],
            'sales_order_start_date' => ['nullable', 'string', new IsValidDate('Y-m-d H:i:s')],
            'sales_order_end_date' => ['nullable', 'string', new IsValidDate('Y-m-d H:i:s'), 'after_or_equal:sales_order_start_date'],
            'sales_order_customer_id' => ['nullable', 'integer', new ExistsForCompany('customers', $request->company_id)],
        ...
        try {
            $result = $this->salesOrderItemActions->readAny(
                withTrashed: $validated['with_trashed'],
                companyId: $validated['company_id'],
                branchId: $validated['branch_id'] ?? null,
                search: $validated['search'] ?? null,

                salesOrderCode: $validated['sales_order_code'] ?? null,
                salesOrderStartDate: $validated['sales_order_start_date'] ?? null,
                salesOrderEndDate: $validated['sales_order_end_date'] ?? null,
                salesOrderCustomerId: $validated['sales_order_customer_id'] ?? null,
        ...
        return SalesOrderItemResource::collection($result);
    }

    public function read(SalesOrderItem $salesOrderItem)
    {
        if (! Auth::check()) return response()->error(trans('auth.unauthenticated'), 401);
        $this->authorize('view', $salesOrderItem);

        $result = null;
        $errorMsg = '';

        try {
            $result = $this->salesOrderItemActions->read($salesOrderItem);
        } catch (Exception $e) {
            $errorMsg = app()->environment('production') ? '' : $e->getMessage();
        }
        ...
        return new SalesOrderItemResource($result);
    }
}
