...
const titleView = ref<string>('views.asset_adjustment.page_title');
...
const assetAdjustmentNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[assetAdjustmentNotification]', (el: NotificationElement) => {
  assetAdjustmentNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-asset-adjustment-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.asset_adjustment.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.asset_adjustment.actions.edit';
      break;
    case ViewMode.INDEX:
    case ViewMode.LIST:
    default:
      titleView.value = 'views.asset_adjustment.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  if (assetAdjustmentNotification.value) {
    assetAdjustmentNotification.value.showToast();
  }
};
...
const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('ASSET_ADJUSTMENT_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('ASSET_ADJUSTMENT_EDIT');
      break;
    default:
      break;
  }
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="assetAdjustmentNotification" :options="{ duration: 3000 }" class="flex">
