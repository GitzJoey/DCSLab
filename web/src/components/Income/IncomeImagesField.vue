...
import IncomeImageService from '@/services/IncomeImageService';
...
const emit = defineEmits<{
  ...
  (e: 'update:existingImages', value: IncomeImage[]): void;
  (e: 'update:deleteImageIds', value: Array<number | string>): void;
}>();

const { t } = useI18n();
const incomeImageService = new IncomeImageService();
...
const existingImages = computed({
  ...
  set: (value: IncomeImage[]) => emit('update:existingImages', value),
});
...
const newImages = ref<IncomeImage[]>([]);
...
const isImageMain = (image: IncomeImage): boolean => {
...
const handleUpload = async (file: File) => {
  ...
  try {
    const uploadResponse: ServiceResponse<IncomeImage | null> = await incomeImageService.upload(file);
...
const setMainImage = (image: IncomeImage) => {
...
const capturePhoto = () => {
  ...
  canvas.toBlob((blob) => {
    ...
    const file = new File([blob], `income_${Date.now()}.jpg`, { type: 'image/jpeg' });
    handleUpload(file);
    stopCamera();
  }, 'image/jpeg');
};
