...
const receivableCategoryLists = ref<Resource<Array<ReceivableCategory>> | null>({
  data: [],
});
// #endregion
...
onMounted(async () => {
  ...
  await getReceivableCategories('', true);
});
// #endregion

// #region Methods
const getReceivableCategories = async (search: string, refresh: boolean) => {
  emits('loading-state', true);

  const result: ServiceResponse<Resource<Array<ReceivableCategory>> | null> = await receivableCategoryService.readAnyGet({
    with_trashed: false,
  ...
  if (result.success && result.data) {
    receivableCategoryLists.value = result.data;
...
const onDataListChanged = async () => {
  await getReceivableCategories('', false);
};
...
const editSelected = (item: ReceivableCategory) => {
  router.push({
...
const deleteSelected = (item: ReceivableCategory) => {
...
const confirmDelete = async () => {
  ...
  const result: ServiceResponse<boolean | null> = await receivableCategoryService.delete(deleteUlid.value);

  if (result.success) {
    emits('update-profile');
    await getReceivableCategories('', true);
    showNotification(
      t('views.receivable_category.alert.delete_receivable_category.title'),
      t('views.receivable_category.alert.delete_receivable_category.content'),
    );
  } else {
...
<template>
  <DataList
    :title="t('views.receivable_category.table.title')"
    :enable-search="false"
    :can-print="true"
    :can-export="true"
    :pagination="null"
  ...
  >
    <template #content>
      <Table class="mt-5" :hover="true">
        ...
        <Table.Tbody v-if="receivableCategoryLists !== null">
          <template v-if="receivableCategoryLists.data.length == 0">
          ...
          <template v-else v-for="item in receivableCategoryLists.data" :key="item.ulid">
            <Table.Tr class="intro-x">
              <Table.Td>
              ...
              <Table.Td>
                <div class="flex justify-end gap-1">
                  ...
                  <Button variant="outline-secondary" @click="editSelected(item as ReceivableCategory)">
                    <Lucide icon="Pen" class="w-4 h-4" />
                  </Button>
                  <Button variant="outline-secondary" @click="deleteSelected(item as ReceivableCategory)">
