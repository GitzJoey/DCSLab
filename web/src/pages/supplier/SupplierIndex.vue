...
const titleView = ref<string>('views.supplier.page_title');
...
const supplierNotification = ref<NotificationElement>();
...
provide("bind[supplierNotification]", (el: NotificationElement) => {
    supplierNotification.value = el;
});
// #endregion
...
const createNew = () => {
    ...
    router.push({ name: 'side-menu-supplier-supplier-create' });
};
...
const backToList = async () => {
    ...
    router.push({ name: 'side-menu-supplier-supplier-list' });
};
...
const onModeStateChanged = (state: ViewMode) => {
    ...
    switch (state) {
        case ViewMode.FORM_CREATE:
            titleView.value = 'views.supplier.actions.create';
            break;
        case ViewMode.FORM_EDIT:
            titleView.value = 'views.supplier.actions.edit';
            break;
        case ViewMode.INDEX:
        case ViewMode.LIST:
        default:
            titleView.value = 'views.supplier.page_title';
            break;
    }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
    ...
    if (supplierNotification.value)
        supplierNotification.value.showToast();
};

const clearCache = (mode: ViewMode) => {
    switch (mode) {
        case ViewMode.FORM_CREATE:
            cacheService.removeLastEntity('SUPPLIER_CREATE');
            break;
        case ViewMode.FORM_EDIT:
            cacheService.removeLastEntity('SUPPLIER_EDIT');
            break;
        default:
            break;
    }
    ...
    <div class="mt-8">
        <LoadingOverlay :visible="loading">
            ...
            <RouterView @loading-state="onLoadingStateChanged" @mode-state="onModeStateChanged"
                @update-profile="onUpdateProfileTriggered" @show-alert-placeholder="onAlertPlaceholderTriggered"
                @show-notification="onShowNotificationTriggered" />
        </LoadingOverlay>
        <Notification ref-key="supplierNotification" :options="{ duration: 3000, }" class="flex">
