...
import SalesOrderPaymentService from '@/services/SalesOrderPaymentService';
...
import type { SalesOrderPaymentReadAnyPaginateRequest } from '@/types/services/sales-order-payment/SalesOrderPaymentRequest';
...
const lists = ref<Collection<Array<SalesOrderPayment>> | null>({
  data: [],
...
onMounted(async () => {
  ...
  await Promise.all([loadCustomerDDL(), loadCashAccountDDL(), loadAllocationStatusDDL()]);
...
const getLists = async (search: string, refresh: boolean, page: number, perPage: number) => {
  ...
  const request: SalesOrderPaymentReadAnyPaginateRequest = {
    with_trashed: false,
  ...
  const result = (await salesOrderPaymentService.readAnyPaginate(
    request,
  )) as ServiceResponse<Collection<Array<SalesOrderPayment>> | null>;
...
const loadAllocationStatusDDL = async () => {
  const result = await salesOrderPaymentService.readAllocationStatuses();
  if (result.success && result.data) {
...
const editSelected = (index: number) => {
  const ulid = lists.value?.data?.[index]?.sales_order?.ulid;
  if (!ulid) return;
  ...
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12">
      <div class="grid grid-cols-12 gap-4 mb-5">
        ...
        <div class="col-span-12 md:col-span-3">
          <FormLabel>{{ t('views.sales_order.fields.cash_account_id') }}</FormLabel>
          <FormSelectSearch
        ...
        <div v-if="!props.hideAllocationStatusFilter" class="col-span-12 md:col-span-3">
          <FormLabel>{{ t('views.sales_order.filters.payment_allocation_status') }}</FormLabel>
      ...
      >
        <template #row="{ item, index }">
          <div class="col-span-12 md:col-span-12 lg:col-span-4 self-start">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as SalesOrderPayment).sales_order?.code ?? '-' }}</div>
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).sales_order?.date ? formatDate((item as SalesOrderPayment).sales_order?.date as string, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.customer_id') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).sales_order?.customer?.name ?? '-' }}
                </div>
                ...
                <div class="col-span-8 text-slate-700 dark:text-slate-200">
                  {{ (item as SalesOrderPayment).sales_order?.due_days ?? 0 }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.remarks') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).sales_order?.remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-6 lg:col-span-4 self-start md:pr-3 lg:pr-4">
            <div class="space-y-2">
              <div class="text-primary text-xs font-semibold uppercase tracking-wide">
                {{ t('views.sales_order.field_groups.payments') }}
              </div>
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.code') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">{{ (item as SalesOrderPayment).code ?? '-' }}</div>
                <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.date') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).date ? formatDate((item as SalesOrderPayment).date, 'DD-MMM-YYYY HH:mm:ss') : '-' }}
                </div>
                <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.cash_account_id') }}</div>
                <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).cash_account?.name ?? '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="col-span-12 md:col-span-5 lg:col-span-3 self-start md:pl-3 lg:pl-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                <div class="col-span-7 text-slate-500">{{ t('views.sales_order.fields.amount') }}</div>
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200">{{ formatCurrency((item as SalesOrderPayment).amount ?? 0) }}</div>
                <div class="col-span-7 text-slate-500">{{ t('views.sales_order.fields.amount_allocated') }}</div>
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200">{{ formatCurrency((item as SalesOrderPayment).amount_allocated ?? 0) }}</div>
                <div class="col-span-7 text-slate-500">{{ t('views.sales_order.fields.remarks') }}</div>
                <div class="col-span-5 text-right text-slate-700 dark:text-slate-200 break-words">
                  {{ (item as SalesOrderPayment).remarks?.trim() || '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="expandDetail === index" class="col-span-12 border-t border-slate-200 dark:border-darkmode-400 mt-2 pt-3">
            <div class="grid grid-cols-12 gap-4">
              <div class="col-span-12 lg:col-span-6 space-y-3">
                ...
                <div class="grid grid-cols-12 items-center gap-x-3 gap-y-2 text-xs">
                  <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.company_id') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as SalesOrderPayment).company?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.customer_id') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200">{{ (item as SalesOrderPayment).sales_order?.customer?.name ?? '-' }}</div>
                  <div class="col-span-4 text-slate-500">{{ t('views.sales_order.fields.amount_available_down_payment') }}</div>
                  <div class="col-span-8 text-slate-700 dark:text-slate-200 break-words">
                    {{ formatCurrency(((item as SalesOrderPayment).amount ?? 0) - ((item as SalesOrderPayment).amount_allocated ?? 0)) }}
                  </div>
                </div>
              </div>
              <div class="col-span-12 lg:col-span-6 space-y-3">
