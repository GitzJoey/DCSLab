...
const titleView = ref<string>('views.purchase_additional_cost.page_title');
...
const purchaseAdditionalCostNotification = ref<NotificationElement>();
const notificationTitle = ref<string>('');
const notificationContent = ref<string>('');

provide('bind[purchaseAdditionalCostNotification]', (el: NotificationElement) => {
  purchaseAdditionalCostNotification.value = el;
});

const createNew = () => {
  resetAlertPlaceholder();
  mode.value = ViewMode.FORM_CREATE;
  router.push({ name: 'side-menu-purchase-additional-cost-create' });
};
...
const onModeStateChanged = (state: ViewMode) => {
  ...
  switch (state) {
    case ViewMode.FORM_CREATE:
      titleView.value = 'views.purchase_additional_cost.actions.create';
      break;
    case ViewMode.FORM_EDIT:
      titleView.value = 'views.purchase_additional_cost.actions.edit';
      break;
    default:
      titleView.value = 'views.purchase_additional_cost.page_title';
      break;
  }
};
...
const onShowNotificationTriggered = (notification: NotificationData) => {
  ...
  purchaseAdditionalCostNotification.value?.showToast();
};

const clearCache = (currentMode: ViewMode) => {
  switch (currentMode) {
    case ViewMode.FORM_CREATE:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_CREATE');
      break;
    case ViewMode.FORM_EDIT:
      cacheService.removeLastEntity('PURCHASE_ADDITIONAL_COST_EDIT');
      break;
  }
};
...
<template>
  <div class="mt-8">
    ...
    <Notification ref-key="purchaseAdditionalCostNotification" :options="{ duration: 3000 }" class="flex">
